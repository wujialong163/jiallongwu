...
int main( int argc, char **argv )
{
    ...
    int thsupport,count,pvar_index;
    MPI_T_pvar_session p_session;
    MPI_T_pvar_handle handle1,handle2;
...
/*  Initialize MPI */
    ...
    char *pvar_name[]={"mvp_coll_bcast_mcast_internode","mvp_coll_allreduce_shm_rs"};
    //pvar init 
    MPI_T_init_thread(MPI_THREAD_SINGLE,&thsupport);
    MPI_T_pvar_session_create(&p_session);
    MPI_T_pvar_get_index("mvp_coll_allreduce_shm_rs",MPI_T_PVAR_CLASS_COUNTER,&pvar_index);
    MPI_T_pvar_handle_alloc(p_session,pvar_index,&comm,&handle1,&count);
    MPI_T_pvar_get_index("mvp_coll_allreduce_pt2pt_rs_bytes_send",MPI_T_PVAR_CLASS_COUNTER,&pvar_index);
    MPI_T_pvar_handle_alloc(p_session,pvar_index,&comm,&handle2,&count);
    
    MPI_T_pvar_start(p_session, handle1);
    MPI_T_pvar_start(p_session, handle2);
    
    //MPI_T_pvar_reset(p_session, handle1);
    //MPI_T_pvar_reset(p_session, handle2);
...
#endif
    MPI_T_pvar_stop(p_session,handle1);
    MPI_T_pvar_stop(p_session,handle2);
    
    MPI_T_pvar_read(p_session,handle1,&num);
    MPI_T_pvar_read(p_session,handle2,&timer);
    printf("num=%llu   timer=%llu \n",num,timer);

    
    MPI_T_pvar_handle_free(p_session,&handle1);
    MPI_T_pvar_handle_free(p_session,&handle2);
    MPI_T_pvar_session_free(&p_session);
    MPI_T_finalize();
    MPI_Finalize();
