                                  ...
                                  MPIR_Errflag_t *errflag)
{
    ...
    if (comm_ptr->dev.ch.rank_list == NULL) {
        ...
        return MPIR_Reduce_scatter_ring(sendbuf, recvbuf, recvcnts, datatype,
                                        op, comm_ptr, errflag);
    }

    MPIR_CHKLMEM_DECL(3);

    MPIR_T_PVAR_COUNTER_INC(MVP, mvp_coll_reduce_scatter_ring_2lvl, 1);
    ...
    while (nread < max_elem_per_rank) {
        ...
        for (dist = comm_size - 1; dist >= 0; dist--) {
            ...
            if (dist > 0) {
                MPIR_PVAR_INC(reduce_scatter, ring_2lvl, send, send_count,
                              datatype);
                MPIR_PVAR_INC(reduce_scatter, ring_2lvl, recv, recv_count,
                              datatype);
