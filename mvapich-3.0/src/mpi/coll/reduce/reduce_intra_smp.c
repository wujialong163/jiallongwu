                          ...
                          MPIR_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    /* If all processes on a single node,
     * skip internode reduction in this 2lvl algo */
    if (comm_ptr->node_comm != NULL &&
        ...
        if (comm_ptr->node_comm != NULL &&
            ...
            if (mpi_errno) {
                 ...
                 */
                *errflag =
                    MPIX_ERR_PROC_FAILED == MPIR_ERR_GET_CLASS(mpi_errno) ?
        ...
        if (comm_ptr->node_roots_comm != NULL) {
            if (comm_ptr->node_roots_comm->rank !=
                ...
                const void *buf =
                    (comm_ptr->node_comm == NULL ? sendbuf : tmp_buf);
                mpi_errno = MPIR_Reduce(buf, NULL, count, datatype, op,
                                        MPIR_Get_internode_rank(comm_ptr, root),
            ...
            } else { /* I am on root's node. I have not participated in the
                        earlier reduce. */
                if (comm_ptr->rank != root) {
                    ...
                    mpi_errno =
                        MPIR_Reduce(sendbuf, tmp_buf, count, datatype, op,
                                    MPIR_Get_internode_rank(comm_ptr, root),
                                    comm_ptr->node_roots_comm, errflag);
                    if (mpi_errno) {
                        /* for communication errors, just record the error but
                         * continue */
                        *errflag = MPIX_ERR_PROC_FAILED ==
                                           MPIR_ERR_GET_CLASS(mpi_errno) ?
                                       MPIR_ERR_PROC_FAILED :
                                       MPIR_ERR_OTHER;
                        MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
                        MPIR_ERR_ADD(mpi_errno_ret, mpi_errno);
                    ...
                    /* point sendbuf at tmp_buf to make final intranode reduce
                     * easy */
                    sendbuf = tmp_buf;
                } else {
                    ...
                    mpi_errno =
                        MPIR_Reduce(sendbuf, recvbuf, count, datatype, op,
                                    MPIR_Get_internode_rank(comm_ptr, root),
                                    comm_ptr->node_roots_comm, errflag);
                    if (mpi_errno) {
                        /* for communication errors, just record the error but
                    ...
                    /* set sendbuf to MPI_IN_PLACE to make final intranode
                     * reduce easy. */
    ...
    if (comm_ptr->node_comm != NULL &&
        ...
        if (mpi_errno) {
            /* for communication errors, just record the error but continue */
            *errflag = MPIX_ERR_PROC_FAILED == MPIR_ERR_GET_CLASS(mpi_errno) ?
                           MPIR_ERR_PROC_FAILED :
                           MPIR_ERR_OTHER;
            MPIR_ERR_SET(mpi_errno, *errflag, "**fail");
            MPIR_ERR_ADD(mpi_errno_ret, mpi_errno);
