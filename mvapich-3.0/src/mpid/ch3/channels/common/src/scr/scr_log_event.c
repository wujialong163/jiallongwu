...
int main(int argc, char *argv[])
{
    ...
    if (!processArgs(argc, argv, &args)) {
        scr_err("scr_log_event: Failed to process args @ %s:%d", __FILE__,
                __LINE__);
        return 1;
    }
        ...
        if (scr_log_init() == SCR_SUCCESS) {
            ...
            if (args.username != NULL && args.jobname != NULL) {
                if (scr_log_job(args.username, args.jobname, args.start) !=
                    SCR_SUCCESS) {
                    scr_err("scr_log_event: Failed to register job, disabling "
                            "logging @ %s:%d",
                            __FILE__, __LINE__);
                    scr_log_enable = 0;
                    rc = 1;
                ...
                scr_err("scr_log_event: Missing username, jobname, or start "
            ...
            scr_err("scr_log_event: Failed to initialize SCR logging, "
