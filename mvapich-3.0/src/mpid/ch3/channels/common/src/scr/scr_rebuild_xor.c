...
static int scr_compute_crc(scr_filemap *map, int id, int rank, const char *file)
{
    ...
    if (scr_crc32(file, &crc_file) != SCR_SUCCESS) {
        scr_err("Failed to compute crc for file %s @ %s:%d", file, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (meta == NULL) {
        scr_abort(-1, "Failed to allocate meta data object @ %s:%d", __FILE__,
                  __LINE__);
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        printf("Usage: scr_rebuild_xor <size> <root> <missing_xor_filename> "
               "<ordered_remaining_xor_filenames>\n");
        return 1;
    }
    ...
    if (buffer_A == NULL || buffer_B == NULL) {
        scr_err("Failed to allocate buffer memory @ %s:%d", __FILE__, __LINE__);
        return 1;
    }
    ...
    if (xor_set_size <= 0) {
        scr_err("Invalid XOR set size argument %s @ %s:%d", argv[index - 1],
                __FILE__, __LINE__);
        return 1;
    }
    ...
    if (num_files == NULL || offsets == NULL || xor_files == NULL ||
        xor_fds == NULL || xor_headers == NULL) {
        scr_err("Failed to allocate buffer memory @ %s:%d", __FILE__, __LINE__);
        return 1;
    }
    ...
    if (root < 0 || root >= xor_set_size) {
        scr_err("Invalid root argument %s @ %s:%d", argv[index - 1], __FILE__,
                __LINE__);
        return 1;
    }
    ...
    if (xor_files[0] == NULL) {
        scr_err("Failed to dup XOR filename @ %s:%d", __FILE__, __LINE__);
        return 1;
    }

    /* read in the xor filenames (expected to be in order of XOR segment number)
     */
    /* we order ranks so that root is index 0, the rank to the right of root is
     * index 1, and so on */
    for (i = 0; i < xor_set_size; i++) {
        xor_headers[i] = scr_hash_new();

        /* we'll get the XOR file name for root from the header stored in the
         * XOR file of the partner */
        if (i == root) {
            continue;
        }
    ...
    for (i = 1; i < xor_set_size; i++) {
        ...
        if (xor_fds[i] < 0) {
            scr_err(
                "Opening xor segment file: scr_open(%s) errno=%d %s @ %s:%d",
        ...
        if (scr_hash_read_fd(xor_files[i], xor_fds[i], xor_headers[i]) < 0) {
            scr_err("Failed to read XOR header from %s @ %s:%d", xor_files[i],
                    __FILE__, __LINE__);
            return 1;
        }
    }
    ...
    if (xor_set_size >= 2) {
        ...
        scr_hash *rhs_hash =
            scr_hash_get(xor_headers[1], SCR_KEY_COPY_XOR_PARTNER);
        ...
        scr_hash *lhs_hash = scr_hash_get(xor_headers[xor_set_size - 1],
                                          SCR_KEY_COPY_XOR_CURRENT);
        ...
        if (scr_hash_util_get_int(lhs_hash, SCR_KEY_COPY_XOR_RANK,
                                  &partner_rank) != SCR_SUCCESS) {
            scr_err("Failed to read partner rank from XOR file header in %s @ "
                    "%s:%d",
    ...
    scr_hash *missing_current_hash =
        scr_hash_get(xor_headers[0], SCR_KEY_COPY_XOR_CURRENT);

    /* read the rank */
    int my_rank = -1;
    if (scr_hash_util_get_int(missing_current_hash, SCR_KEY_COPY_XOR_RANK,
                              &my_rank) != SCR_SUCCESS) {
    ...
    /* get the dataset */
    scr_dataset *dataset =
        scr_hash_get(xor_headers[0], SCR_KEY_COPY_XOR_DATASET);
    ...
    /* read the ranks */
    int num_ranks = -1;
    if (scr_hash_util_get_int(xor_headers[0], SCR_KEY_COPY_XOR_RANKS,
                              &num_ranks) != SCR_SUCCESS) {
    ...
    if (scr_hash_util_get_unsigned_long(xor_headers[0], SCR_KEY_COPY_XOR_CHUNK,
    ...
    for (i = 0; i < xor_set_size; i++) {
        ...
        scr_hash *current_hash =
            scr_hash_get(xor_headers[i], SCR_KEY_COPY_XOR_CURRENT);
        if (scr_hash_util_get_int(current_hash, SCR_KEY_COPY_XOR_FILES,
                                  &num_files[i]) != SCR_SUCCESS) {
    ...
    /* allocate space for a file descriptor, file name pointer, and filesize for
     * each user file */
    ...
    unsigned long *user_filesizes =
        (unsigned long *)malloc(total_num_files * sizeof(unsigned long));
    if (user_fds == NULL || user_files == NULL || user_rel_files == NULL ||
        user_filesizes == NULL) {
        scr_err("Failed to allocate buffer memory @ %s:%d", __FILE__, __LINE__);
        return 1;
    }
    ...
    for (i = 0; i < xor_set_size; i++) {
        ...
        /* for each file belonging to this rank, get filename, filesize, and
         * open file */
        for (j = 0; j < num_files[i]; j++) {
            ...
            scr_meta *meta =
                scr_hash_get_kv_int(current_hash, SCR_KEY_COPY_XOR_FILE, j);
            if (meta == NULL) {
                scr_err("Failed to read meta data for file %d in %s @ %s:%d", j,
                        xor_files[i], __FILE__, __LINE__);
                return 1;
            }

            /* record the filesize of this file */
            if (scr_meta_get_filesize(meta, &user_filesizes[offset]) !=
                SCR_SUCCESS) {
                scr_err(
                    "Failed to read filesize field for file %d in %s @ %s:%d",
            ...
            if (scr_meta_get_origname(meta, &origname) != SCR_SUCCESS) {
                scr_err(
                    "Failed to read original name for file %d in %s @ %s:%d", j,
                    xor_files[i], __FILE__, __LINE__);
                return 1;
            }
            ...
            if (preserve_dirs) {
                ...
                if (scr_meta_get_origpath(meta, &origpath) != SCR_SUCCESS) {
                    scr_err("Failed to read original path for file %d in %s @ "
                            "%s:%d",
                            j, xor_files[i], __FILE__, __LINE__);
                    return 1;
                }
            ...
            scr_path *path_user_rel =
                scr_path_relative(path_dset, path_user_full);
            ...
            if (i == 0) {
                ...
                if (!scr_path_is_null(user_dir_path)) {
                    ...
                    if (scr_mkdir(user_dir, mode_dir) != SCR_SUCCESS) {
                        scr_err("Failed to create directory for user file %s @ "
                                "%s:%d",
                ...
                user_fds[offset] =
                    scr_open(user_files[offset], O_WRONLY | O_CREAT | O_TRUNC,
                             mode_file);
                if (user_fds[offset] < 0) {
                    scr_err("Opening user file for writing: scr_open(%s) "
                            "errno=%d %s @ %s:%d",
                            user_files[offset], errno, strerror(errno),
                            __FILE__, __LINE__);
                    return 1;
                }
            } else {
                ...
                if (user_fds[offset] < 0) {
                    scr_err("Opening user file for reading: scr_open(%s) "
                            "errno=%d %s @ %s:%d",
                            user_files[offset], errno, strerror(errno),
                            __FILE__, __LINE__);
                    return 1;
    ...
    mode_t mode_file = scr_getmode(1, 1, 0);
    xor_fds[0] =
        scr_open(xor_files[0], O_WRONLY | O_CREAT | O_TRUNC, mode_file);
    if (xor_fds[0] < 0) {
        scr_err("Opening xor file to be reconstructed: scr_open(%s) errno=%d "
                "%s @ %s:%d",
    ...
    /* this offset array records the current position we are in the logical file
     * for each rank */
        ...
        while (nread < chunk_size && rc == 0) {
            ...
            for (i = 1; i < xor_set_size; i++) {
                ...
                if (chunk_id != ((i + root) % xor_set_size)) {
                    ...
                    if (scr_read_pad_n(
                            num_files[i], &user_files[offsets[i]],
                            &user_fds[offsets[i]], buffer_B, count, offset[i],
                            &user_filesizes[offsets[i]]) != SCR_SUCCESS) {
                    ...
                    if (scr_read_attempt(xor_files[i], xor_fds[i], buffer_B,
                                         count) != count) {
                        /* our read failed, set the return code to an error */
                        rc = 1;
                        count = 0;
                ...
                /* TODO: XORing with unsigned long would be faster here (if
                 * chunk size is multiple of this size) */
            ...
            /* at this point, we have the data from the missing rank, write it
             * out */
            if (chunk_id != root) {
                /* write chunk to logical file for the missing rank */
                if (scr_write_pad_n(num_files[0], &user_files[0], &user_fds[0],
                                    buffer_A, count, write_pos,
                                    &user_filesizes[0]) != SCR_SUCCESS) {
                ...
                if (scr_write_attempt(xor_files[0], xor_fds[0], buffer_A,
                                      count) != count) {
                    /* our write failed, set the return code to an error */
                    rc = 1;
                }
    ...
    for (j = 0; j < num_files[0]; j++) {
        ...
        if (filesize != user_filesizes[j]) {
            ...
            scr_meta *meta = scr_hash_get_kv_int(missing_current_hash,
                                                 SCR_KEY_COPY_XOR_FILE, j);
            scr_meta_set_complete(meta, 0);

            rc = 1;
        }
    }
    /* TODO: we didn't record the filesize of the XOR file for the missing rank
     * anywhere */
    ...
    /* write meta data for each of the user files and add each one to the
     * filemap */
    for (j = 0; j < num_files[0]; j++) {
        ...
        scr_meta *meta =
            scr_hash_get_kv_int(missing_current_hash, SCR_KEY_COPY_XOR_FILE, j);
    ...
    for (j = 0; j < num_files[0]; j++) {
        ...
        char *user_file_relative = user_rel_files[j];
        if (scr_compute_crc(map, dset_id, my_rank, user_file_relative) !=
            SCR_SUCCESS) {
