...
static time_t scr_flush_async_timestamp_start; /* records the time the async
                                                  flush started */
static double scr_flush_async_time_start; /* records the time the async flush
                                             started */
static scr_hash *scr_flush_async_file_list =
    NULL; /* tracks list of files written with flush */
static scr_hash *scr_flush_async_hash =
    NULL; /* tracks list of files written with flush */
static int scr_flush_async_num_files =
    0; /* records the number of files this process must flush */
...
static int scr_flush_async_file_test(const scr_hash *hash, double *bytes)
{
    ...
    *bytes = 0.0;
    ...
    scr_hash_elem *elem;
    for (elem = scr_hash_elem_first(files_hash); elem != NULL;
        ...
        if (scr_hash_util_get_bytecount(file_hash, "SIZE", &size) !=
                SCR_SUCCESS ||
            scr_hash_util_get_bytecount(file_hash, "WRITTEN", &written) !=
                SCR_SUCCESS) {
            transfer_complete = 0;
            continue;
        }
...
static int scr_flush_async_file_dequeue(scr_hash *hash1, scr_hash *hash2)
{
    ...
    if (file_hash != NULL) {
        scr_hash_elem *elem;
        for (elem = scr_hash_elem_first(file_hash); elem != NULL;
             elem = scr_hash_elem_next(elem)) {
...
static int scr_flush_async_state_wait(char *state)
{
    ...
    while (!all_valid) {
        ...
        if (scr_storedesc_cntl->rank == 0) {
            ...
            scr_hash *state_hash =
                scr_hash_get_kv(hash, SCR_TRANSFER_KEY_STATE, state);
            if (state_hash == NULL) {
                valid = 0;
            }
...
/* start an asynchronous flush from cache to parallel file system under
 * SCR_PREFIX */
int scr_flush_async_start(scr_filemap *map, int id)
{
    ...
    for (elem = scr_hash_elem_first(files); elem != NULL;
         ...
         * bytes written */
        scr_hash *transfer_file_hash =
            scr_hash_set_kv(scr_flush_async_hash, SCR_TRANSFER_KEY_FILES, file);
        if (file_hash != NULL) {
            ...
            scr_hash_util_set_str(transfer_file_hash,
                                  SCR_TRANSFER_KEY_DESTINATION, dest_file);
            scr_hash_util_set_bytecount(transfer_file_hash,
                                        SCR_TRANSFER_KEY_SIZE, filesize);
            scr_hash_util_set_bytecount(transfer_file_hash,
                                        SCR_TRANSFER_KEY_WRITTEN, 0);
    ...
    /* have master on each node write the transfer file, everyone else sends
     * data to him */
    if (scr_storedesc_cntl->rank == 0) {
        /* receive hash data from other processes on the same node and merge
         * with our data */
        int i;
        ...
        /* TODO: somewhat hacky way to determine number of nodes and therefore
         * number of writers */
        int writers;
        ...
        double percent;
        if (scr_hash_util_get_double(hash, SCR_TRANSFER_KEY_PERCENT,
                                     &percent) != SCR_SUCCESS) {
        ...
        /* unset the DONE flag */
        scr_hash_unset_kv(hash, SCR_TRANSFER_KEY_FLAG,
                          SCR_TRANSFER_KEY_FLAG_DONE);
    ...
    *bytes = 0.0;
    ...
    if (scr_storedesc_cntl->rank == 0) {
        ...
        if (scr_hash_read_with_lock(scr_transfer_file, hash) == SCR_SUCCESS) {
            ...
            if (scr_flush_async_file_test(hash, &bytes_written) !=
                SCR_SUCCESS) {
                transfer_complete = 0;
            }
        } else {
            /* failed to read the transfer file, can't determine whether the
             * flush is complete */
            transfer_complete = 0;
        }
    ...
    MPI_Allreduce(&bytes_written, bytes, 1, MPI_DOUBLE, MPI_SUM,
                  scr_comm_world);
...
int scr_flush_async_complete(scr_filemap *map, int id)
{
    ...
    /* TODO: have master tell each rank on node whether its files were written
     * successfully */
    ...
    for (elem = scr_hash_elem_first(files); elem != NULL;
        ...
        /* record the filename in the hash, and get reference to a hash for this
         * file */
        scr_path *path_file = scr_path_from_str(file);
        scr_path_basename(path_file);
        char *name = scr_path_strdup(path_file);
        scr_hash *file_hash =
            scr_hash_set_kv(data, SCR_SUMMARY_6_KEY_FILE, name);
        scr_free(&name);
    ...
    /* write summary file */
    if (scr_flush_complete(id, scr_flush_async_file_list, data) !=
        SCR_SUCCESS) {
        flushed = SCR_FAILURE;
    }
    ...
    if (scr_storedesc_cntl->rank == 0) {
        ...
        scr_hash_util_set_str(transfer_hash, SCR_TRANSFER_KEY_COMMAND,
                              SCR_TRANSFER_KEY_COMMAND_STOP);
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_dbg(1,
                "scr_flush_async_complete: %f secs, %e bytes, %f MB/s, %f MB/s "
                "per proc",
        ...
        if (flushed == SCR_SUCCESS) {
            ...
            scr_dbg(1,
                    "scr_flush_async_complete: Flush of dataset %d succeeded",
                    id);

            /* log details of flush */
            if (scr_log_enable) {
                time_t now = scr_log_seconds();
                scr_log_event("ASYNC FLUSH SUCCEEDED", NULL, &id, &now,
                              &time_diff);
            }
        } else {
            ...
            if (scr_log_enable) {
                time_t now = scr_log_seconds();
                scr_log_event("ASYNC FLUSH FAILED", NULL, &id, &now,
                              &time_diff);
            }
        }
    }
...
int scr_flush_async_wait(scr_filemap *map)
{
    if (scr_flush_async_in_progress) {
        while (scr_bool_is_flushing(scr_flush_async_dataset_id)) {
            /* test whether the flush has completed, and if so complete the
             * flush */
            double bytes = 0.0;
            if (scr_flush_async_test(map, scr_flush_async_dataset_id, &bytes) ==
                SCR_SUCCESS) {
                /* complete the flush */
