 ...
 *
 * <variable length block table> (excluded from SCR_FILE_COMPRESSED_HEADER_SIZE
 * constant)
 *
 * (4) uint32_t header crc (from first byte of magic number to last byte of
 * block table) */
...
static int scr_compress_zlib(int level, /* set compression level 0=none, 9=max
                                         */
                             const void *inbuf, size_t insize, void *outbuf,
                             size_t outsize, size_t *outwritten)
{
    int rc = SCR_SUCCESS;
    ...
    *outwritten = 0;
    ...
    do {
        ...
        if (ret == Z_OK || ret == Z_BUF_ERROR || ret == Z_STREAM_END) {
            ...
            scr_err("Error during compression (ret=%d) @ %s:%d", ret, __FILE__,
                    __LINE__);
            rc = SCR_FAILURE;
        }
    } while (strm.avail_in != 0 && strm.avail_out != 0 && ret != Z_BUF_ERROR &&
             rc == SCR_SUCCESS);

    /* check that we compressed the entire block */
    if (strm.avail_in != 0 || ret != Z_STREAM_END) {
        scr_err("Failed to compress @ %s:%d", __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }
...
/* compress insize bytes from in inbuf and store in outbuf which has
 * up to outsize bytes available, return number written in outwritten */
static int scr_uncompress_zlib(const void *inbuf, size_t insize, void *outbuf,
                               size_t outsize, size_t *outwritten)
{
    /* initialize output parameters */
    *outwritten = 0;
    ...
    do {
        ...
        if (ret == Z_NEED_DICT || ret == Z_DATA_ERROR || ret == Z_MEM_ERROR ||
            ret == Z_STREAM_ERROR) {
            /* hit an error of some sort */
            scr_err("Error during decompression (ret=%d) @ %s:%d", ret,
                    __FILE__, __LINE__);
            rc = SCR_FAILURE;
        } else {
    ...
    if (strm.avail_in != 0 || ret != Z_STREAM_END) {
        scr_err("Failed to decompress @ %s:%d", __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }
...
int scr_compress_buf(const void *inbuf, size_t insize, void **outbuf,
                     size_t *outsize)
{
    ...
    *outsize = 0;
    ...
    if (buf == NULL) {
        scr_abort(-1,
                  "Allocating compress buffer malloc(%ld) errno=%d %s @ %s:%d",
    ...
    scr_pack_uint32_t(header, header_size, &header_offset,
                      (uint32_t)SCR_FILE_MAGIC);
    scr_pack_uint16_t(header, header_size, &header_offset,
                      (uint16_t)SCR_FILE_TYPE_COMPRESSED);
    scr_pack_uint16_t(header, header_size, &header_offset,
                      (uint16_t)SCR_FILE_VERSION_COMPRESSED_1);

    /* write the size of the header, the original file size, block size, and
     * number of blocks */
    scr_pack_uint64_t(header, header_size, &header_offset,
    ...
    scr_pack_uint64_t(header, header_size, &header_offset,
                      (uint64_t)block_offset_cmp);
    scr_pack_uint64_t(header, header_size, &header_offset,
                      (uint64_t)block_size_cmp);
 ...
 * returns SCR_SUCCESS if successful, in which case, caller must free buffer */
int scr_uncompress_buf(const void *inbuf, size_t insize, void **outbuf,
                       size_t *outsize)
{
    /* set outbuf to NULL and outsize to 0 */
    *outbuf = NULL;
    *outsize = 0;
    ...
    if (magic != SCR_FILE_MAGIC || type != SCR_FILE_TYPE_COMPRESSED ||
    ...
    char *buf_src = (char *)header + SCR_FILE_COMPRESSED_HEADER_SIZE +
                    num_blocks * (2 * sizeof(uint64_t) + 2 * sizeof(uint32_t));
    ...
    if (crc_cmp != file_crc_cmp) {
        ...
        scr_err("CRC32 mismatch detected in compressed block @ %s:%d", __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (buf == NULL) {
        scr_abort(-1,
                  "Allocating buffer to decompress data malloc(%ld) errno=%d "
                  "%s @ %s:%d",
    ...
    size_t written;
    int rc =
        scr_uncompress_zlib(buf_src, block_size_cmp, buf, block_size, &written);
...
/* compress the specified file using blocks of size block_size and store as
 * file_dst */
int scr_compress_in_place(const char *file_src, const char *file_dst,
                          unsigned long block_size, int level)
{
    /* set compression level 0=none, 9=max */
    //  int compression_level = Z_DEFAULT_COMPRESSION;
    int compression_level = level;
    ...
    if (file_src == NULL || file_dst == NULL) {
        scr_err("NULL filename @ %s:%d", __FILE__, __LINE__);
        return SCR_FAILURE;
    }

    /* check that the source file exists and that we can read and write to it */
    if (access(file_src, F_OK | R_OK | W_OK) != 0) {
        scr_err("File %s does not exist or does not have read/write permission "
                "@ %s:%d",
    ...
    if (header == NULL) {
        scr_err("Allocating header buffer when compressing %s: malloc(%ld) "
                "errno=%d %s @ %s:%d",
                file_src, header_size, errno, strerror(errno), __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (buf_src == NULL) {
        scr_err("Allocating source buffer when compressing %s: malloc(%ld) "
                "errno=%d %s @ %s:%d",
                file_src, block_size, errno, strerror(errno), __FILE__,
                __LINE__);
        scr_free(&header);
        return SCR_FAILURE;
    }
    ...
    if (buf_dst == NULL) {
        scr_err("Allocating compress buffer when compressing %s: malloc(%ld) "
                "errno=%d %s @ %s:%d",
                file_src, block_size, errno, strerror(errno), __FILE__,
                __LINE__);
        scr_free(&buf_src);
    ...
    if (fd_src < 0) {
        scr_err("Opening file: %s errno=%d %s @ %s:%d", file_src, errno,
     ...
     * we must make sure that we never overrun the original data when
     * compressing */
    off_t pos_src = 0;
    off_t pos_dst = 0;
    ...
    while (compressing && rc == SCR_SUCCESS) {
        ...
        if (lseek(fd_src, (off_t)pos_src, SEEK_SET) == (off_t)-1) {
            scr_err("Seek to read position failed in %s @ %s:%d", file_src,
                    __FILE__, __LINE__);
            rc = SCR_FAILURE;
        }
            ...
            do {
                ...
                do {
                    ...
                    if (ret == Z_OK || ret == Z_BUF_ERROR ||
                        ret == Z_STREAM_END) {
                        /* compute number of bytes written by this call to
                         * deflate */
                        have = block_size - strm.avail_out;
                    } else {
                        /* hit an error of some sort */
                        scr_err(
                            "Error during compression in %s (ret=%d) @ %s:%d",
                            file_src, ret, __FILE__, __LINE__);
                        rc = SCR_FAILURE;
                    }
                } while (strm.avail_in != 0 && strm.avail_out != 0 &&
                         ret != Z_BUF_ERROR && rc == SCR_SUCCESS);

                /* TODO: if compression produces very small blocks, this will be
                 * inefficient, would be better to use a buffered write like
                 * fwrite here */
                ...
                if (have > 0 && rc == SCR_SUCCESS) {
                    /* compute crc of compressed block */
                    crc_cmp =
                        crc32(crc_cmp, (const Bytef *)buf_dst, (uInt)have);

                    /* check that we won't overrun our read position when we
                     * write out this data */
                    ...
                    block_size_cmp += have;
                     ...
                     rc == SCR_SUCCESS); /* strm.avail_out == 0 */

            /* check that we compressed the entire block */
            if (strm.avail_in != 0 || ret != Z_STREAM_END) {
                scr_err("Failed to compress file %s @ %s:%d", file_src,
                        __FILE__, __LINE__);
                rc = SCR_FAILURE;
            }
    ...
    if (lseek(fd_src, (off_t)0, SEEK_SET) == (off_t)-1) {
        scr_err("Seek to beginning of header failed in %s @ %s:%d", file_src,
                __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }
    ...
    if (nwrite_header != header_size) {
        scr_err("Failed to write header to file %s @ %s:%d", file_src, __FILE__,
                __LINE__);
        rc = SCR_FAILURE;
    }
...
/* compress the specified file using blocks of size block_size and store as
 * file_dst */
int scr_compress(const char *file_src, const char *file_dst,
                 unsigned long block_size, int level)
{
    /* set compression level 0=none, 9=max */
    //  int compression_level = Z_DEFAULT_COMPRESSION;
    ...
    if (access(file_src, F_OK | R_OK) != 0) {
        scr_err(
            "File %s does not exist or does not have read permission @ %s:%d",
            file_src, __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (fd_src < 0) {
        scr_err("Opening file for reading: %s errno=%d %s @ %s:%d", file_src,
    ...
    if (fd_dst < 0) {
        scr_err("Opening file for writing: %s errno=%d %s @ %s:%d", file_dst,
                errno, strerror(errno), __FILE__, __LINE__);
        scr_close(file_src, fd_src);
        scr_free(&buf_dst);
        scr_free(&buf_src);
    ...
    if (lseek(fd_dst, (off_t)header_size, SEEK_SET) == (off_t)-1) {
        scr_err("Seek to end of header failed in %s @ %s:%d", file_dst,
                __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }
    ...
    if (lseek(fd_dst, (off_t)0, SEEK_SET) == (off_t)-1) {
        scr_err("Seek to beginning of header failed in %s @ %s:%d", file_dst,
                __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }
    ...
    if (nwrite_header != header_size) {
        scr_err("Failed to write header to file %s @ %s:%d", file_dst, __FILE__,
                __LINE__);
        rc = SCR_FAILURE;
    }
...
int scr_uncompress_in_place(const char *file_src, const char *file_dst)
{
    ...
    if (nread < sizeof(buf)) {
        scr_err("Failed to read header from file %s @ %s:%d", file_src,
                __FILE__, __LINE__);
        scr_close(file_src, fd_src);
        return SCR_FAILURE;
    }
    ...
    if (magic != SCR_FILE_MAGIC || type != SCR_FILE_TYPE_COMPRESSED ||
        version != SCR_FILE_VERSION_COMPRESSED_1) {
        scr_err(
            "File type does not match values for a compressed file %s @ %s:%d",
            file_src, __FILE__, __LINE__);
        scr_close(file_src, fd_src);
        return SCR_FAILURE;
    }
    ...
    void *header = malloc(header_size);
    if (header == NULL) {
        scr_err("Allocating header buffer when decompressing %s: malloc(%ld) "
                "errno=%d %s @ %s:%d",
                file_src, header_size, errno, strerror(errno), __FILE__,
                __LINE__);
        scr_close(file_src, fd_src);
    ...
    void *buf_src = scr_align_malloc((size_t)block_size, page_size);
    if (buf_src == NULL) {
        scr_err("Allocating source buffer when decompressing %s: malloc(%ld) "
                "errno=%d %s @ %s:%d",
                file_src, block_size, errno, strerror(errno), __FILE__,
                __LINE__);
        scr_free(&header);
    ...
    void *buf_dst = scr_align_malloc((size_t)block_size, page_size);
    if (buf_dst == NULL) {
        scr_err("Allocating compress buffer when decompressing %s: malloc(%ld) "
                "errno=%d %s @ %s:%d",
                file_src, block_size, errno, strerror(errno), __FILE__,
                __LINE__);
        scr_free(&buf_src);
    ...
    if (lseek(fd_src, 0, SEEK_SET) == (off_t)-1) {
        scr_err("Failed to seek to start of file %s @ %s:%d", file_src,
                __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }
    ...
    if (nread < header_size) {
        scr_err("Failed to read in header from file %s @ %s:%d", file_src,
                __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }
    ...
    /* check that crc values match */
    if ((uLong)crc_header != crc) {
        scr_err("CRC32 mismatch detected in header of %s @ %s:%d", file_src,
                __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }

    /* set pointer to end of block table */
    header_offset = sizeof(uint32_t) + 2 * sizeof(uint16_t) +
                    4 * sizeof(uint64_t) +
                    num_blocks * (2 * sizeof(uint64_t) + 2 * sizeof(uint32_t));
    ...
    while (block_count < num_blocks && rc == SCR_SUCCESS) {
        ...
        uint64_t block_offset_cmp, block_size_cmp;
        uint32_t file_crc_cmp, file_crc_orig;
        scr_unpack_uint64_t(header, header_size, &header_offset,
                            &block_offset_cmp);
        scr_unpack_uint64_t(header, header_size, &header_offset,
                            &block_size_cmp);
        scr_unpack_uint32_t(header, header_size, &header_offset, &file_crc_cmp);
        scr_unpack_uint32_t(header, header_size, &header_offset,
                            &file_crc_orig);
        ...
        int ret = inflateInit(&strm);
        if (ret != Z_OK) {
            scr_err("Failed to initialize decompression stream when processing "
                    "%s (ret=%d) @ %s:%d",
                    file_src, ret, __FILE__, __LINE__);
            rc = SCR_FAILURE;
        }
        ...
        while (total_read < block_size_cmp && rc == SCR_SUCCESS) {
            ...
            /* TODO: these reads will be inefficient for very small compressed
             * blocks
            ...
            if (nread > 0) {
                /* TODO: would be nice to handle this case, but we don't yet
                 * when doing in place decompression */
                if (nread < block_size_cmp) {
                    scr_err("Failed to read full compressed block from file %s "
                            "@ %s:%d",
                            file_src, __FILE__, __LINE__);
                    rc = SCR_FAILURE;
                }
                ...
                do {
                    ...
                    do {
                        ...
                        if (ret == Z_NEED_DICT || ret == Z_DATA_ERROR ||
                            ...
                            /* hit an error of some sort */
                            scr_err("Error during decompression in %s (ret=%d) "
                                    "@ %s:%d",
                                    file_src, ret, __FILE__, __LINE__);
                            rc = SCR_FAILURE;
                        } else {
                            /* compute number of uncompressed bytes written so
                             * far */
                            have = block_size - strm.avail_out;
                        }
                    } while (strm.avail_in != 0 && strm.avail_out != 0 &&
                    ...
                    if (have > 0 && rc == SCR_SUCCESS) {
                        /* compute crc of uncompressed block */
                        crc_orig =
                            crc32(crc_orig, (const Bytef *)buf_dst, (uInt)have);

                        /* determine byte location to start writing this data */
                        pos_dst = (num_blocks - block_count - 1) * block_size +
                                  total_written;
                        ...
                        off_t pos_end =
                            pos_src; /* TODO: Here, we assume that we read the
                                        entire block starting at pos_src */
                        if (pos_dst < pos_end && pos_src != header_size) {
                            /* TODO: unwind what decompression we have done if
                             * any, for now we just make this a fatal error */
                            scr_err(
                                "Failed to decompress file in place %s @ %s:%d",
                                file_src, __FILE__, __LINE__);
                            rc = SCR_FAILURE;
                        }

                        /* seek to current location for writing */
                        if (lseek(fd_src, (off_t)pos_dst, SEEK_SET) ==
                            (off_t)-1) {
                            scr_err(
                                "Seek to write position failed in %s @ %s:%d",
                                file_src, __FILE__, __LINE__);
                            rc = SCR_FAILURE;
                        }
                        ...
                        ssize_t nwrite =
                            scr_write(file_src, fd_src, buf_dst, have);
                        if (nwrite != have) {
                            scr_err("Error writing to %s @ %s:%d", file_src,
                                    __FILE__, __LINE__);
                            rc = SCR_FAILURE;
                        }

                        /* update the number of uncompressed bytes we've written
                         * for this block */
                        if (nwrite > 0) {
                ...
                /* check that we uncompressed the entire block */
                if (strm.avail_in != 0 || ret != Z_STREAM_END) {
                    scr_err("Failed to decompress file %s @ %s:%d", file_src,
                            __FILE__, __LINE__);
                    rc = SCR_FAILURE;
                }
        ...
        if (crc_cmp != file_crc_cmp) {
            scr_err(
                "CRC32 mismatch detected in compressed block #%d in %s @ %s:%d",
                block_count, file_src, __FILE__, __LINE__);
            rc = SCR_FAILURE;
        }
        if (crc_orig != file_crc_orig) {
            scr_err("CRC32 mismatch detected in decompressed block #%d in %s @ "
                    "%s:%d",
                    block_count, file_src, __FILE__, __LINE__);
            rc = SCR_FAILURE;
        }
...
int scr_uncompress(const char *file_src, const char *file_dst)
{
    ...
    /* set header offset to point to entry for first block */
    header_offset =
        sizeof(uint32_t) + 2 * sizeof(uint16_t) + 4 * sizeof(uint64_t);
            ...
            if (nread > 0) {
                ...
                do {
                    ...
                    if (have > 0 && rc == SCR_SUCCESS) {
                        ...
                        /* write uncompressed data to file */
                        ssize_t nwrite =
                            scr_write(file_dst, fd_dst, buf_dst, have);
                        if (nwrite != have) {
                            scr_err("Error writing to %s @ %s:%d", file_dst,
                                    __FILE__, __LINE__);
                            rc = SCR_FAILURE;
                        }
                    }
