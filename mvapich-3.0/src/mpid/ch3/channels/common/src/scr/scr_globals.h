...
extern char scr_cntl_base[SCR_MAX_FILENAME];  /* base directory for control
                                                 directory */
extern char scr_cache_base[SCR_MAX_FILENAME]; /* base directory for cache
                                                 directory */

extern char *scr_cntl_prefix; /* path of control directory (adds to base
                                 directory) */
...
extern int scr_in_output;     /* flag tracks whether we are between start and
                                 complete calls */
extern int scr_initialized;   /* indicates whether the library has been
                                 initialized */
...
extern int scr_page_size;     /* records block size for aligning MPI and file
                                 buffers */
...
extern size_t scr_file_buf_size; /* set buffer size to chunk file copies to/from
                                    parallel file system */

extern int scr_halt_seconds; /* secs remaining in allocation before job should
                                be halted */

extern int scr_distribute; /* whether to call scr_distribute_files during
                              SCR_Init */
extern int scr_fetch;      /* whether to call scr_fetch_files during SCR_Init */
extern int scr_fetch_width;      /* specify number of processes to read files
                                    simultaneously */
extern int scr_flush;            /* how many checkpoints between flushes */
extern int scr_flush_width;      /* specify number of processes to write files
                                    simultaneously */
extern int scr_flush_on_restart; /* specify whether to flush cache on restart */
extern int scr_global_restart;   /* set if code must be restarted from parallel
                                    file system */
...
extern double scr_flush_async_bw; /* bandwidth limit imposed during async flush
                                   */
extern double scr_flush_async_percent;  /* runtime limit imposed during async
                                           flush */
extern int scr_flush_async_in_progress; /* tracks whether an async flush is
                                           currently underway */
extern int scr_flush_async_dataset_id;  /* tracks the id of the checkpoint being
                                           flushed */
extern double scr_flush_async_bytes; /* records the total number of bytes to be
                                        flushed */

extern int scr_crc_on_copy;   /* whether to enable crc32 checks during
                                 scr_swap_files() */
extern int scr_crc_on_flush;  /* whether to enable crc32 checks during flush and
                                 fetch */
extern int scr_crc_on_delete; /* whether to enable crc32 checks when deleting
                                 checkpoints */

extern int scr_preserve_directories; /* whether to preserve user-defined
                                        directories during flush */
extern int scr_use_containers; /* whether to fetch from / flush to container
                                  files */
extern unsigned long scr_container_size; /* max number of bytes to store in a
                                            container */

extern int scr_checkpoint_interval; /* times to call Need_checkpoint between
                                       checkpoints */
...
extern double scr_checkpoint_overhead; /* max allowed overhead for checkpointing
                                        */
extern int scr_need_checkpoint_count;  /* tracks the number of times
                                          Need_checkpoint has been called */
extern double scr_time_checkpoint_total; /* keeps a running total of the time
                                            spent to checkpoint */
extern int scr_time_checkpoint_count; /* keeps a running count of the number of
                                         checkpoints taken */

extern time_t scr_timestamp_checkpoint_start; /* record timestamp of start of
                                                 checkpoint */
extern double scr_time_checkpoint_start;      /* records the start time of the
                                                 current checkpoint */
extern double scr_time_checkpoint_end; /* records the end time of the current
                                          checkpoint */

extern time_t scr_timestamp_compute_start; /* record timestamp of start of
                                              compute phase */
extern double scr_time_compute_start; /* records the start time of the current
                                         compute phase */
extern double scr_time_compute_end;   /* records the end time of the current
                                         compute phase */
...
extern MPI_Comm scr_comm_node_across; /* communicator of tasks with same rank on
                                         each node */

extern scr_hash *scr_groupdesc_hash; /* hash defining group descriptors to be
                                        used */
extern scr_hash *scr_storedesc_hash; /* hash defining store descriptors to be
                                        used */
extern scr_hash *scr_reddesc_hash; /* hash defining redudancy descriptors to be
                                      used */
...
extern scr_storedesc *scr_storedesc_cntl; /* store descriptor struct for control
                                             directory */

extern int scr_nreddescs; /* number of redundancy descriptors in scr_reddescs
                             list */
