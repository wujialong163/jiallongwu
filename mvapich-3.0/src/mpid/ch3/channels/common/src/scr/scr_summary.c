...
/* read in the summary file from dir assuming file is using version 4 format or
 * earlier, convert to version 5 hash */
static int scr_summary_read_v4_to_v5(const scr_path *dir,
                                     scr_hash *summary_hash)
{
    ...
    if (fs == NULL) {
        scr_err("Opening summary file for read: fopen(%s, \"r\") errno=%d %s @ "
                "%s:%d",
    ...
    /* all versions greater than 1, have two header lines, read and throw away
     * the second */
    if (version > 1) {
        /* version 3 and higher writes the number of rows in the file (ranks may
         * write 0 or more files) */
        if (version >= 3) {
            fgets(line, sizeof(line), fs);
            linenum++;
     ...
     */
    if (num_records <= 0) {
        scr_err("No file records found in summary file %s, perhaps it is "
                "corrupt or incomplete @ %s:%d",
    ...
    /* set the version number in the summary hash, initialize a pointer to the
     * checkpoint hash */
    scr_hash_set_kv_int(summary_hash, SCR_SUMMARY_KEY_VERSION,
                        SCR_SUMMARY_FILE_VERSION_5);
    ...
    for (i = 0; i < num_records; i++) {
        ...
        if (version == 1) {
            ...
            n = fscanf(fs, "%d\t%d\t%d\t%d\t%d\t%d\t%lu\t%d\t%lu\t%s\n", &rank,
                       &scr, &ranks, &pattern, &checkpoint_id, &complete,
        ...
        if (n == EOF) {
            scr_err("Early EOF in summary file %s at line %d.  Only read %d of "
                    "%d expected records @ %s:%d",
            ...
            scr_err("Invalid read of record %d in %s at line %d @ %s:%d", i,
                    summary_file, linenum, __FILE__, __LINE__);
            fclose(fs);
            scr_hash_unset_all(summary_hash);
            scr_free(&summary_file);
            return SCR_FAILURE;
        ...
        /* TODO: check whether all files are complete, match expected size,
         * number of ranks, checkpoint_id, etc */
        if (rank < 0 || rank >= scr_ranks_world) {
            bad_values = 1;
            scr_err("Invalid rank detected (%d) in a job with %d tasks in %s "
                    "at line %d @ %s:%d",
                    rank, scr_my_rank_world, summary_file, linenum, __FILE__,
                    __LINE__);
        }
        ...
        if (ckpt_hash == NULL) {
            ...
            ckpt_hash = scr_hash_set_kv_int(
                summary_hash, SCR_SUMMARY_5_KEY_CKPT, checkpoint_id);
        }

        /* get a pointer to the hash for this rank, and then to the file for
         * this rank */
        scr_hash *rank_hash =
            scr_hash_set_kv_int(ckpt_hash, SCR_SUMMARY_5_KEY_RANK, rank);
        scr_hash *file_hash =
            scr_hash_set_kv(rank_hash, SCR_SUMMARY_5_KEY_FILE, base);

        /* set the file size, and the crc32 value if it was computed */
        scr_hash_util_set_bytecount(file_hash, SCR_SUMMARY_5_KEY_SIZE,
                                    exp_filesize);
        if (crc_computed) {
        ...
        /* check that the checkpoint id matches all other checkpoint ids in the
         * file */
        if (checkpoint_id != all_checkpoint_id) {
            if (all_checkpoint_id == -1) {
                ...
                bad_values = 1;
                scr_err("Checkpoint id %d on record %d does not match expected "
                        "checkpoint id %d in %s at line %d @ %s:%d",
                        checkpoint_id, i, all_checkpoint_id, summary_file,
                        linenum, __FILE__, __LINE__);
            }
        }

        /* check that the number of ranks matches all the number of ranks
         * specified by all other records in the file */
        if (ranks != all_ranks) {
            if (all_ranks == -1) {
                all_ranks = ranks;
            } else {
                bad_values = 1;
                scr_err("Number of ranks %d on record %d does not match "
                        "expected number of ranks %d in %s at line %d @ %s:%d",
                        ranks, i, all_ranks, summary_file, linenum, __FILE__,
                        __LINE__);
            }
        }
    }

    /* we've read in all of the records, now set the values for the complete
     * field and the number of ranks field */
    if (ckpt_hash != NULL) {
        scr_hash_set_kv_int(ckpt_hash, SCR_SUMMARY_5_KEY_COMPLETE,
                            all_complete);
    ...
    /* if we found any problems while reading the file, clear the hash and
...
static int scr_summary_check_v5(scr_hash *hash)
{
    ...
    int version;
    if (scr_hash_util_get_int(hash, SCR_SUMMARY_KEY_VERSION, &version) !=
        SCR_SUCCESS) {
    ...
    if (version != SCR_SUMMARY_FILE_VERSION_5) {
        ...
        scr_err("Found version number %d when %d was expected in summary file "
                "@ %s:%d",
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    int complete;
    if (scr_hash_util_get_int(ckpt, SCR_SUMMARY_5_KEY_COMPLETE, &complete) !=
        SCR_SUCCESS) {
    ...
    int ranks;
    if (scr_hash_util_get_int(ckpt, SCR_SUMMARY_5_KEY_RANKS, &ranks) !=
        SCR_SUCCESS) {
    ...
    /* check that the number of ranks matches the number we're currently running
     * with */
    if (ranks != scr_ranks_world) {
        scr_err("Number of ranks %d that wrote checkpoint does not match "
                "current number of ranks %d @ %s:%d",
...
static int scr_summary_read_v5(const scr_path *dir, scr_hash *summary_hash)
{
    ...
    if (scr_hash_read_path(summary_path, summary_hash) != SCR_SUCCESS) {
        scr_err("Reading summary file %s @ %s:%d", summary_file, __FILE__,
                __LINE__);
        goto cleanup;
    }
...
static int scr_summary_read_v6(const scr_path *dir, scr_hash *summary_hash)
{
    ...
    int version;
    if (scr_hash_util_get_int(summary_hash, SCR_SUMMARY_KEY_VERSION,
                              &version) != SCR_SUCCESS) {
    ...
    if (version != SCR_SUMMARY_FILE_VERSION_6) {
        scr_err("Summary file %s is version %d instead of version %d @ %s:%d",
                summary_file, version, SCR_SUMMARY_FILE_VERSION_6, __FILE__,
                __LINE__);
        goto cleanup;
    }
...
int scr_summary_read(const scr_path *dir, scr_hash *summary_hash)
{
    ...
    if (scr_summary_read_v6(dir, summary_hash) != SCR_SUCCESS) {
        ...
        if (scr_summary_read_v5(dir, summary_hash_v5) != SCR_SUCCESS) {
            /* failed to read the summary file, try again, but now assume an
             * older format */
            if (scr_summary_read_v4_to_v5(dir, summary_hash_v5) !=
                SCR_SUCCESS) {
                /* we still failed, don't report an error here, just return
                 * failure, the read functions will report errors as needed */
                scr_err("Reading summary file in %s @ %s:%d", dir, __FILE__,
                        __LINE__);
        ...
        if (scr_summary_check_v5(summary_hash_v5) != SCR_SUCCESS) {
            ...
            scr_err("Invalid version 5 summary file in %s @ %s:%d", dir,
                    __FILE__, __LINE__);
            scr_hash_delete(&summary_hash_v5);
            return SCR_FAILURE;
        }
        ...
        if (scr_summary_convert_v5_to_v6(summary_hash_v5, summary_hash) !=
            SCR_SUCCESS) {
...
int scr_summary_write(const scr_path *dir, const scr_dataset *dataset,
                      int all_complete, scr_hash *data)
{
    /* build the summary filename */
    scr_path *summary_path = scr_path_dup(dir);
    scr_path_append_str(summary_path, ".scr");
    ...
    scr_hash *rank2file_hash =
        scr_hash_get(summary_hash, SCR_SUMMARY_6_KEY_RANK2FILE);
    scr_hash_util_set_int(rank2file_hash, SCR_SUMMARY_6_KEY_RANKS,
                          scr_ranks_world);
