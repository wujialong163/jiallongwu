...
{
    printf("\n");
    printf("scr_log_transfer -- record a file transfer operation in the SCR "
           "log\n");
    printf("\n");
    printf("Options:\n");
    printf("  -u <username>  Username of job owner, reads $USER if not "
           "specified\n");
    printf("  -j <jobname>   Job name of job, reads $SCR_JOB_NAME if not "
           "specified\n");
    printf("  -s <seconds>   Job start time, uses current UNIX timestamp if "
           "not specified\n");
    printf("\n");
    ...
    printf(
        "  -S <start>     Transfer start time as UNIX timestamp (integer)\n");
...
int main(int argc, char *argv[])
{
    ...
    if (!processArgs(argc, argv, &args)) {
        scr_err("scr_log_transfer: Failed to process args @ %s:%d", __FILE__,
                __LINE__);
        return 1;
    }
    ...
    if (scr_log_enable) {
        ...
        if (scr_log_init() == SCR_SUCCESS) {
            ...
            if (args.username != NULL && args.jobname != NULL) {
                if (scr_log_job(args.username, args.jobname, args.start) !=
                    SCR_SUCCESS) {
                    scr_err("scr_log_transfer: Failed to register job, "
                ...
                scr_err("scr_log_transfer: Missing username, jobname, or start "
                        "time, disabling logging @ %s:%d",
                        __FILE__, __LINE__);
                scr_log_enable = 0;
                rc = 1;
            }
        } else {
            scr_err("scr_log_transfer: Failed to initialize SCR logging, "
                    "disabling logging @ %s:%d",
                    __FILE__, __LINE__);
            scr_log_enable = 0;
            rc = 1;
    ...
    if (scr_log_enable) {
        //  if (scr_log_event(args.event_type, args.event_note, args.event_ckpt,
        //  args.event_start, args.event_secs) != SCR_SUCCESS)
        if (scr_log_transfer(args.transfer_type, args.transfer_from,
                             args.transfer_to, args.transfer_ckpt,
                             args.transfer_start, args.transfer_secs,
                             args.transfer_bytes) != SCR_SUCCESS) {
            rc = 1;
        }
    }
