 ...
 */
static scr_hash *scr_filemap_get_rdf(const scr_hash *hash, int dset, int rank,
                                     const char *file)
{
...
/* unset RANK/DSET and DSET/RANK indicies if the map for this rank and dataset
 * is empty */
...
/* adds a new filename to the filemap and associates it with a specified dataset
 * id and a rank */
...
int scr_filemap_remove_file(scr_filemap *map, int dset, int rank,
                            const char *file)
{
...
/* copies the redundancy descriptor hash for the given rank and dataset id into
 * hash */
int scr_filemap_get_desc(const scr_filemap *map, int dset, int rank,
                         scr_hash *hash)
{
    /* get RANK/DSET hash */
    scr_hash *rd = scr_filemap_get_rd(map, dset, rank);
...
int scr_filemap_set_flushdesc(scr_filemap *map, int dset, int rank,
                              scr_hash *hash)
{
    /* set indicies and get hash reference */
    scr_hash *rd = scr_filemap_set_rd(map, dset, rank);
...
/* copies the flush/scavenge descriptor hash for the given rank and dataset id
 * into hash */
int scr_filemap_get_flushdesc(const scr_filemap *map, int dset, int rank,
                              scr_hash *hash)
{
    /* get RANK/DSET hash */
    scr_hash *rd = scr_filemap_get_rd(map, dset, rank);
...
int scr_filemap_set_dataset(scr_filemap *map, int dset, int rank,
                            scr_hash *hash)
{
    /* set indicies and get hash reference */
    scr_hash *rd = scr_filemap_set_rd(map, dset, rank);
...
int scr_filemap_get_dataset(const scr_filemap *map, int dset, int rank,
                            scr_hash *hash)
{
    /* get RANK/CKPT hash */
    scr_hash *rd = scr_filemap_get_rd(map, dset, rank);
...
int scr_filemap_set_expected_files(scr_filemap *map, int dset, int rank,
                                   int expect)
{
    /* set indicies and get hash reference */
    scr_hash *rd = scr_filemap_set_rd(map, dset, rank);
...
/* return the number of expected files in the hash for a given dataset id and
 * rank */
...
int scr_filemap_set_meta(scr_filemap *map, int dset, int rank, const char *file,
                         const scr_meta *meta)
{
...
int scr_filemap_get_meta(const scr_filemap *map, int dset, int rank,
                         const char *file, scr_meta *meta)
{
    /* get RANK/DSET/FILE hash */
    scr_hash *rdf = scr_filemap_get_rdf(map, dset, rank, file);
...
int scr_filemap_unset_meta(scr_filemap *map, int dset, int rank,
                           const char *file)
{
    /* set indicies and get hash reference */
    scr_hash *rdf = scr_filemap_get_rdf(map, dset, rank, file);
...
{
    /* remove dataset from the RANK/DSET index, and remove RANK if that was the
     * last item */
    scr_hash *r = scr_filemap_get_r(map, rank);
    scr_hash_unset_kv_int(r, SCR_FILEMAP_KEY_DSET, dset);
    if (scr_hash_size(r) == 0) {
    ...
    /* remove rank from the DSET/RANK index, and remove DSET if that was the
     * last item */
    scr_hash *d = scr_filemap_get_d(map, dset);
    scr_hash_unset_kv_int(d, SCR_FILEMAP_KEY_RANK, rank);
    if (scr_hash_size(d) == 0) {
...
int scr_filemap_remove_rank(scr_filemap *map, int rank)
{
    ...
    while (dset_elem != NULL) {
        ...
        /* get pointer to the next dataset, since we will remove the current one
         * from the list */
...
int scr_filemap_remove_dataset(scr_filemap *map, int dset)
{
    ...
    while (rank_elem != NULL) {
        ...
        /* get pointer to the next rank, since we will remove the current one
         * from the list */
...
int scr_filemap_clear(scr_filemap *map) { return scr_hash_unset_all(map); }
...
int scr_filemap_latest_dataset(const scr_filemap *map)
{
    ...
    if (hash != NULL) {
        scr_hash_elem *elem;
        for (elem = scr_hash_elem_first(hash); elem != NULL;
...
/* returns the oldest dataset id (smallest int larger than younger_than) in
 * given map */
int scr_filemap_oldest_dataset(const scr_filemap *map, int younger_than)
{
    ...
    /* now scan through each dataset and find the smallest id that is larger
     * than younger_than */
    scr_hash *hash = scr_filemap_get_dh(map);
    if (hash != NULL) {
        scr_hash_elem *elem;
...
int scr_filemap_list_ranks_by_dataset(const scr_filemap *map, int dset, int *n,
                                      int **v)
{
    scr_hash *d = scr_filemap_get_d(map, dset);
...
/* given a filemap, a dataset id, and a rank, return the number of files and a
 * list of the filenames */
int scr_filemap_list_files(const scr_filemap *map, int dset, int rank, int *n,
                           char ***v)
{
    ...
    *v = NULL;
    ...
    if (list == NULL) {
        scr_err("Failed to allocate filename list for dataset id %d and rank "
                "%d at %s:%d",
    ...
    count = 0;
    scr_hash_elem *file;
    for (file = scr_hash_elem_first(fh); file != NULL;
...
/* given a filemap, return a hash elem pointer to the first rank for a given
 * dataset */
scr_hash_elem *scr_filemap_first_rank_by_dataset(const scr_filemap *map,
                                                 int dset)
{
    scr_hash *d = scr_filemap_get_d(map, dset);
    scr_hash *rh = scr_filemap_get_rh(d);
...
/* given a filemap, return a hash elem pointer to the first dataset for a given
 * rank */
scr_hash_elem *scr_filemap_first_dataset_by_rank(const scr_filemap *map,
                                                 int rank)
{
    scr_hash *r = scr_filemap_get_r(map, rank);
...
/* given a filemap, a dataset id, and a rank, return a hash elem pointer to the
 * first file */
scr_hash_elem *scr_filemap_first_file(const scr_filemap *map, int dset,
                                      int rank)
{
    scr_hash *fh = scr_filemap_get_fh(map, dset, rank);
...
/* free memory resources associated with filemap */
...
/* adds all files from map2 to map1 and updates num_expected_files to total file
 * count */
...
scr_filemap *scr_filemap_extract_rank(scr_filemap *map, int rank)
{
    ...
    if (r != NULL) {
        ...
        scr_hash *new_r =
            scr_hash_set_kv_int(new_map, SCR_FILEMAP_KEY_RANK, rank);

        /* copy the rank hash for the given rank to the new map */
        scr_hash_merge(new_r, r);

        /* for each dataset we have for this rank, set the DSET/RANK index the
         * new map */
        scr_hash_elem *dset_elem;
        for (dset_elem = scr_filemap_first_dataset_by_rank(map, rank);
             dset_elem != NULL; dset_elem = scr_hash_elem_next(dset_elem)) {
            int dset = scr_hash_elem_key_int(dset_elem);
...
int scr_filemap_read(const scr_path *path_file, scr_filemap *map)
{
    ...
    /* can't read file, return error (special case so as not to print error
     * message below) */
    ...
    if (scr_hash_read(file, map) != SCR_SUCCESS) {
        scr_err("Reading filemap %s @ %s:%d", file, __FILE__, __LINE__);
        goto cleanup;
    }
...
int scr_filemap_write(const scr_path *file, const scr_filemap *map)
{
    ...
    if (scr_hash_write_path(file, map) != SCR_SUCCESS) {
        ...
        scr_err("Writing filemap %s @ %s:%d", path_err, __FILE__, __LINE__);
        return SCR_FAILURE;
    }
