...
/* given a hash and a key, return the hash associated with key, returns NULL if
 * not found */
...
/* given a hash and a key, extract and return hash for specified key, returns
 * NULL if not found */
...
/* traverse the given hash using a printf-like format string setting an
 * arbitrary list of keys to set (or reset) the hash associated with the last
 * key */
...
/* shortcut to get hash assocated with the subkey of a key in a hash with one
 * call */
...
/* unset subkey under key, and if that removes the only element for key, unset
 * key as well */
...
/* same as scr_hash_elem_key, but converts the key as an int (returns 0 if key
 * is not defined) */
...
/* given a hash and a key, return a pointer to the key of the first element of
 * that key's hash */
char *scr_hash_elem_get_first_val(const scr_hash *hash, const char *key);

/* given a hash and a key, find first matching element, remove it from the hash,
 * and return it */
...
/* packs the given hash into specified buf and returns the number of bytes
 * written */
size_t scr_hash_pack(char *buf, const scr_hash *hash);

/* unpacks hash from specified buffer into given hash object and returns the
...
/* opens specified file and reads in a hash storing its contents in the given
 * hash object */
...
/* given a filename and hash, lock/open/read/close/unlock the file storing its
 * contents in the hash */
int scr_hash_read_with_lock(const char *file, scr_hash *hash);

/* given a filename and hash, lock the file, open it, and read it into hash, set
 * fd to the opened file descriptor */
int scr_hash_lock_open_read(const char *file, int *fd, scr_hash *hash);

/* given a filename, an opened file descriptor, and a hash, overwrite file with
 * hash, close, and unlock file */
int scr_hash_write_close_unlock(const char *file, int *fd,
                                const scr_hash *hash);
