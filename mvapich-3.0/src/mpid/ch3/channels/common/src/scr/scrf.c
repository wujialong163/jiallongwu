...
/* convert a Fortran string to a C string, removing any trailing spaces and
...
static int scr_cstr2fstr(const char *cstr, char *fstr, int flen)
{
    ...
    if (clen <= flen) {
        ...
        /* C string is longer than our Fortran buffer, copy what we can then
         * truncate */
...
FORTRAN_API void FORT_CALL scr_route_file_(char *name FORT_MIXED_LEN(name_len),
                                           ...
                                           int *ierror FORT_END_LEN(name_len)
                                               FORT_END_LEN(file_len))
{
