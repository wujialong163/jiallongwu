...
int scr_reddesc_init(scr_reddesc *d)
{
    ...
    if (d == NULL) {
        scr_err("No redundancy descriptor to fill from hash @ %s:%d", __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
...
static int scr_reddesc_create_partner(scr_reddesc *d)
{
    ...
    scr_reddesc_partner *state =
        (scr_reddesc_partner *)malloc(sizeof(scr_reddesc_partner));
    ...
    scr_set_partners(d->comm, 1, &state->lhs_rank, &state->lhs_rank_world,
                     &state->lhs_hostname, &state->rhs_rank,
                     &state->rhs_rank_world, &state->rhs_hostname);

    /* check that we got valid partners */
    if (state->lhs_hostname == NULL || state->rhs_hostname == NULL ||
        ...
        d->enabled = 0;
        scr_warn("Failed to find partner processes for redundancy descriptor "
                 "%d, disabling checkpoint, too few nodes? @ %s:%d",
                 d->index, __FILE__, __LINE__);
        rc = SCR_FAILURE;
    } else {
        scr_dbg(2,
                "LHS partner: %s (%d)  -->  My name: %s (%d)  -->  RHS "
                "partner: %s (%d)",
                state->lhs_hostname, state->lhs_rank_world, scr_my_hostname,
                scr_my_rank_world, state->rhs_hostname, state->rhs_rank_world);
    }
...
static int scr_reddesc_create_xor(scr_reddesc *d)
{
    ...
    if (ranks_comm > 0) {
        ...
        /* gather rank values */
        MPI_Allgather(&scr_my_rank_world, 1, MPI_INT, ranklist, 1, MPI_INT,
                      d->comm);
        ...
        for (i = 0; i < ranks_comm; i++) {
            int rank = ranklist[i];
            scr_hash_setf(hash, NULL, "%s %d %d", SCR_KEY_COPY_XOR_GROUP_RANK,
                          i, rank);
        }
...
scr_reddesc *scr_reddesc_for_checkpoint(int id, int ndescs, scr_reddesc *descs)
{
    ...
    int interval = 0;
    for (i = 0; i < ndescs; i++) {
        if (descs[i].enabled && interval < descs[i].interval &&
...
static int scr_reddesc_group_id(int rank, int ranks, int minsize, int *group_id)
{
...
static int scr_reddesc_split_across(MPI_Comm comm_parent, MPI_Comm comm_group,
...
static int scr_reddesc_type_int_from_str(const char *value, int *type)
{
    ...
    if (strcasecmp(value, "SINGLE") == 0) {
        ...
        if (scr_my_rank_world == 0) {
            scr_warn("Unknown copy type %s @ %s:%d", value, __FILE__, __LINE__);
        }
        rc = SCR_FAILURE;
    }
...
int scr_reddesc_create_from_hash(scr_reddesc *d, int index,
                                 const scr_hash *hash)
{
    int rc = SCR_SUCCESS;
    ...
    char *base;
    if (scr_hash_util_get_str(hash, SCR_CONFIG_KEY_BASE, &base) ==
        SCR_SUCCESS) {
        d->base = strdup(base);
    ...
    char *type;
    if (scr_hash_util_get_str(hash, SCR_CONFIG_KEY_TYPE, &type) ==
        SCR_SUCCESS) {
        if (scr_reddesc_type_int_from_str(type, &d->copy_type) != SCR_SUCCESS) {
            /* don't recognize copy type, disable this descriptor */
            d->enabled = 0;
            if (scr_my_rank_world == 0) {
                scr_warn("Unknown copy type %s in redundancy descriptor %d, "
                         "disabling checkpoint @ %s:%d",
        ...
        if (groupdesc != NULL && groupdesc->ranks == scr_ranks_world) {
            if (scr_my_rank_world == 0) {
                if (d->copy_type != SCR_COPY_SINGLE) {
                    ...
                    scr_warn("Forcing copy type to SINGLE in redundancy "
                             "descriptor %d @ %s:%d",
                             d->index, __FILE__, __LINE__);
                }
            }
        ...
        switch (d->copy_type) {
            ...
            case SCR_COPY_PARTNER:
                ...
                groupdesc = scr_groupdescs_from_name(SCR_GROUP_NODE);
                if (groupdesc != NULL) {
                    scr_reddesc_split_across(scr_comm_world, groupdesc->comm,
                                             &d->comm);
                } else {
                    /* TODO: we could fall back to SINGLE here instead */
                    scr_abort(-1,
                              "Failed to get communicator across failure "
                              "groups @ %s:%d",
                              __FILE__, __LINE__);
                }
                break;
            case SCR_COPY_XOR:
                ...
                if (groupdesc != NULL) {
                    ...
                    scr_reddesc_group_id(rank_across, ranks_across, set_size,
                                         &split_id);

                    /* split communicator into groups */
                    MPI_Comm_split(comm_across, split_id, scr_my_rank_world,
                                   &d->comm);
        ...
        MPI_Allreduce(&group_master, &d->groups, 1, MPI_INT, MPI_SUM,
                      scr_comm_world);
...
int scr_reddesc_restore_from_hash(scr_reddesc *d, const scr_hash *hash)
{
    int rc = SCR_SUCCESS;
    ...
    if (value != NULL) {
        ...
        scr_abort(-1, "Missing directory in descriptor hash @ %s:%d", __FILE__,
                  __LINE__);
    }
    ...
    if (value != NULL) {
        if (scr_reddesc_type_int_from_str(value, &d->copy_type) !=
            SCR_SUCCESS) {
            d->enabled = 0;
            if (scr_my_rank_world == 0) {
                scr_warn("Unknown copy type %s in redundancy descriptor hash, "
                         "disabling checkpoint @ %s:%d",
    ...
    char *group_id_str =
        scr_hash_elem_get_first_val(hash, SCR_CONFIG_KEY_GROUP_ID);
    char *group_rank_str =
        scr_hash_elem_get_first_val(hash, SCR_CONFIG_KEY_GROUP_RANK);
...
char *scr_reddesc_val_from_filemap(scr_filemap *map, int ckpt, int rank,
                                   char *name)
{
...
{
    char *value =
        scr_reddesc_val_from_filemap(map, ckpt, rank, SCR_CONFIG_KEY_BASE);
    return value;
}
...
{
    char *value =
        scr_reddesc_val_from_filemap(map, ckpt, rank, SCR_CONFIG_KEY_DIRECTORY);
    return value;
}

/* build a redundancy descriptor from its corresponding hash
 * stored in the filemap, this function is collective */
int scr_reddesc_create_from_filemap(scr_filemap *map, int id, int rank,
                                    scr_reddesc *d)
{
...
int scr_reddescs_create()
{
    ...
    if (scr_nreddescs > 0) {
        scr_reddescs =
            (scr_reddesc *)malloc(scr_nreddescs * sizeof(scr_reddesc));
        /* TODO: check for errors */
    }
    ...
    if (scr_my_rank_world == 0) {
         ...
         * descriptors */
        scr_hash_elem *elem;
        for (elem = scr_hash_elem_first(descs); elem != NULL;
            ...
            /* create descriptor */
            if (scr_reddesc_create_from_hash(&scr_reddescs[index], index,
                                             hash) != SCR_SUCCESS) {
