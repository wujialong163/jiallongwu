...
int scr_str_recv(char **str, int rank, MPI_Comm comm)
{
    ...
    if (len > 0) {
        ...
        if (recvstr == NULL) {
            scr_abort(-1,
                      "Failed to allocate %d bytes to receive string @ %s:%d",
    ...
    if (str != NULL) {
        *str = recvstr;
        ...
        scr_abort(-1,
                  "Given invalid pointer to record address of allocated string "
                  "@ %s:%d",
                  __FILE__, __LINE__);
    }
...
int scr_str_sendrecv(const char *send_str, int send_rank, char **recv_str,
                     int recv_rank, MPI_Comm comm)
{
    MPI_Status status;
    ...
    int recv_len = 0;
    MPI_Sendrecv(&send_len, 1, MPI_INT, send_rank, 999, &recv_len, 1, MPI_INT,
                 recv_rank, 999, comm, &status);
    ...
    /* exchange strings */
    MPI_Sendrecv((void *)send_str, send_len, MPI_CHAR, send_rank, 999,
                 (void *)tmp_str, recv_len, MPI_CHAR, recv_rank, 999, comm,
                 &status);
...
int scr_str_bcast(char **str, int root, MPI_Comm comm)
{
    ...
    if (rank == root) {
        ...
        if (len > 0) {
            ...
            if (tmp_str == NULL) {
                scr_abort(-1,
                          "Failed to allocate %d bytes to bcast string @ %s:%d",
                          len, __FILE__, __LINE__);
            }
        }
    }
...
int scr_strn_bcast(char *str, int n, int root, MPI_Comm comm)
{
    ...
    if (str == NULL) {
        scr_abort(-1, "Can't bcast a NULL string @ %s:%d", __FILE__, __LINE__);
    }
    ...
    if (len > n || n < 0) {
        scr_abort(
            -1,
            "String buffer of %d bytes too short for %d byte string @ %s:%d", n,
            len, __FILE__, __LINE__);
    }
 ...
 * ranks and hostnames */
int scr_set_partners(MPI_Comm comm, int dist, int *lhs_rank,
                     int *lhs_rank_world, char **lhs_hostname, int *rhs_rank,
                     int *rhs_rank_world, char **rhs_hostname)
{
