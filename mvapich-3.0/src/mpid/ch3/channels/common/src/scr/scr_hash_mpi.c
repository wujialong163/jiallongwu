...
int scr_hash_send(const scr_hash *hash, int rank, MPI_Comm comm)
{
    ...
    if (pack_size > INT_MAX) {
        scr_abort(
            -1,
            "scr_hash_send: hash size %lu is bigger than INT_MAX %d @ %s:%d",
    ...
    if (size > 0) {
        ...
        if (buf != NULL) {
            ...
            scr_abort(
                -1,
                "scr_hash_send: Failed to malloc buffer to pack hash @ %s:%d",
                __FILE__, __LINE__);
        }
    }
...
int scr_hash_recv(scr_hash *hash, int rank, MPI_Comm comm)
{
    ...
    if (size > 0) {
        ...
        if (buf != NULL) {
            ...
            scr_abort(-1,
                      "scr_hash_recv: Failed to malloc buffer to receive hash "
                      "@ %s:%d",
                      __FILE__, __LINE__);
        }
    }
                      ...
                      scr_hash *hash_recv, int rank_recv, MPI_Comm comm)
{
    ...
    int size_recv = 0;
    if (have_incoming) {
        MPI_Irecv(&size_recv, 1, MPI_INT, rank_recv, 0, comm,
                  &request[num_req]);
        num_req++;
    }
    if (have_outgoing) {
        ...
        if (pack_size > max_int) {
            scr_abort(-1,
                      "scr_hash_sendrecv: hash size %lu is bigger than INT_MAX "
                      "%d @ %s:%d",
                      (unsigned long)pack_size, INT_MAX, __FILE__, __LINE__);
        }
        ...
        MPI_Isend(&size_send, 1, MPI_INT, rank_send, 0, comm,
                  &request[num_req]);
        num_req++;
    }
    if (num_req > 0) {
    ...
    if (size_recv > 0) {
        ...
        MPI_Irecv(buf_recv, size_recv, MPI_BYTE, rank_recv, 0, comm,
                  &request[num_req]);
        num_req++;
    }
    if (size_send > 0) {
        ...
        MPI_Isend(buf_send, size_send, MPI_BYTE, rank_send, 0, comm,
                  &request[num_req]);
        num_req++;
    }
    if (num_req > 0) {
...
int scr_hash_bcast(scr_hash *hash, int root, MPI_Comm comm)
{
    ...
    if (rank == root) {
        ...
        if (pack_size > max_int) {
            scr_abort(-1,
                      "scr_hash_bcast: hash size %lu is bigger than INT_MAX %d "
                      "@ %s:%d",
                      (unsigned long)pack_size, INT_MAX, __FILE__, __LINE__);
        }
        ...
        if (size > 0) {
            ...
            if (buf != NULL) {
                ...
                scr_abort(-1,
                          "scr_hash_bcast: Failed to malloc buffer to pack "
                          "hash @ %s:%d",
                          __FILE__, __LINE__);
            }
        }
        ...
        if (size > 0) {
            ...
            if (buf != NULL) {
                ...
                scr_abort(-1,
                          "scr_hash_bcast: Failed to malloc buffer to receive "
                          "hash @ %s:%d",
                          __FILE__, __LINE__);
            }
        }
...
static int scr_hash_exchange_direction_hops(const scr_hash *hash_in,
                                            scr_hash *hash_out, MPI_Comm comm,
                                            ...
                                            int hops)
{
    ...
    for (elem = scr_hash_elem_first(hash_in); elem != NULL;
    ...
    while (step < ranks && (hops < 0 || hop_count < hops)) {
        ...
        for (elem = scr_hash_elem_first(dest_hash); elem != NULL;
             elem = scr_hash_elem_next(elem)) {
...
int scr_hash_exchange(const scr_hash *send, scr_hash *recv, MPI_Comm comm)
{
    ...
    scr_hash_elem *elem;
    for (elem = scr_hash_elem_first(send); elem != NULL;
         elem = scr_hash_elem_next(elem)) {
...
int scr_hash_exchange_direction(const scr_hash *send, scr_hash *recv,
                                MPI_Comm comm, scr_hash_exchange_enum dir)
{
