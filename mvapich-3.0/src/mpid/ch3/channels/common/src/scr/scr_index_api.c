...
/* read the index file from given directory and merge its contents into the
 * given hash */
...
int scr_index_write(const scr_path *dir, scr_hash *index)
{
    ...
    if (version == NULL) {
        scr_hash_util_set_int(index, SCR_INDEX_KEY_VERSION,
                              SCR_INDEX_FILE_VERSION_1);
    }
...
int scr_index_add_dir(scr_hash *index, int id, const char *name)
{
    /* set the directory */
    scr_hash *dset_hash =
        scr_hash_set_kv_int(index, SCR_INDEX_1_KEY_DATASET, id);
...
int scr_index_remove_dir(scr_hash *index, const char *name)
{
    ...
    if (scr_index_get_id_by_dir(index, name, &id) == SCR_SUCCESS) {
        ...
        scr_hash *dset =
            scr_hash_get_kv_int(index, SCR_INDEX_1_KEY_DATASET, id);
        ...
        scr_err("Named directory was not found in index file: %s @ %s:%d", name,
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
}
...
/* write completeness code (0 or 1) for given dataset id and directory in given
 * hash */
int scr_index_set_complete(scr_hash *index, int id, const char *name,
                           int complete)
{
...
/* write completeness code (0 or 1) for given dataset id and directory in given
 * hash */
int scr_index_set_dataset(scr_hash *index, int id, const char *name,
                          const scr_dataset *dataset, int complete)
{
...
int scr_index_get_complete(scr_hash *index, int id, const char *name,
                           int *complete)
{
    int rc = SCR_FAILURE;
    *complete = 0;
    ...
    int complete_tmp;
    if (scr_hash_util_get_int(dir_hash, SCR_INDEX_1_KEY_COMPLETE,
                              &complete_tmp) == SCR_SUCCESS) {
...
/* lookup the dataset id corresponding to the given dataset directory name in
 * given hash (assumes a directory maps to a single dataset id) */
int scr_index_get_id_by_dir(const scr_hash *index, const char *name, int *id)
{
    ...
    *id = -1;
    ...
    int id_tmp;
    if (scr_hash_util_get_int(dir_hash, SCR_INDEX_1_KEY_DATASET, &id_tmp) ==
        SCR_SUCCESS) {
...
/* lookup the most recent complete dataset id and directory whose id is less
 * than earlier_than setting earlier_than = -1 disables this filter */
int scr_index_get_most_recent_complete(const scr_hash *index, int earlier_than,
                                       int *id, char *name)
{
    /* assume that we won't find this dataset */
    *id = -1;

    /* search for the maximum dataset id which is complete and less than
     * earlier_than if earlier_than is set */
    int max_id = -1;
    scr_hash *dsets = scr_hash_get(index, SCR_INDEX_1_KEY_DATASET);
    scr_hash_elem *dset = NULL;
    for (dset = scr_hash_elem_first(dsets); dset != NULL;
        ...
        if (key != NULL) {
            ...
            if ((earlier_than == -1 || current_id <= earlier_than) &&
                current_id > max_id) {
                /* alright, this dataset id is within range to be the most
                 * recent, now scan the various dirs we have for this dataset
                ...
                for (dir = scr_hash_elem_first(dirs); dir != NULL;
                    ...
                    int complete;
                    if (scr_hash_util_get_int(dir_hash,
                                              SCR_INDEX_1_KEY_COMPLETE,
                    ...
                    scr_hash *failed =
                        scr_hash_get(dir_hash, SCR_INDEX_1_KEY_FAILED);
                    if (failed != NULL) {
                        found_one = 0;
                    }

                    /* TODO: also avoid dataset if we've tried to read it too
                     * many times */

                    /* if we found one, copy the dataset id and directory name,
                     * and update our max */
                    if (found_one) {
                        *id = current_id;
                        strcpy(name, dir_key);
