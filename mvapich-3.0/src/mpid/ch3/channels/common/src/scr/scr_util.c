...
int scr_atod(char *str, double *val)
{
    ...
    if (val == NULL) {
        scr_err("scr_atod: NULL address to store value @ %s:%d", __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (errno == 0) {
        ...
        *val = value;
        ...
        scr_err("scr_atod: Invalid double: %s @ %s:%d", str, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
...
int scr_abtoull(char *str, unsigned long long *val)
{
    ...
    /* check that we have a value to write to */
    if (val == NULL) {
        scr_err("scr_abtoull: NULL address to store value @ %s:%d", __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (errno != 0) {
        scr_err("scr_abtoull: Invalid double: %s @ %s:%d", str, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (*next != '\0') {
        switch (*next) {
            ...
            case 'G':
                ...
                break;
            default:
                scr_err("scr_abtoull: Unexpected byte string %s @ %s:%d", str,
                        __FILE__, __LINE__);
                return SCR_FAILURE;
        }
    ...
    if (num < 0) {
        scr_err("scr_abtoull: Byte string must be positive: %s @ %s:%d", str,
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
...
/* caller really passes in a void**, but we define it as just void* to avoid
 * printing a bunch of warnings */
...
int scr_pack_uint16_t(void *buf, size_t buf_size, size_t *buf_pos, uint16_t val)
{
    ...
    if (buf == NULL || buf_pos == NULL) {
        scr_err("NULL pointer to buffer or buffer position @ %s:%d", __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
...
int scr_unpack_uint16_t(const void *buf, size_t buf_size, size_t *buf_pos,
                        uint16_t *val)
{
    /* check that we have a valid pointer to a buffer position value */
...
int scr_unpack_uint32_t(const void *buf, size_t buf_size, size_t *buf_pos,
                        uint32_t *val)
{
    /* check that we have a valid pointer to a buffer position value */
    if (buf == NULL || buf_pos == NULL || val == NULL) {
...
int scr_unpack_uint64_t(const void *buf, size_t buf_size, size_t *buf_pos,
                        uint64_t *val)
{
    /* check that we have a valid pointer to a buffer position value */
    if (buf == NULL || buf_pos == NULL || val == NULL) {
