...
scr_dataset *scr_dataset_new()
{
    ...
    if (dataset == NULL) {
        scr_err("Failed to allocate dataset object @ %s:%d", __FILE__,
                __LINE__);
    }
    return dataset;
}
...
/* gets the jobid in which the dataset was created, returns SCR_SUCCESS if
 * successful */
...
/* gets the cluster name on which the dataset was created, returns SCR_SUCCESS
 * if successful */
