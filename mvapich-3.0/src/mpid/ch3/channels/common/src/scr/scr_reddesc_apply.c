                        ...
                        MPI_Comm comm)
{
    ...
    int have_outgoing = 0;
    if (rank_send != MPI_PROC_NULL && file_send != NULL &&
        strcmp(file_send, "") != 0) {
        have_outgoing = 1;
    } else {
    ...
    int have_incoming = 0;
    if (rank_recv != MPI_PROC_NULL && dir_recv != NULL &&
        strcmp(dir_recv, "") != 0) {
        have_incoming = 1;
    } else {
...
static int scr_swap_files_copy(int have_outgoing, const char *file_send,
                               scr_meta *meta_send, int rank_send,
                               uLong *crc32_send, int have_incoming,
                               const char *file_recv, scr_meta *meta_recv,
                               int rank_recv, uLong *crc32_recv, MPI_Comm comm)
{
    int rc = SCR_SUCCESS;
    ...
    if (have_outgoing) {
        ...
        if (buf_send == NULL) {
            scr_abort(-1, "Allocating memory: malloc(%ld) errno=%d %s @ %s:%d",
                      scr_mpi_buf_size, errno, strerror(errno), __FILE__,
                      __LINE__);
            return SCR_FAILURE;
        }
    }
    ...
    if (have_outgoing) {
        ...
        if (fd_send < 0) {
            scr_abort(-1,
                      "Opening file for send: scr_open(%s, O_RDONLY) errno=%d "
                      "%s @ %s:%d",
    ...
    if (have_incoming) {
        ...
        if (fd_recv < 0) {
            scr_abort(-1,
                      "Opening file for recv: scr_open(%s, O_WRONLY | O_CREAT "
                      "| O_TRUNC, ...) errno=%d %s @ %s:%d",
    ...
    while (sending || receiving) {
        ...
        if (receiving) {
            MPI_Irecv(buf_recv, scr_mpi_buf_size, MPI_BYTE, rank_recv, 0, comm,
                      &request[0]);
        }
        ...
        if (sending) {
            ...
            if (scr_crc_on_copy && nread > 0) {
                *crc32_send =
                    crc32(*crc32_send, (const Bytef *)buf_send, (uInt)nread);
            }
            if (nread < 0) {
                nread = 0;
            }
            MPI_Isend(buf_send, nread, MPI_BYTE, rank_send, 0, comm,
                      &request[1]);
        ...
        if (receiving) {
            ...
            if (scr_crc_on_copy && nwrite > 0) {
                *crc32_recv =
                    crc32(*crc32_recv, (const Bytef *)buf_recv, (uInt)nwrite);
            }
    ...
    if (scr_crc_on_copy && have_outgoing) {
        ...
        if (scr_meta_get_crc32(meta_send, &meta_send_crc) != SCR_SUCCESS) {
            ...
            /* TODO: we could check that the crc on the sent file matches and
             * take some action if not */
        }
    }
...
static int scr_swap_files_move(int have_outgoing, const char *file_send,
                               scr_meta *meta_send, int rank_send,
                               uLong *crc32_send, int have_incoming,
                               const char *file_recv, scr_meta *meta_recv,
                               int rank_recv, uLong *crc32_recv, MPI_Comm comm)
{
    ...
    int fd = -1;
    if (have_outgoing) {
        /* we'll overwrite our send file (or just read it if there is no
         * incoming) */
        ...
        if (fd < 0) {
            ...
            scr_abort(-1,
                      "Opening file for send/recv: scr_open(%s, O_RDWR) "
                      "errno=%d %s @ %s:%d",
                      file_send, errno, strerror(errno), __FILE__, __LINE__);
        }
    ...
    while (sending || receiving) {
        ...
        if (receiving) {
            ...
            /* write those bytes to file (if nwrite is smaller than
             * scr_mpi_buf_size, then we've received the whole file) */
    ...
    if (have_outgoing && have_incoming) {
        /* sent and received a file; close it, truncate it to correct size,
         * rename it */
        scr_close(file_send, fd);
        truncate(file_send, write_pos);
        rename(file_send, file_recv);
    } else if (have_outgoing) {
        /* only sent a file; close it, delete it, and remove its completion
         * marker */
        scr_close(file_send, fd);
                   ...
                   int rank_recv, MPI_Comm comm)
{
    ...
    /* determine whether we are expecting to receive a file */
    int have_incoming = 0;
    if (rank_recv != MPI_PROC_NULL && file_recv != NULL &&
        strcmp(file_recv, "") != 0) {
        have_incoming = 1;
    }
    ...
    /* exchange files */
    if (swap_type == COPY_FILES) {
        scr_swap_files_copy(have_outgoing, file_send, meta_send, rank_send,
                            &crc32_send, have_incoming, file_recv, meta_recv,
                            rank_recv, &crc32_recv, comm);
    } else if (swap_type == MOVE_FILES) {
        scr_swap_files_move(have_outgoing, file_send, meta_send, rank_send,
                            &crc32_send, have_incoming, file_recv, meta_recv,
                            rank_recv, &crc32_recv, comm);
    } else {
        scr_err("Unknown file transfer type: %d @ %s:%d", swap_type, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (have_incoming) {
        ...
        if (scr_crc_on_copy) {
            /* if we computed crc during the copy, and crc is set in the
             * received meta data check that our computed value matches */
            uLong crc32_recv_meta;
            if (scr_meta_get_crc32(meta_recv, &crc32_recv_meta) ==
                SCR_SUCCESS) {
                if (crc32_recv != crc32_recv_meta) {
                    scr_err("CRC32 mismatch detected when receiving file %s @ "
                            "%s:%d",
                            file_recv, __FILE__, __LINE__);
                    scr_meta_set_complete(meta_recv, 0);
                    rc = SCR_FAILURE;
...
static int scr_reddesc_apply_partner(scr_filemap *map, const scr_reddesc *c,
                                     int id)
{
    int rc = SCR_SUCCESS;
    ...
    /* first, determine how many files we'll be sending and receiving with our
     * partners */
    MPI_Status status;
    int send_num = numfiles;
    int recv_num = 0;
    MPI_Sendrecv(&send_num, 1, MPI_INT, state->rhs_rank, 0, &recv_num, 1,
                 MPI_INT, state->lhs_rank, 0, c->comm, &status);
    ...
    scr_hash_util_set_int(flushdesc, SCR_SCAVENGE_KEY_CONTAINER,
                          scr_use_containers);
    scr_hash_util_set_str(flushdesc, SCR_SCAVENGE_KEY_PARTNER,
                          state->lhs_hostname);
    ...
    scr_hash_sendrecv(my_desc_hash, state->rhs_rank, lhs_desc_hash,
                      state->lhs_rank, c->comm);
    ...
    while (send_num > 0 || recv_num > 0) {
        ...
        scr_swap_file_names(file, send_rank, file_partner, sizeof(file_partner),
                            recv_rank, dir, c->comm);

        /* if we'll receive a file, record the name of our partner's file in the
         * filemap */
        ...
        if (scr_swap_files(COPY_FILES, file, send_meta, send_rank, file_partner,
                           recv_meta, recv_rank, c->comm) != SCR_SUCCESS) {
            rc = SCR_FAILURE;
        }
        scr_filemap_set_meta(map, id, state->lhs_rank_world, file_partner,
                             recv_meta);
...
static int scr_reddesc_apply_xor(scr_filemap *map, const scr_reddesc *c, int id)
{
    ...
    if (send_buf == NULL) {
        scr_abort(
            -1,
            "Allocating memory for send buffer: malloc(%d) errno=%d %s @ %s:%d",
    ...
    if (recv_buf == NULL) {
        scr_abort(
            -1,
            "Allocating memory for recv buffer: malloc(%d) errno=%d %s @ %s:%d",
            scr_mpi_buf_size, errno, strerror(errno), __FILE__, __LINE__);
    }

    /* count the number of files I have and allocate space in structures for
     * each of them */
    ...
    if (num_files > 0) {
        ...
        if (fds == NULL || filenames == NULL || filesizes == NULL) {
            scr_abort(-1, "Failed to allocate file arrays @ %s:%d", __FILE__,
                      __LINE__);
        }
    }
    ...
    for (file_elem = scr_filemap_first_file(map, id, scr_my_rank_world);
        ...
        /* read the meta data for this file and insert it into the current_files
         * hash */
        scr_meta *file_hash = scr_meta_new();
        scr_filemap_get_meta(map, id, scr_my_rank_world, filenames[file_count],
                             file_hash);
        ...
        if (fds[file_count] < 0) {
            /* TODO: try again? */
            scr_abort(-1,
                      "Opening checkpoint file for copying: scr_open(%s, "
                      "O_RDONLY) errno=%d %s @ %s:%d",
                      filenames[file_count], errno, strerror(errno), __FILE__,
                      __LINE__);
        }
    ...
    scr_hash_sendrecv(current_hash, state->rhs_rank, partner_hash,
                      state->lhs_rank, c->comm);
    ...
    MPI_Allreduce(&my_bytes, &max_bytes, 1, MPI_UNSIGNED_LONG, MPI_MAX,
                  c->comm);

    /* TODO: use unsigned long integer arithmetic (with proper byte padding)
     * instead of char to speed things up */

    /* compute chunk size according to maximum file length and number of ranks
     * in xor set */
    /* if filesize doesn't divide evenly, then add one byte to chunk_size */
    /* TODO: check that ranks > 1 for this divide to be safe (or at partner
     * selection time) */
    ...
    /* set chunk filenames of form:
     * <xor_rank+1>_of_<xor_ranks>_in_<group_id>.xor */
    char my_chunk_file[SCR_MAX_FILENAME];
    char dir[SCR_MAX_FILENAME];
    scr_cache_dir_get(c, id, dir);
    sprintf(my_chunk_file, "%s/%d_of_%d_in_%d.xor", dir, c->my_rank + 1,
            c->ranks, c->group_id);
    ...
    mode_t mode_file = scr_getmode(1, 1, 0);
    int fd_chunk =
        scr_open(my_chunk_file, O_WRONLY | O_CREAT | O_TRUNC, mode_file);
    if (fd_chunk < 0) {
        /* TODO: try again? */
        scr_abort(-1,
                  "Opening XOR chunk file for writing: scr_open(%s) errno=%d "
                  "%s @ %s:%d",
    ...
    while (nread < chunk_size) {
        ...
        for (chunk_id = c->ranks - 1; chunk_id >= 0; chunk_id--) {
            /* read the next set of bytes for this chunk from my file into
             * send_buf */
            if (chunk_id > 0) {
                int chunk_id_rel =
                    (c->my_rank + c->ranks + chunk_id) % c->ranks;
                if (chunk_id_rel > c->my_rank) {
                    chunk_id_rel--;
                }
                unsigned long offset =
                    chunk_size * (unsigned long)chunk_id_rel + nread;
                if (scr_read_pad_n(num_files, filenames, fds, send_buf, count,
                                   offset, filesizes) != SCR_SUCCESS) {
                    rc = SCR_FAILURE;
                }
            } else {
            ...
            /* TODO: XORing with unsigned long would be faster here (if chunk
             * size is multiple of this size) */
            ...
            if (chunk_id > 0) {
                ...
                MPI_Irecv(recv_buf, count, MPI_BYTE, state->lhs_rank, 0,
                          c->comm, &request[0]);
                MPI_Isend(send_buf, count, MPI_BYTE, state->rhs_rank, 0,
                          c->comm, &request[1]);
                MPI_Waitall(2, request, status);
            } else {
                /* write send block to send chunk file */
                if (scr_write_attempt(my_chunk_file, fd_chunk, send_buf,
                                      count) != count) {
                    rc = SCR_FAILURE;
                }
            }
    ...
    /* free the buffers */
    scr_free(&filesizes);
    /* in this case, we don't free each name, since we copied the pointer to the
     * string in the filemap */
    scr_free(&filenames);
    scr_free(&fds);
    ...
    if (scr_crc_on_copy) {
        ...
        /* TODO: would be nice to save this CRC in our partner's XOR file so we
         * can check correctness on a rebuild */
    }
                      ...
                      double *bytes)
{
    ...
    *bytes = 0.0;
    ...
    /* step through each of my files for the specified dataset to scan for any
     * incomplete files */
    int valid = 1;
    double my_bytes = 0.0;
    scr_hash_elem *file_elem;
    for (file_elem = scr_filemap_first_file(map, id, scr_my_rank_world);
        ...
        /* check the file */
        if (!scr_bool_have_file(map, id, scr_my_rank_world, file,
                                scr_ranks_world)) {
        ...
        /* if crc_on_copy is set, compute crc and update meta file (PARTNER does
    ...
    if (!all_valid) {
        if (scr_my_rank_world == 0) {
            scr_dbg(
                1,
                "Exiting copy since one or more checkpoint files is invalid");
        }
        return SCR_FAILURE;
    }
    ...
    if (!valid_copy) {
        scr_err("scr_copy_files failed with return code %d @ %s:%d", rc,
                __FILE__, __LINE__);
    }
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_dbg(1,
                "scr_copy_files: %f secs, %e bytes, %f MB/s, %f MB/s per proc",
        ...
        if (scr_log_enable) {
            char dir[SCR_MAX_FILENAME];
            scr_cache_dir_get(c, id, dir);
            scr_log_transfer("COPY", c->base, dir, &id, &timestamp_start,
                             &time_diff, bytes);
        }
    }
