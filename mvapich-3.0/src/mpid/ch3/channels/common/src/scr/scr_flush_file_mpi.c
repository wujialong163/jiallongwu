...
int scr_bool_need_flush(int id)
{
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_hash *dset_hash =
            scr_hash_get_kv_int(hash, SCR_FLUSH_KEY_DATASET, id);
        scr_hash *in_cache = scr_hash_get_kv(dset_hash, SCR_FLUSH_KEY_LOCATION,
                                             SCR_FLUSH_KEY_LOCATION_CACHE);
        scr_hash *in_pfs = scr_hash_get_kv(dset_hash, SCR_FLUSH_KEY_LOCATION,
                                           SCR_FLUSH_KEY_LOCATION_PFS);
...
int scr_bool_is_flushing(int id)
{
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_hash *dset_hash =
            scr_hash_get_kv_int(hash, SCR_FLUSH_KEY_DATASET, id);
        scr_hash *flushing_hash = scr_hash_get_kv(
            dset_hash, SCR_FLUSH_KEY_LOCATION, SCR_FLUSH_KEY_LOCATION_FLUSHING);
...
int scr_flush_file_location_set(int id, const char *location)
{
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_hash *dset_hash =
            scr_hash_set_kv_int(hash, SCR_FLUSH_KEY_DATASET, id);
...
int scr_flush_file_location_test(int id, const char *location)
{
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_hash *dset_hash =
            scr_hash_get_kv_int(hash, SCR_FLUSH_KEY_DATASET, id);
        scr_hash *value =
            scr_hash_get_kv(dset_hash, SCR_FLUSH_KEY_LOCATION, location);
        if (value != NULL) {
            at_location = 1;
        }
