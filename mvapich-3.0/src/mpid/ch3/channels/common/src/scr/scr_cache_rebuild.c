...
int scr_scatter_filemaps(scr_filemap *my_map)
{
    ...
    if (scr_storedesc_cntl->rank == 0) {
        ...
        if (ranks == NULL) {
            scr_abort(
                -1,
                "Failed to allocate memory to record local rank list @ %s:%d",
                __FILE__, __LINE__);
        }
        MPI_Gather(&scr_my_rank_world, 1, MPI_INT, ranks, 1, MPI_INT, 0,
                   scr_storedesc_cntl->comm);
        ...
        for (i = 0; i < scr_storedesc_cntl->ranks; i++) {
            ...
            if (scr_filemap_have_rank(all_map, rank)) {
                ...
                /* get a reference to the hash object that we'll send to this
                 * rank, and merge this filemap into it */
        ...
        while (j < num) {
            ...
            /* extract the filemap for this rank */
            scr_filemap *tmp_map =
                scr_filemap_extract_rank(all_map, remaining_ranks[j]);
        ...
        MPI_Gather(&scr_my_rank_world, 1, MPI_INT, NULL, 1, MPI_INT, 0,
                   scr_storedesc_cntl->comm);
    }
...
static int scr_distribute_datasets(scr_filemap *map, int id)
{
    ...
    for (i = 0; i < nranks; i++) {
        ...
        if (rank < 0 || rank >= scr_ranks_world) {
            scr_err("Invalid rank id %d in world of %d @ %s:%d", rank,
                    scr_ranks_world, __FILE__, __LINE__);
...
static int scr_distribute_reddescs(scr_filemap *map, int id, scr_reddesc *red)
{
    ...
    if (!scr_alltrue(num_desc > 0)) {
        ...
        scr_hash_delete(&send_hash);
        scr_dbg(2,
                "Cannot find process that has my redundancy descriptor @ %s:%d",
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
 ...
 * to new rank mapping */
static int scr_distribute_files(scr_filemap *map, const scr_reddesc *red,
                                int id)
{
    int i, round;
    int rc = SCR_SUCCESS;
    ...
    if (have_rank_by_round == NULL || send_flag_by_round == NULL) {
        scr_abort(
            -1, "Failed to allocate memory to record rank id by round @ %s:%d",
            __FILE__, __LINE__);
    }
    ...
    for (elem = scr_hash_elem_first(recv_hash); elem != NULL;
    ...
    if (red->copy_type != SCR_COPY_XOR && !scr_alltrue(can_get_files)) {
        ...
        if (!can_get_files) {
            scr_dbg(2,
                    "Cannot find process that has my checkpoint files @ %s:%d",
                    __FILE__, __LINE__);
        }
        return SCR_FAILURE;
    }
    ...
    int max_rounds = 0;
    MPI_Allreduce(&retrieve_round, &max_rounds, 1, MPI_INT, MPI_MAX,
                  scr_comm_world);
    ...
    for (round = 0; round <= max_rounds; round++) {
        ...
        if (send_rank == scr_my_rank_world) {
            ...
            for (i = 0; i < numfiles; i++) {
                ...
                /* if the new file name is different from the old name, rename
                 * it */
                if (strcmp(file, newfile) != 0) {
                    ...
                    /* rename the file */
                    scr_dbg(2, "Round %d: rename(%s, %s)", round, file,
                            newfile);
                    tmp_rc = rename(file, newfile);
                    if (tmp_rc != 0) {
                        /* TODO: to cross mount points, if tmp_rc == EXDEV,
                         * open new file, copy, and delete orig */
                        scr_err("Moving checkpoint file: rename(%s, %s) %s "
                                "errno=%d @ %s:%d",
                                file, newfile, strerror(errno), errno, __FILE__,
                                __LINE__);
                        rc = SCR_FAILURE;
                    }
            ...
            if (send_rank != MPI_PROC_NULL || recv_rank != MPI_PROC_NULL) {
                ...
                int num_req = 0;
                if (have_incoming) {
                    MPI_Irecv(&recv_num, 1, MPI_INT, recv_rank, 0,
                              scr_comm_world, &request[num_req]);
                    num_req++;
                }
                if (have_outgoing) {
                    MPI_Isend(&send_num, 1, MPI_INT, send_rank, 0,
                              scr_comm_world, &request[num_req]);
                    num_req++;
                }
                if (num_req > 0) {
                ...
                char **files = NULL;
                if (have_outgoing) {
                    scr_filemap_list_files(map, id, send_rank, &numfiles,
                                           &files);
                }
                ...
                while (have_incoming || have_outgoing) {
                    ...
                    if (have_outgoing) {
                        ...
                        scr_filemap_get_meta(map, id, send_rank, file,
                                             send_meta);
                    }

                    /* exhange file names with partners */
                    char file_partner[SCR_MAX_FILENAME];
                    scr_swap_file_names(file, send_rank, file_partner,
                                        sizeof(file_partner), recv_rank, dir,
                                        scr_comm_world);
                    ...
                    if (recv_rank != MPI_PROC_NULL) {
                        ...
                        scr_filemap_add_file(map, id, scr_my_rank_world,
                                             file_partner);
                        scr_filemap_write(scr_map_file, map);
                    }

                    /* either sending or receiving a file this round, since we
                     * move files, it will be deleted or overwritten */
                    if (scr_swap_files(MOVE_FILES, file, send_meta, send_rank,
                                       file_partner, recv_meta, recv_rank,
                                       scr_comm_world) != SCR_SUCCESS) {
                        scr_err("Swapping files: %s to %d, %s from %d @ %s:%d",
                                file, send_rank, file_partner, recv_rank,
                                __FILE__, __LINE__);
                        rc = SCR_FAILURE;
                    }
                    ...
                    if (have_incoming) {
                        ...
                        scr_filemap_set_meta(map, id, scr_my_rank_world,
                                             file_partner, recv_meta);
                    ...
                    /* if we sent a file, remove it from the filemap and
...
int scr_cache_rebuild(scr_filemap *map)
{
    ...
    do {
        ...
        if (current_id != -1) {
            /* remember that we made an attempt to distribute at least one
             * dataset */
            distribute_attempted = 1;

            /* log the attempt */
            if (scr_my_rank_world == 0) {
                scr_dbg(1, "Attempting to distribute and rebuild dataset %d",
                        current_id);
                if (scr_log_enable) {
                    time_t now = scr_log_seconds();
                    scr_log_event("REBUILD STARTED", NULL, &current_id, &now,
                                  NULL);
                }
            }
            ...
            if (scr_distribute_datasets(map, current_id) == SCR_SUCCESS) {
                ...
                scr_reddesc reddesc;
                if (scr_distribute_reddescs(map, current_id, &reddesc) ==
                    SCR_SUCCESS) {
                    ...
                    if (tmp_rc == SCR_SUCCESS) {
                        ...
                        /* update our flush file to indicate this dataset is in
                         * cache */
                        scr_flush_file_location_set(
                            current_id, SCR_FLUSH_KEY_LOCATION_CACHE);

                        /* TODO: if storing flush file in control directory on
                         * each node, if we find any process that has marked the
                         * dataset as flushed, marked it as flushed in every
                         * flush file */

                        /* TODO: would like to restore flushing status to
                         * datasets that were in the middle of a flush, but we
                         * need to better manage the transfer file to do this,
                         * so for now just forget about flushing this dataset */
                        scr_flush_file_location_unset(
                            current_id, SCR_FLUSH_KEY_LOCATION_FLUSHING);
                    }
            ...
            if (!rebuild_succeeded) {
                /* log that we failed */
                if (scr_my_rank_world == 0) {
                    scr_dbg(1, "Failed to distribute and rebuild dataset %d",
                            current_id);
                    if (scr_log_enable) {
                        time_t now = scr_log_seconds();
                        scr_log_event("REBUILD FAILED", NULL, &current_id, &now,
                                      NULL);
                    }
                }

                /* TODO: there is a bug here, since scr_cache_delete needs to
                 * read the redundancy descriptor from the filemap in order to
                 * delete the cache directory, but we may have failed to
                 * distribute the reddescs above so not every task has one */
                ...
                if (scr_my_rank_world == 0) {
                    ...
                    if (scr_log_enable) {
                        time_t now = scr_log_seconds();
                        scr_log_event("REBUILD SUCCEEDED", NULL, &current_id,
                                      &now, NULL);
                    }
                }
            }
    ...
    if (scr_my_rank_world == 0) {
        ...
        if (distribute_attempted) {
            if (rc == SCR_SUCCESS) {
                scr_dbg(1,
                        "Scalable restart succeeded for checkpoint %d, took %f "
                        "secs",
                        scr_checkpoint_id, time_diff);
                if (scr_log_enable) {
                    scr_log_event("RESTART SUCCEEDED", NULL, &scr_checkpoint_id,
                                  &time_t_start, &time_diff);
                }
            } else {
                /* scr_checkpoint_id is not defined */
                scr_dbg(1, "Scalable restart failed, took %f secs", time_diff);
                if (scr_log_enable) {
                    scr_log_event("RESTART FAILED", NULL, NULL, &time_t_start,
                                  &time_diff);
                }
            }
        }
    ...
    if (scr_my_rank_world == 0) {
        ...
        while (flush_index < flush_ndsets && cache_index < cache_ndsets) {
            ...
            if (flush_dset < cache_dset) {
                ...
                scr_hash *dset_hash = scr_hash_set_kv_int(
                    hash, SCR_FLUSH_KEY_DATASET, cache_dset);
                scr_hash_set_kv(dset_hash, SCR_FLUSH_KEY_LOCATION,
                ...
                scr_hash *dset_hash = scr_hash_set_kv_int(
                    hash, SCR_FLUSH_KEY_DATASET, cache_dset);
                scr_hash_unset_kv(dset_hash, SCR_FLUSH_KEY_LOCATION,
                                  SCR_FLUSH_KEY_LOCATION_CACHE);
                scr_hash_set_kv(dset_hash, SCR_FLUSH_KEY_LOCATION,
                                SCR_FLUSH_KEY_LOCATION_CACHE);
                flush_index++;
        ...
        while (cache_index < cache_ndsets) {
            ...
            int cache_dset = cache_dsets[cache_index];
            scr_hash *dset_hash =
                scr_hash_set_kv_int(hash, SCR_FLUSH_KEY_DATASET, cache_dset);
            scr_hash_set_kv(dset_hash, SCR_FLUSH_KEY_LOCATION,
                            SCR_FLUSH_KEY_LOCATION_CACHE);
            cache_index++;
        }
