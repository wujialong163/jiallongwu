...
static scr_path_elem *scr_path_elem_index(const scr_path *path, int index)
{
    ...
    if (path == NULL) {
        scr_abort(-1, "Assert that path are not NULL @ %s:%d", __FILE__,
                  __LINE__);
    }

    /* check that index is in range */
    if (index < 0 || index >= path->components) {
        scr_abort(-1, "Offset %d is out of range [0,%d) @ %s:%d", index,
                  path->components, __FILE__, __LINE__);
    }
...
static int scr_path_elem_insert(scr_path *path, int offset, scr_path_elem *elem)
{
    ...
    if (offset < 0 || offset > path->components) {
        scr_abort(-1, "Offset %d is out of range @ %s:%d", offset,
                  path->components, __FILE__, __LINE__);
    }
 ...
 * after */
static char *scr_path_alloc_strf(const char *format, va_list args1,
                                 va_list args2)
{
    ...
    if (str == NULL) {
        scr_abort(-1,
                  "Failed to allocate memory for path component string @ %s:%d",
                  __FILE__, __LINE__);
    }
    ...
    if (str != NULL) {
        ...
        while (1) {
            ...
            if (buf == NULL) {
                scr_abort(
                    -1,
                    "Failed to allocate memory for component string @ %s:%d",
                    __FILE__, __LINE__);
            }
            ...
            if (elem == NULL) {
                scr_abort(
                    -1, "Failed to allocate memory for path component @ %s:%d",
                    __FILE__, __LINE__);
            }
...
scr_path *scr_path_dup(const scr_path *path)
{
    ...
    if (dup_path == NULL) {
        scr_abort(-1, "Failed to allocate path object @ %s:%d", __FILE__,
                  __LINE__);
    }
...
size_t scr_path_strcpy(char *buf, size_t n, const scr_path *path)
{
    ...
    if (path == NULL) {
        scr_abort(-1, "Cannot copy NULL pointer to string @ %s:%d", __FILE__,
                  __LINE__);
    }

    /* we can't copy a NULL path */
    if (scr_path_is_null(path)) {
        scr_abort(-1, "Cannot copy a NULL path to string @ %s:%d", __FILE__,
                  __LINE__);
    }
    ...
    if (n < strlen) {
        scr_abort(-1,
                  "User buffer of %d bytes is too small to hold string of %d "
                  "bytes @ %s:%d",
...
char *scr_path_strdup(const scr_path *path)
{
    ...
    char *buf = (char *)malloc(buflen);
    if (buf == NULL) {
        scr_abort(-1, "Failed to allocate buffer for path @ %s:%d", __FILE__,
                  __LINE__);
    }
...
static int scr_path_combine(scr_path *path1, int offset, scr_path **ptr_path2)
{
    if (path1 != NULL) {
        ...
        if (offset < 0 || offset > components) {
            scr_abort(-1, "Offset %d is out of range [0,%d] @ %s:%d", offset,
        ...
        if (ptr_path2 != NULL) {
            ...
            if (path2 != NULL) {
                ...
                /* if offset equals number of components, insert after last
                 * element, otherwise, insert element before specified element
                 */
                if (offset == components) {
                    ...
                    if (prev != NULL) {
                        ...
                        /* if prev is NULL, head of path2 will be new head of
                         * path1 */
                        path1->head = head2;
                    }
        ...
        scr_abort(-1, "Cannot attach a path to a NULL path @ %s:%d", __FILE__,
                  __LINE__);
    }
...
int scr_path_insert_str(scr_path *path, int offset, const char *str)
{
    ...
    if (path == NULL) {
        scr_abort(-1, "Cannot insert string to a NULL path @ %s:%d", __FILE__,
                  __LINE__);
    }
    ...
    if (newpath == NULL) {
        scr_abort(-1, "Failed to allocate path for insertion @ %s:%d", __FILE__,
                  __LINE__);
    }
...
int scr_path_append_str(scr_path *path, const char *str)
{
    ...
    if (path != NULL) {
        ...
        scr_abort(-1, "Cannot attach string to a NULL path @ %s:%d", __FILE__,
                  __LINE__);
    }
    return rc;
}
...
int scr_path_insert_strf(scr_path *path, int offset, const char *format, ...)
{
    /* verify that we got a path as input */
    if (path == NULL) {
        scr_abort(-1, "Cannot append string to a NULL path @ %s:%d", __FILE__,
                  __LINE__);
    }
...
scr_path *scr_path_relative(const scr_path *src, const scr_path *dst)
{
    ...
    if (src == NULL || dst == NULL) {
        scr_abort(-1, "Either src or dst pointer is NULL @ %s:%d", __FILE__,
                  __LINE__);
    }
