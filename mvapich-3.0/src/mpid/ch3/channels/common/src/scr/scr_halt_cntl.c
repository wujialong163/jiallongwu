...
/* read in halt file (which program may have changed), update internal data
 * structure, set & unset any fields, and write out halt file all while locked
 */
int scr_halt_sync_and_set(const scr_path *file_path, struct arglist *args,
                          scr_hash *data)
{
    ...
    if (args->set_checkpoints) {
        ...
        scr_hash_setf(data, NULL, "%s %lu", SCR_HALT_KEY_CHECKPOINTS,
                      args->value_checkpoints);
    ...
    if (args->set_before) {
        ...
        scr_hash_setf(data, NULL, "%s %lu", SCR_HALT_KEY_EXIT_BEFORE,
                      args->value_before);
    ...
    if (args->set_after) {
        ...
        scr_hash_setf(data, NULL, "%s %lu", SCR_HALT_KEY_EXIT_AFTER,
                      args->value_after);
    ...
    if (args->set_seconds) {
        ...
        scr_hash_setf(data, NULL, "%s %lu", SCR_HALT_KEY_SECONDS,
                      args->value_seconds);
...
int main(int argc, char *argv[])
{
    ...
    if (args.list) {
        /* if the user wants to list the values, just read the file, print the
         * values, and exit */
    ...
    if (scr_hash_util_get_str(data, SCR_HALT_KEY_EXIT_REASON, &value) ==
        SCR_SUCCESS) {
        printf("  ExitReason:      %s\n", value);
        have_one = 1;
    }
    ...
    if (scr_hash_util_get_int(data, SCR_HALT_KEY_CHECKPOINTS,
    ...
    int exit_after = -1;
    if (scr_hash_util_get_int(data, SCR_HALT_KEY_EXIT_AFTER, &exit_after) ==
        SCR_SUCCESS) {
    ...
    int exit_before = -1;
    if (scr_hash_util_get_int(data, SCR_HALT_KEY_EXIT_BEFORE, &exit_before) ==
        SCR_SUCCESS) {
    ...
    int halt_seconds = -1;
    if (scr_hash_util_get_int(data, SCR_HALT_KEY_SECONDS, &halt_seconds) ==
        SCR_SUCCESS) {
