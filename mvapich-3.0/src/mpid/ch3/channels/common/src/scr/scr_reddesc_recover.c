...
/* returns true if a an XOR file is found for this rank for the given checkpoint
 * id, sets xor_file to full filename */
static int scr_bool_have_xor_file(scr_filemap *map, int checkpoint_id,
                                  char *xor_file)
{
    int rc = 0;

    /* find the name of my xor chunk file: read filemap and check filetype of
     * each file */
    scr_hash_elem *file_elem = NULL;
    for (file_elem =
             scr_filemap_first_file(map, checkpoint_id, scr_my_rank_world);
        ...
        /* if the filetype of this file is an XOR file, copy the filename and
         * bail out */
        char *filetype = NULL;
...
/* given a filemap, a redundancy descriptor, a dataset id, and a failed rank in
 * my xor set, rebuild files and add them to the filemap */
static int scr_reddesc_recover_xor(scr_filemap *map, const scr_reddesc *c,
                                   int id, int root)
{
    int rc = SCR_SUCCESS;
    int i;
    ...
    if (root != c->my_rank) {
        ...
        if (!scr_bool_have_xor_file(map, id, full_chunk_filename)) {
            scr_abort(-1, "Missing XOR file %s @ %s:%d", full_chunk_filename,
                      __FILE__, __LINE__);
        }
        ...
        if (fd_chunk < 0) {
            scr_abort(-1,
                      "Opening XOR file for reading in XOR rebuild: "
                      "scr_open(%s, O_RDONLY) errno=%d %s @ %s:%d",
                      full_chunk_filename, errno, strerror(errno), __FILE__,
                      __LINE__);
        }
        ...
        if (scr_hash_util_get_int(current_hash, SCR_KEY_COPY_XOR_FILES,
                                  &num_files) != SCR_SUCCESS) {
            scr_abort(-1,
                      "Failed to read number of files from XOR file header: %s "
                      "@ %s:%d",
                      full_chunk_filename, __FILE__, __LINE__);
        }

        /* allocate arrays to hold file descriptors, filenames, and filesizes
         * for each of our files */
        if (num_files > 0) {
            fds = (int *)malloc(num_files * sizeof(int));
            filenames = (char **)malloc(num_files * sizeof(char *));
            filesizes =
                (unsigned long *)malloc(num_files * sizeof(unsigned long));
        ...
        for (i = 0; i < num_files; i++) {
            ...
            scr_hash *meta_tmp =
                scr_hash_get_kv_int(current_hash, SCR_KEY_COPY_XOR_FILE, i);
            if (meta_tmp == NULL) {
                scr_abort(
                    -1, "Failed to find file %d in XOR file header %s @ %s:%d",
            ...
            if (scr_meta_get_filename(meta_tmp, &filename) != SCR_SUCCESS) {
                scr_abort(-1,
                          "Failed to read filename for file %d in XOR file "
                          "header %s @ %s:%d",
                          i, full_chunk_filename, __FILE__, __LINE__);
            }
            ...
            if (scr_meta_get_filesize(meta_tmp, &filesizes[i]) != SCR_SUCCESS) {
                scr_abort(-1,
                          "Failed to read file size for file %s in XOR file "
                          "header during rebuild @ %s:%d",
                          full_file, __FILE__, __LINE__);
            }
            ...
            if (fds[i] < 0) {
                /* TODO: try again? */
                scr_abort(-1,
                          "Opening checkpoint file for reading in XOR rebuild: "
                          "scr_open(%s, O_RDONLY) errno=%d %s @ %s:%d",
                          full_file, errno, strerror(errno), __FILE__,
                          __LINE__);
            }
        ...
        /* if failed rank is to my left, i have the meta for his files, send him
         * the header */
        if (root == state->lhs_rank) {
            scr_hash_send(header, state->lhs_rank, c->comm);
        }

        /* if failed rank is to my right, send him my file info so he can write
         * his XOR header */
        ...
        /* receive number of files our left-side partner has and allocate an
         * array of meta structures to store info */
        ...
        if (scr_hash_util_get_int(current_hash, SCR_KEY_COPY_XOR_FILES,
                                  &num_files) != SCR_SUCCESS) {
            scr_abort(-1,
                      "Failed to read number of files from XOR file header "
                      "during rebuild @ %s:%d",
                      __FILE__, __LINE__);
        }
        if (num_files > 0) {
        ...
        /* set chunk filename of form:
         * <xor_rank+1>_of_<xor_groupsize>_in_<xor_groupid>.xor */
        scr_path *path_full_chunk_filename = scr_path_from_str(dir);
        scr_path_append_strf(path_full_chunk_filename, "%d_of_%d_in_%d.xor",
                             dir, c->my_rank + 1, c->ranks, c->group_id);
        scr_path_strcpy(full_chunk_filename, sizeof(full_chunk_filename),
                        path_full_chunk_filename);
        ...
        /* record our chunk file and each of our files in the filemap before
         * creating */
        ...
        scr_filemap_set_expected_files(map, id, scr_my_rank_world,
                                       num_files + 1);
        ...
        fd_chunk = scr_open(full_chunk_filename, O_WRONLY | O_CREAT | O_TRUNC,
                            mode_file);
        if (fd_chunk < 0) {
            /* TODO: try again? */
            scr_abort(-1,
                      "Opening XOR chunk file for writing in XOR rebuild: "
                      "scr_open(%s) errno=%d %s @ %s:%d",
                      full_chunk_filename, errno, strerror(errno), __FILE__,
                      __LINE__);
        }
        ...
        for (i = 0; i < num_files; i++) {
            ...
            fds[i] =
                scr_open(filenames[i], O_WRONLY | O_CREAT | O_TRUNC, mode_file);
            if (fds[i] < 0) {
                /* TODO: try again? */
                scr_abort(-1,
                          "Opening file for writing in XOR rebuild: "
                          "scr_open(%s) errno=%d %s @ %s:%d",
                          filenames[i], errno, strerror(errno), __FILE__,
                          __LINE__);
            }
        }
    ...
    if (scr_hash_util_get_unsigned_long(header, SCR_KEY_COPY_XOR_CHUNK,
                                        &chunk_size) != SCR_SUCCESS) {
        scr_abort(-1,
                  "Failed to read chunk size from XOR file header %s @ %s:%d",
                  full_chunk_filename, __FILE__, __LINE__);
    }
        ...
        while (nread < chunk_size) {
            ...
            if (root != c->my_rank) {
                ...
                if (chunk_id != c->my_rank) {
                    ...
                    if (scr_read_pad_n(num_files, filenames, fds, send_buf,
                                       count, offset,
                                       filesizes) != SCR_SUCCESS) {
                    ...
                    if (scr_read_attempt(full_chunk_filename, fd_chunk,
                ...
                /* if not start of pipeline, receive data from left and xor with
                 * my own */
                if (root != state->lhs_rank) {
                    int i;
                    MPI_Recv(recv_buf, count, MPI_BYTE, state->lhs_rank, 0,
                             c->comm, &status[0]);
                    for (i = 0; i < count; i++) {
                ...
                MPI_Send(send_buf, count, MPI_BYTE, state->rhs_rank, 0,
                         c->comm);
            } else {
                /* root of rebuild, just receive incoming chunks and write them
                 * out */
                MPI_Recv(recv_buf, count, MPI_BYTE, state->lhs_rank, 0, c->comm,
                         &status[0]);

                /* if this is not my xor chunk, write data to normal file,
                 * otherwise write to my xor chunk */
                if (chunk_id != c->my_rank) {
                    /* for this chunk, write data to the logical file */
                    if (scr_write_pad_n(num_files, filenames, fds, recv_buf,
                                        count, offset,
                                        filesizes) != SCR_SUCCESS) {
                    ...
                    if (scr_write_attempt(full_chunk_filename, fd_chunk,
    ...
    if (root == c->my_rank) {
        ...
        for (i = 0; i < num_files; i++) {
            ...
            scr_hash *meta_tmp =
                scr_hash_get_kv_int(current_hash, SCR_KEY_COPY_XOR_FILE, i);
            scr_filemap_set_meta(map, id, scr_my_rank_world, filenames[i],
                                 meta_tmp);

            /* TODODSET:write out filemap here? */

            /* if crc_on_copy is set, compute and store CRC32 value for each
             * file */
            if (scr_crc_on_copy) {
                /* check for mismatches here, in case we failed to rebuild the
                 * file correctly */
                if (scr_compute_crc(map, id, scr_my_rank_world, filenames[i]) !=
                    SCR_SUCCESS) {
                    scr_err("Failed to verify CRC32 after rebuild on file %s @ "
                            "%s:%d",
        ...
        scr_filemap_set_meta(map, id, scr_my_rank_world, full_chunk_filename,
                             meta_chunk);
        scr_filemap_write(scr_map_file, map);
         ...
         */
        if (scr_crc_on_copy) {
            /* TODO: would be nice to check for mismatches here, but we did not
             * save this value in the partner XOR file */
...
/* given a dataset id, check whether files can be rebuilt via xor and execute
 * the rebuild if needed */
static int scr_reddesc_recover_xor_attempt(scr_filemap *map,
                                           const scr_reddesc *c, int id)
{
    ...
    if (!scr_alltrue(rc == SCR_SUCCESS)) {
        if (scr_my_rank_world == 0) {
            scr_dbg(1,
                    "One or more processes failed to rebuild its files @ %s:%d",
                    __FILE__, __LINE__);
        }
        return SCR_FAILURE;
    }
...
/* rebuilds files for specified dataset id using specified redundancy
 * descriptor, adds them to filemap, and returns SCR_SUCCESS if all processes
 * succeeded */
int scr_reddesc_recover(scr_filemap *map, const scr_reddesc *c, int id)
{
    ...
    if (rc != SCR_SUCCESS) {
        if (scr_my_rank_world == 0) {
            scr_dbg(1, "Missing files @ %s:%d", __FILE__, __LINE__);
        }
        return SCR_FAILURE;
    }

    /* at this point, we should have all of our files, check that they're all
     * here */
    ...
    /* for SINGLE and PARTNER, we need to apply the copy to complete the
     * rebuild, with XOR the copy is done as part of the rebuild process */
