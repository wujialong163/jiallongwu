...
{
    printf("\n");
    printf("  Usage:  %s --dir <dir> [--latest || --needflush <id> | "
           "--location <id> | --subdir <id>]\n",
           PROG);
    printf("\n");
    exit(1);
}
...
int process_args(int argc, char **argv, struct arglist *args)
{
    ...
    if (args->dir == NULL) {
        scr_err("%s: Must specify directory containing flush file via '--dir "
                "<dir>'",
                PROG);
        return 0;
    }
    if (opCount > 1) {
        scr_err("%s: Must specify only a single operation per invocation, e.g. "
                "not both --location and --needflush'",
                PROG);
        return 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (file == NULL) {
        scr_err(
            "%s: Failed to allocate storage to store nodes file name @ %s:%d",
    ...
    if (args.need_flush != -1) {
        ...
        scr_hash *dset_hash =
            scr_hash_get_kv_int(hash, SCR_FLUSH_KEY_DATASET, args.need_flush);
        if (dset_hash != NULL) {
            /* now check for the PFS location marker */
            scr_hash *location_hash =
                scr_hash_get(dset_hash, SCR_FLUSH_KEY_LOCATION);
            scr_hash_elem *pfs_elem =
                scr_hash_elem_get(location_hash, SCR_FLUSH_KEY_LOCATION_PFS);
            if (pfs_elem == NULL) {
    ...
    if (args.location != -1) {
        ...
        scr_hash *dset_hash =
            scr_hash_get_kv_int(hash, SCR_FLUSH_KEY_DATASET, args.location);
        if (dset_hash != NULL) {
    ...
    if (args.subdir != -1) {
        ...
        scr_hash *dset_hash =
            scr_hash_get_kv_int(hash, SCR_FLUSH_KEY_DATASET, args.subdir);
        if (dset_hash != NULL) {
            /* now check for the subdir */
            char *subdir;
            if (scr_hash_util_get_str(dset_hash, SCR_FLUSH_KEY_DIRECTORY,
                                      &subdir) == SCR_SUCCESS) {
    ...
    if (args.latest) {
        ...
        for (elem = scr_hash_elem_first(latest_hash); elem != NULL;
