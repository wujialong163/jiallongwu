...
static int scr_fetch_file(const char *dst_file, const char *src_dir,
                          const scr_meta *meta)
{
    int rc = SCR_SUCCESS;
    ...
    if (scr_meta_get_crc32(meta, &meta_crc) == SCR_SUCCESS) {
        if (rc == SCR_SUCCESS && scr_crc_on_flush && crc != meta_crc) {
            rc = SCR_FAILURE;
            scr_err("CRC32 mismatch detected when fetching file from %s to %s "
                    "@ %s:%d",
...
int scr_container_get_name_size_offset_length(const scr_hash *segment,
                                              ...
                                              unsigned long *length)
{
    ...
    if (segment == NULL || containers == NULL || name == NULL || size == NULL ||
        offset == NULL || length == NULL) {
        return SCR_FAILURE;
    }

    /* lookup the segment length */
    if (scr_hash_util_get_bytecount(segment, SCR_SUMMARY_6_KEY_LENGTH,
                                    length) != SCR_SUCCESS) {
        return SCR_FAILURE;
    }
    ...
    int id;
    if (scr_hash_util_get_int(container, SCR_SUMMARY_6_KEY_ID, &id) !=
        SCR_SUCCESS) {
        return SCR_FAILURE;
    }

    /* lookup the offset value */
    if (scr_hash_util_get_bytecount(container, SCR_SUMMARY_6_KEY_OFFSET,
                                    offset) != SCR_SUCCESS) {
        return SCR_FAILURE;
    }
...
static int scr_fetch_file_from_containers(const char *file, scr_meta *meta,
                                          scr_hash *segments,
                                          const scr_hash *containers)
{
    ...
    if (file == NULL || strcmp(file, "") == 0) {
        scr_err("Invalid source file @ %s:%d", __FILE__, __LINE__);
        return SCR_FAILURE;
    }

    /* check that our other arguments are valid */
    if (meta == NULL || segments == NULL || containers == NULL) {
        scr_err("Invalid metadata, segments, or container @ %s:%d", __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (fd_src < 0) {
        scr_err("Opening file to copy: scr_open(%s) errno=%d %s @ %s:%d", file,
    ...
    */
    ...
    if (buf == NULL) {
        scr_err("Allocating memory: malloc(%llu) errno=%d %s @ %s:%d", buf_size,
                errno, strerror(errno), __FILE__, __LINE__);
    ...
    scr_hash_elem *elem;
    for (elem = scr_hash_elem_first(segments); elem != NULL;
        ...
        if (scr_container_get_name_size_offset_length(
                hash, containers, &container_name, &container_size,
                &container_offset, &segment_length) != SCR_SUCCESS) {
            scr_err("Failed to get segment offset and length @ %s:%d", __FILE__,
                    __LINE__);
            rc = SCR_FAILURE;
            break;
        }
        ...
        if (fd_container < 0) {
            scr_err(
                "Opening file for reading: scr_open(%s) errno=%d %s @ %s:%d",
        ...
        from the file again, and it won't bother keeping them in the page cache.
        */
        posix_fadvise(fd_container, 0, 0,
        ...
        if (lseek(fd_container, pos, SEEK_SET) == (off_t)-1) {
            ...
            scr_err("Failed to seek to byte %lu in %s @ %s:%d", pos,
                    container_name, __FILE__, __LINE__);
            rc = SCR_FAILURE;
            break;
        }
        ...
        while (remaining > 0) {
            ...
            int nread =
                scr_read_attempt(container_name, fd_container, buf, count);
    ...
    if (rc == SCR_SUCCESS) {
        ...
        if (scr_crc_on_flush) {
            if (scr_meta_get_crc32(meta, &crc2) == SCR_SUCCESS) {
                ...
                if (crc != crc2) {
                    scr_err(
                        "CRC32 mismatch detected when fetching file %s @ %s:%d",
...
static int scr_fetch_files_list(const scr_hash *file_list, const char *dir,
                                scr_filemap *map)
{
    ...
    for (file_elem = scr_hash_elem_first(files); file_elem != NULL;
         ...
         * the file names */
        scr_hash_elem *no_fetch_hash =
            scr_hash_elem_get(hash, SCR_SUMMARY_6_KEY_NOFETCH);
        ...
        if (scr_hash_util_get_unsigned_long(hash, SCR_KEY_SIZE, &filesize) !=
            SCR_SUCCESS) {
        ...
        int complete = 1;
        if (scr_hash_util_get_int(hash, SCR_KEY_COMPLETE, &complete) !=
            SCR_SUCCESS) {
        ...
        if (containers != NULL) {
            ...
            if (scr_fetch_file_from_containers(newfile, meta, segments,
                                               containers) != SCR_SUCCESS) {
            ...
            char *from_dir;
            if (scr_hash_util_get_str(hash, SCR_KEY_PATH, &from_dir) ==
                SCR_SUCCESS) {
...
static int scr_fetch_rank2file_map(const scr_path *dataset_path, int depth,
                                   int *ptr_valid, char **ptr_file,
                                   unsigned long *ptr_offset)
{
    ...
    if (valid) {
        ...
        if (fd >= 0) {
            ...
            if (read_rc < 0) {
                scr_err("Failed to read from %s @ %s:%d", file, __FILE__,
                        __LINE__);
                rc = SCR_FAILURE;
            }
            ...
            scr_err("Failed to open rank2file map %s @ %s:%d", file, __FILE__,
                    __LINE__);
            rc = SCR_FAILURE;
        }
    }
    ...
    if (elem != NULL) {
        ...
        if (scr_hash_util_get_bytecount(elem_hash, SCR_SUMMARY_6_KEY_OFFSET,
                                        &newoffset) != SCR_SUCCESS) {
            rc = SCR_FAILURE;
        }
    }
    ...
    *ptr_offset = newoffset;
    ...
    if (level_id > 1) {
        rc = scr_fetch_rank2file_map(dataset_path, depth + 1, ptr_valid,
                                     ptr_file, ptr_offset);
    }
...
static int scr_summary_read_mpi(const scr_path *dataset_path,
                                scr_hash *file_list)
{
    int rc = SCR_SUCCESS;
    ...
    if (scr_my_rank_world == 0) {
        ...
        if (fd >= 0) {
            ...
            scr_err("Failed to open summary file %s @ %s:%d", summary_file,
                    __FILE__, __LINE__);
            rc = SCR_FAILURE;
        }
    ...
    if (valid) {
        ...
        if (fd >= 0) {
            ...
            if (readsize < 0) {
                scr_err("Failed to read rank2file map file %s @ %s:%d", file,
                        __FILE__, __LINE__);
                rc = SCR_FAILURE;
            }
            ...
            if (ranks != scr_ranks_world) {
                scr_err(
                    "Invalid number of ranks in %s, got %d expected %d @ %s:%d",
     ...
     * list of files */
    scr_hash_elem *elem;
    for (elem = scr_hash_elem_first(recv); elem != NULL;
         elem = scr_hash_elem_next(elem)) {
 ...
 * filemap */
static int scr_fetch_data(const scr_hash *file_list, const char *dir,
                          scr_filemap *map)
{
    ...
    if (scr_my_rank_world == 0) {
        ...
        while (i < scr_ranks_world || outstanding > 0) {
            ...
            while (i < scr_ranks_world && outstanding < w) {
                 ...
                 * rank i is done */
                MPI_Irecv(&flags[index + w], 1, MPI_INT, i, 0, scr_comm_world,
                          &req[index + w]);

                /* send a start signal to this rank */
                flags[index] = success;
                MPI_Isend(&flags[index], 1, MPI_INT, i, 0, scr_comm_world,
                          &req[index]);
...
static int scr_fetch_files(scr_filemap *map, scr_path *fetch_path,
                           int *dataset_id, int *checkpoint_id)
{
    ...
    if (scr_fetch_summary(fetch_dir, file_list) != SCR_SUCCESS) {
        if (scr_my_rank_world == 0) {
            scr_dbg(1, "Failed to read summary file @ %s:%d", __FILE__,
                    __LINE__);
            if (scr_log_enable) {
                double time_end = MPI_Wtime();
                double time_diff = time_end - time_start;
                time_t now = scr_log_seconds();
                scr_log_event("FETCH FAILED", fetch_dir, NULL, &now,
                              &time_diff);
            }
        }
    ...
    if (scr_dataset_get_id(dataset, &id) != SCR_SUCCESS) {
        if (scr_my_rank_world == 0) {
            scr_dbg(1, "Invalid id in summary file @ %s:%d", __FILE__,
                    __LINE__);
            if (scr_log_enable) {
                double time_end = MPI_Wtime();
                double time_diff = time_end - time_start;
    ...
    if (scr_dataset_get_ckpt(dataset, &ckpt_id) != SCR_SUCCESS) {
         ...
         * but we don't yet */
        scr_err("Failed to read checkpoint id from dataset @ %s:%d", __FILE__,
                __LINE__);
        scr_hash_delete(&file_list);
        scr_free(&fetch_dir);
        return SCR_FAILURE;
    ...
    scr_reddesc *c =
        scr_reddesc_for_checkpoint(ckpt_id, scr_nreddescs, scr_reddescs);
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_dbg(1,
                "scr_fetch_files: %f secs, %e bytes, %f MB/s, %f MB/s per proc",
        ...
        if (scr_log_enable) {
            time_t now = scr_log_seconds();
            if (rc == SCR_SUCCESS) {
                scr_log_event("FETCH SUCCEEDED", fetch_dir, &id, &now,
                              &time_diff);
            } else {
                scr_log_event("FETCH FAILED", fetch_dir, &id, &now, &time_diff);
            }
...
int scr_fetch_sync(scr_filemap *map, int *fetch_attempted)
{
    ...
    while (continue_fetching) {
        ...
        if (scr_my_rank_world == 0) {
            ...
            char *current_str;
            if (scr_index_get_current(index_hash, &current_str) ==
                SCR_SUCCESS) {
            ...
            if (strcmp(target, "") != 0) {
                ...
                scr_index_get_id_by_dir(index_hash, target,
                                        &next_checkpoint_id);
            } else {
                /* otherwise, just get the most recent complete checkpoint
                 * (that's older than the current id) */
                scr_index_get_most_recent_complete(index_hash,
                                                   current_checkpoint_id,
                                                   &next_checkpoint_id, target);
            }
            ...
            if (strcmp(target, "") != 0) {
                ...
                *fetch_attempted = 1;
                if (current_checkpoint_id != -1) {
                    scr_index_mark_fetched(index_hash, current_checkpoint_id,
                                           target);
        ...
        if (!scr_path_is_null(fetch_path)) {
            ...
            if (rc == SCR_SUCCESS) {
                ...
                if (scr_my_rank_world == 0) {
                    ...
                    if (current_checkpoint_id != -1 &&
                        strcmp(target, "") != 0) {
                        scr_index_mark_failed(index_hash, current_checkpoint_id,
                                              target);
                    }
                    scr_index_write(scr_prefix_path, index_hash);
                }
