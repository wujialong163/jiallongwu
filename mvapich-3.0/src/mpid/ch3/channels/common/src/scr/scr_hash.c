...
#define SCR_FILE_FLAGS_CRC32                                                   \
    (0x1) /* indicates that crc32 is stored at end of file */
...
static scr_hash_elem *scr_hash_elem_init(scr_hash_elem *elem, const char *key,
                                         scr_hash *hash)
{
    if (elem != NULL) {
        if (key != NULL) {
            ...
            elem->key = NULL;
            scr_err("Setting hash element key to NULL @ %s:%d", __FILE__,
                    __LINE__);
        }
        elem->hash = hash;
    }
...
int scr_hash_merge(scr_hash *hash1, const scr_hash *hash2)
{
    ...
    scr_hash_elem *elem;
    for (elem = scr_hash_elem_first(hash2); elem != NULL;
         elem = scr_hash_elem_next(elem)) {
...
scr_hash *scr_hash_setf(scr_hash *hash, scr_hash *hash_value,
                        const char *format, ...)
{
    ...
    if (format_copy == NULL) {
        scr_abort(-1, "Failed to duplicate format string @ %s:%d", __FILE__,
                  __LINE__);
    }
    ...
    while (i < count && token != NULL && h != NULL) {
        ...
        if (strcmp(token, "%s") == 0) {
            ...
            size =
                snprintf(key, sizeof(key), token, va_arg(args, unsigned long));
        } else if (strcmp(token, "%#x") == 0) {
            size =
                snprintf(key, sizeof(key), token, va_arg(args, unsigned int));
        } else if (strcmp(token, "%#lx") == 0) {
            size =
                snprintf(key, sizeof(key), token, va_arg(args, unsigned long));
        } else if (strcmp(token, "%llu") == 0) {
            size = snprintf(key, sizeof(key), token,
                            va_arg(args, unsigned long long));
        } else if (strcmp(token, "%f") == 0) {
            size = snprintf(key, sizeof(key), token, va_arg(args, double));
        } else {
            scr_abort(-1, "Unsupported hash key format '%s' @ %s:%d", token,
                      __FILE__, __LINE__);
        }
        ...
        if (size >= sizeof(key)) {
            scr_abort(-1,
                      "Key buffer too small, have %lu need %d bytes @ %s:%d",
...
int scr_hash_sort(scr_hash *hash, int direction)
{
    ...
    struct sort_elem *list =
        (struct sort_elem *)malloc(count * sizeof(struct sort_elem));
    if (list == NULL) {
        return SCR_FAILURE;
    }
...
int scr_hash_list_int(const scr_hash *hash, int *n, int **v)
{
    ...
    *v = NULL;
    ...
    if (list == NULL) {
        scr_abort(-1, "Failed to allocate integer list at %s:%d", __FILE__,
                  __LINE__);
    }
 ...
 * with one call */
scr_hash *scr_hash_get_kv(const scr_hash *hash, const char *key,
                          const char *val)
{
    if (hash == NULL) {
        return NULL;
 ...
 * element */
scr_hash_elem *scr_hash_elem_extract_by_addr(scr_hash *hash,
                                             scr_hash_elem *elem)
{
...
char *scr_hash_get_val(const scr_hash *hash, const char *key)
{
    ...
    if (key_hash != NULL) {
        ...
        if (size == 1) {
            ...
            /* this is an error */
            scr_err("Hash for key %s expected to have exactly one element, but "
                    "it has %d @ %s:%d",
...
int scr_hash_write_persist(void **ptr_buf, size_t *ptr_size,
                           const scr_hash *hash)
{
    ...
    if (size != bufsize) {
        scr_abort(
            -1,
            "Failed to persist hash wrote %lu bytes != expected %lu @ %s:%d",
...
int scr_hash_write(const char *file, const scr_hash *hash)
{
    ...
    if (fd < 0) {
        scr_err("Opening hash file for write: %s @ %s:%d", file, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
...
ssize_t scr_hash_read_fd(const char *file, int fd, scr_hash *hash)
{
    ...
    /* check that the file type is something we understand */
    /* check that the file version matches */
    if (magic != SCR_FILE_MAGIC || type != SCR_FILE_TYPE_HASH ||
        version != SCR_FILE_VERSION_HASH_1) {
    ...
    if (filesize < SCR_FILE_HASH_HEADER_SIZE) {
        scr_err("Invalid file size stored in %s @ %s:%d", file, __FILE__,
                __LINE__);
        return -1;
    }
    ...
    if (remainder > 0) {
        ...
        if (nread != remainder) {
            scr_err("Failed to read file %s @ %s:%d", file, __FILE__, __LINE__);
            scr_free(&buf);
            return -1;
        }
    }
    ...
    if (crc_set) {
        ...
        /* read the crc value */
        uint32_t crc_file_network, crc_file;
        memcpy(&crc_file_network, buf + filesize - sizeof(uint32_t),
               sizeof(uint32_t));
        crc_file = scr_ntoh32(crc_file_network);

        /* check the crc value */
        if (crc != crc_file) {
            scr_err("CRC32 mismatch detected in %s @ %s:%d", file, __FILE__,
                    __LINE__);
            scr_free(&buf);
            return -1;
        }
...
int scr_hash_read(const char *file, scr_hash *hash)
{
    ...
    if (fd < 0) {
        scr_err("Opening hash file for read %s @ %s:%d", file, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
...
int scr_hash_read_with_lock(const char *file, scr_hash *hash)
{
    ...
    if (file == NULL) {
        scr_err("No filename specified @ %s:%d", __FILE__, __LINE__);
        return SCR_FAILURE;
    }

    /* check that we got a hash to read data into */
    if (hash == NULL) {
        scr_err("No hash provided to read data into @ %s:%d", __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (fd >= 0) {
        ...
        scr_err("Failed to open file with lock %s @ %s:%d", file, __FILE__,
                __LINE__);
    }
...
int scr_hash_lock_open_read(const char *file, int *fd, scr_hash *hash)
{
    ...
    if (fd == NULL) {
        scr_err("Must provide a pointer to an int to return opened file "
                "descriptor @ %s:%d",
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
...
int scr_hash_write_close_unlock(const char *file, int *fd, const scr_hash *hash)
{
    ...
    /* check that we got a hash to read data into */
    if (hash == NULL) {
        scr_err("No hash provided to write data from @ %s:%d", __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
