...
static int scr_sort_bitonic_merge(
    void *value, void *scratch, MPI_Datatype type, size_t type_size,
    size_t offset, int (*compare)(const void *, const void *, size_t),
    int start, int num, int direction, MPI_Comm comm, int tag)
{
    if (num > 1) {
        ...
        if (rank < start + count) {
            ...
            if (dst_rank < start + num) {
                ...
                MPI_Sendrecv(value, 1, type, dst_rank, tag, scratch, 1, type,
                             dst_rank, tag, comm, status);
            ...
            scr_sort_bitonic_merge(value, scratch, type, type_size, offset,
                                   compare, start, count, direction, comm, tag);
        } else {
            ...
            scr_sort_bitonic_merge(value, scratch, type, type_size, offset,
                                   compare, new_start, new_num, direction, comm,
                                   tag);
        }
    }
    ...
    int start, int num, int direction, MPI_Comm comm, int tag)
{
    if (num > 1) {
        ...
        if (rank < start + mid) {
            ...
            scr_sort_bitonic_sort(value, scratch, type, type_size, offset,
                                  compare, start, mid, !direction, comm, tag);
        } else {
        ...
        scr_sort_bitonic_merge(value, scratch, type, type_size, offset, compare,
                               start, num, direction, comm, tag);
    }
...
static int scr_sort_bitonic(void *value, MPI_Datatype type, size_t type_size,
                            size_t data_offset,
                            ...
                            MPI_Comm comm, int tag)
{
    ...
    int rc = scr_sort_bitonic_sort(value, scratch, type, type_size, data_offset,
                                   compare, 0, ranks, 1, comm, tag);
 ...
 *      sides of host value */
static int scr_split_sorted(const void *value, MPI_Datatype type,
                            size_t type_size, size_t data_offset,
                            int (*compare)(const void *, const void *, size_t),
                            MPI_Comm comm, int tag1, int tag2, int recv_ints[7])
{
    int k;
    MPI_Request request[4];
    MPI_Status status[4];

    /* we will fill in 7 integer values (src, left, right, rank, size, groupid,
     * groups) representing the chain data structure for the the globally
    ...
    if (left_rank != MPI_PROC_NULL) {
        MPI_Isend((void *)value, 1, type, left_rank, tag1, comm, &request[k]);
        k++;

        MPI_Irecv(left_buf, 1, type, left_rank, tag1, comm, &request[k]);
        k++;
    }
    if (right_rank != MPI_PROC_NULL) {
        MPI_Isend((void *)value, 1, type, right_rank, tag1, comm, &request[k]);
        k++;

        MPI_Irecv(right_buf, 1, type, right_rank, tag1, comm, &request[k]);
        k++;
    }
    if (k > 0) {
    ...
    while (left_rank != MPI_PROC_NULL || right_rank != MPI_PROC_NULL) {
        ...
        if (left_rank != MPI_PROC_NULL) {
            MPI_Irecv(recv_left_ints, 4, MPI_INT, left_rank, tag1, comm,
                      &request[k]);
            k++;
            ...
            MPI_Isend(send_left_ints, 4, MPI_INT, left_rank, tag1, comm,
                      &request[k]);
            k++;
        }
        ...
        if (right_rank != MPI_PROC_NULL) {
            MPI_Irecv(recv_right_ints, 4, MPI_INT, right_rank, tag1, comm,
                      &request[k]);
            k++;
            ...
            MPI_Isend(send_right_ints, 4, MPI_INT, right_rank, tag1, comm,
                      &request[k]);
            k++;
        }
    ...
    send_ints[CHAIN_SIZE] =
        send_right_ints[SCAN_COUNT] + send_left_ints[SCAN_COUNT] - 1;
    send_ints[CHAIN_ID] = send_right_ints[SCAN_COLOR] - 1;
    send_ints[CHAIN_COUNT] =
        send_right_ints[SCAN_COLOR] + send_left_ints[SCAN_COLOR] - 1;
    ...
    MPI_Isend(send_ints, 7, MPI_INT, send_ints[CHAIN_SRC], tag2, comm,
              &request[0]);
    MPI_Irecv(recv_ints, 7, MPI_INT, MPI_ANY_SOURCE, tag2, comm, &request[1]);
    ...
    scr_sort_bitonic(send_ints, result_type, result_type_size, 0, scr_cmp_int,
                     comm, tag1);
...
int scr_rank_str(MPI_Comm comm, const char *str, int *groups, int *groupid)
{
    ...
    *ptr = rank;
    ...
    scr_sort_bitonic(buf, type, type_size, data_offset, scr_cmp_str_int, comm,
                     tag1);
    ...
    int recv_ints[7];
    scr_split_sorted(buf, type, type_size, data_offset, scr_cmp_str, comm, tag1,
                     tag2, recv_ints);
