...
int process_args(int argc, char **argv, struct arglist *args)
{
    ...
    if (args->dir == NULL) {
        scr_err(
            "%s: Must specify directory containing halt file via '--dir <dir>'",
            PROG);
        return 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    /* otherwise, assume that we don't need to halt, and check for valid
     * condition */
    int rc = DONT_HALT;
    ...
    if (scr_hash_util_get_int(scr_halt_hash, SCR_HALT_KEY_EXIT_BEFORE,
        ...
        if (now >= (exit_before - halt_seconds)) {
            ...
            printf("%s: HALT RUN: Current time (%s) is past "
    ...
    if (scr_hash_util_get_int(scr_halt_hash, SCR_HALT_KEY_EXIT_AFTER,
        ...
        if (now >= exit_after) {
            ...
            strftime(str_now, sizeof(str_now), "%c", localtime(&time_now));
            strftime(str_exit, sizeof(str_exit), "%c", localtime(&time_exit));
            printf("%s: HALT RUN: Current time (%s) is past ExitAfter time "
                   "(%s).\n",
                   PROG, str_now, str_exit);
            rc = NEED_HALT;
        }
