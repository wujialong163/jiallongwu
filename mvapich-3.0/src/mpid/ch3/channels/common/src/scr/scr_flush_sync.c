...
static int scr_flush_a_file(const char *src_file, const char *dst_dir,
                            scr_meta *meta)
{
    ...
    scr_dbg(2,
            "scr_flush_a_file: Read and copied %s to %s with success code %d @ "
            "%s:%d",
    ...
    if (crc_valid) {
        ...
        if (scr_meta_get_crc32(meta, &crc_meta) == SCR_SUCCESS) {
            if (crc != crc_meta) {
                ...
                flushed = SCR_FAILURE;
                scr_err("scr_flush_a_file: CRC32 mismatch detected when "
                        "flushing file %s to %s @ %s:%d",
            ...
            /* the crc was not already in the metafile, but we just computed it,
             * so set it */
    ...
    /* (we don't update the meta file here, since perhaps the file in cache is
     * ok and only the flush failed) */
...
/* given a filename, its meta data, its list of segments, and list of
 * destination containers, copy file to container files */
static int scr_flush_file_to_containers(const char *file, scr_meta *meta,
                                        scr_hash *segments,
                                        const scr_hash *containers)
{
    ...
    */
    ...
    for (elem = scr_hash_elem_first(segments); elem != NULL;
        ...
        /* get the offset into the container and the length of the segment (both
         * in bytes) */
        char *container_name;
        ...
        /* open container file for writing -- we don't truncate here because
         * more than one process may be writing to the same file */
        int fd_container =
            scr_open(container_name, O_WRONLY | O_CREAT, mode_file);
        if (fd_container < 0) {
            scr_err(
                "Opening file for writing: scr_open(%s) errno=%d %s @ %s:%d",
        ...
        */
        ...
        while (remaining > 0) {
            ...
            if (nread > 0) {
                ...
                int nwrite =
                    scr_write_attempt(container_name, fd_container, buf, nread);
    ...
    if (rc == SCR_SUCCESS) {
        ...
        if (scr_crc_on_flush) {
            if (scr_meta_get_crc32(meta, &crc2) == SCR_SUCCESS) {
                ...
                if (crc != crc2) {
                    scr_err(
                        "CRC32 mismatch detected when flushing file %s @ %s:%d",
...
/* flush files specified in list, and record corresponding entries for summary
 * file */
static int scr_flush_files_list(scr_hash *file_list, scr_hash *summary)
{
    ...
    if (scr_hash_util_get_str(file_list, SCR_KEY_PATH, &summary_dir) !=
        SCR_SUCCESS) {
        scr_abort(-1,
                  "Failed to get summary file directory from file list @ %s:%d",
                  __FILE__, __LINE__);
    }
    ...
    for (elem = scr_hash_elem_first(files); elem != NULL;
        ...
        if (containers != NULL) {
            ...
            scr_hash *file_hash =
                scr_hash_set_kv(summary, SCR_SUMMARY_6_KEY_FILE, name);
            scr_free(&name);
            ...
            if (scr_flush_file_to_containers(file, meta, segments,
                ...
                if (scr_meta_get_filesize(meta, &filesize) == SCR_SUCCESS) {
                    scr_hash_util_set_bytecount(
                        file_hash, SCR_SUMMARY_6_KEY_SIZE, filesize);
                }
                ...
                scr_hash_set(file_hash, SCR_SUMMARY_6_KEY_SEGMENT,
                             segments_copy);
            } else {
                /* the flush failed */
                rc = SCR_FAILURE;
            ...
            char *dir;
            if (scr_hash_util_get_str(hash, SCR_KEY_PATH, &dir) ==
                SCR_SUCCESS) {
                /* create full path of destination file */
                scr_path *path_full = scr_path_from_str(dir);
                scr_path_append(path_full, path_name);

                /* get relative path to flushed file from directory containing
                 * summary file */
                scr_path *path_relative =
                    scr_path_relative(path_summary_dir, path_full);
                if (!scr_path_is_null(path_relative)) {
                    /* record the name of the file in the summary hash, and get
                     * reference to a hash for this file */
                    ...
                    if (scr_flush_a_file(file, dir, meta) == SCR_SUCCESS) {
                         ...
                         */
                        unsigned long filesize = 0;
                        if (scr_meta_get_filesize(meta, &filesize) ==
                            SCR_SUCCESS) {
                            scr_hash_util_set_bytecount(
                                file_hash, SCR_SUMMARY_6_KEY_SIZE, filesize);
                        }
                        ...
                        uLong crc = 0;
                        if (scr_meta_get_crc32(meta, &crc) == SCR_SUCCESS) {
                            scr_hash_util_set_crc32(file_hash,
                                                    SCR_SUMMARY_6_KEY_CRC, crc);
                        }
                    } else {
                        /* the flush failed */
                        rc = SCR_FAILURE;
                        ...
                        scr_hash_set_kv_int(file_hash,
                                            SCR_SUMMARY_6_KEY_COMPLETE, 0);
                    }
                } else {
                    scr_abort(-1,
                              "Failed to get relative path to directory %s "
                              "from %s @ %s:%d",
                ...
                scr_abort(-1,
                          "Failed to read directory to flush file to @ %s:%d",
                          __FILE__, __LINE__);
            }
        }
    ...
    if (scr_my_rank_world == 0) {
        ...
        while (i < scr_ranks_world || outstanding > 0) {
            ...
            while (i < scr_ranks_world && outstanding < w) {
                 ...
                 * rank i is done */
                MPI_Irecv(&flags[w + index], 1, MPI_INT, i, 0, scr_comm_world,
                          &req[w + index]);
            ...
            /* someone responded, the send to this rank should also be done, so
             * complete it */
        ...
        /* flush files if we've had success so far, otherwise skip the flush and
         * return failure */
...
int scr_flush_sync(scr_filemap *map, int id)
{
    ...
    if (scr_my_rank_world == 0) {
        ...
        scr_dbg(1,
                "scr_flush_sync: %f secs, %e bytes, %f MB/s, %f MB/s per proc",
