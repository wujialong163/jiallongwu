...
mpigcc -g -O0 -o scr_index_cmd scr_index_cmd.c scr_hash.o
scr_halt_cntl-scr_err_serial.o scr_io.o scr_index.o scr_meta.o scr_filemap.o
tv_data_display.o -lz mpigcc -g -O0 -o scr_index_cmd scr_index_cmd.c scr_hash.o
scr_halt_cntl-scr_err_serial.o scr_io.o scr_index.o scr_meta.o scr_filemap.o
tv_data_display.o -lz
/usr/global/tools/totalview/v/totalview.8X.8.0-4/linux-x86-64/lib/libdbfork_64.a
...
int scr_read_dir(const scr_path *dir, scr_hash *hash)
{
    ...
    if (dirp == NULL) {
        scr_err("Failed to open directory %s (errno=%d %s) @ %s:%d", dir_str,
    ...
    /* close the directory */
    if (closedir(dirp) < 0) {
        scr_err("Failed to close directory %s (errno=%d %s) @ %s:%d", dir_str,
                errno, strerror(errno), __FILE__, __LINE__);
        scr_free(&dir_str);
        return SCR_FAILURE;
    }
...
int scr_summary_write(const scr_path *dir, scr_hash *hash)
{
    ...
    while (elem != NULL) {
        ...
        while (count < 8192) {
            ...
            scr_hash *rank_hash =
                scr_hash_set_kv_int(entries, SCR_SUMMARY_6_KEY_RANK, rank);
        ...
        scr_hash *files_rank_hash =
            scr_hash_set_kv_int(files_hash, SCR_SUMMARY_6_KEY_RANK, writer);
        scr_hash_util_set_str(files_rank_hash, SCR_SUMMARY_6_KEY_FILE,
                              rank2file_name);
        scr_hash_util_set_bytecount(files_rank_hash, SCR_SUMMARY_6_KEY_OFFSET,
                                    offset);
...
/* forks and execs processes to rebuild missing files and waits for them to
 * complete, returns SCR_FAILURE if any dataset failed to rebuild, SCR_SUCCESS
 * otherwise */
int scr_fork_rebuilds(const scr_path *dir, scr_hash *cmds)
{
    ...
    if (builds > 0) {
        ...
        if (pids == NULL) {
            scr_err("Failed to allocate space to record pids @ %s:%d", __FILE__,
                    __LINE__);
            return SCR_FAILURE;
        }
    }
    ...
    for (elem = scr_hash_elem_first(cmds); elem != NULL;
         elem = scr_hash_elem_next(elem)) {
        ...
        for (arg_elem = scr_hash_elem_first(cmd_hash); arg_elem != NULL;
            ...
            if (remaining > 0) {
                offset +=
                    snprintf(full_cmd + offset, remaining, "%s ", arg_str);
            }
        }
        ...
        if (pids[pid_count] == 0) {
            ...
            if (chdir(dir_str) != 0) {
                scr_err("Failed to change to directory %s @ %s:%d", dir_str,
                        __FILE__, __LINE__);
                exit(1);
            }
    ...
    while (pid_count > 0) {
        ...
        if (ret == -1 || ret == (pid_t)-1) {
            scr_err("Got a -1 from wait @ %s:%d", __FILE__, __LINE__);
            rc = SCR_FAILURE;
        } else if (stat != 0) {
            scr_err("Child returned with non-zero @ %s:%d", __FILE__, __LINE__);
            rc = SCR_FAILURE;
        }
        pid_count--;
    }
...
int scr_rebuild_scan(const scr_path *dir, scr_hash *scan)
{
    ...
    for (dset_elem = scr_hash_elem_first(dsets_hash); dset_elem != NULL;
        ...
        /* if the dataset is marked as inconsistent -- consider it to be beyond
         * repair */
        ...
        if (missing_hash != NULL) {
            ...
            for (xor_elem = scr_hash_elem_first(xors_hash); xor_elem != NULL;
                ...
                int members;
                if (scr_hash_util_get_int(xor_hash, SCR_SCAN_KEY_MEMBERS,
                                          &members) != SCR_SUCCESS) {
                    /* unknown number of members in this set, skip this set */
                    scr_err("Unknown number of members in XOR set %d in "
                            "dataset %d @ %s:%d",
                 ...
                 */
                scr_hash *members_hash =
                    scr_hash_get(xor_hash, SCR_SCAN_KEY_MEMBER);
                int members_have = scr_hash_size(members_hash);
                if (members_have < members - 1) {
                    /* not enough members to attempt rebuild of this set, skip
                     * it */
                    /* TODO: most likely this means that a rank can't be
                     * recovered */
                    rc = SCR_FAILURE;
                    continue;
                }
                 ...
                 *   or if we have all members but one of the corresponding
                 * ranks is missing files (got the XOR file, but missing the
                 * full file) */
                int missing_count = 0;
                int missing_member = -1;
                int member;
                for (member = 1; member <= members; member++) {
                    scr_hash *member_hash = scr_hash_get_kv_int(
                        xor_hash, SCR_SCAN_KEY_MEMBER, member);
                    if (member_hash == NULL) {
                        ...
                        char *rank_str;
                        if (scr_hash_util_get_str(member_hash,
                                                  SCR_SUMMARY_6_KEY_RANK,
                                                  &rank_str) == SCR_SUCCESS) {
                            /* check whether we're missing any files for this
                             * rank */
                            scr_hash *missing_rank_hash =
                                scr_hash_get(missing_hash, rank_str);
                            if (missing_rank_hash != NULL) {
                                /* we have the XOR file for this member, but
                                 * we're missing one or more regular files */
                                missing_member = member;
                                missing_count++;
                            }
                        } else {
                            /* couldn't identify rank for this member, print an
                             * error */
                            scr_err(
                                "Could not identify rank corresponding to "
                                "member %d of XOR set %d in dataset %d @ %s:%d",
                ...
                if (missing_count > 1) {
                    /* TODO: unrecoverable */
                    scr_hash_set_kv_int(dset_hash, SCR_SCAN_KEY_UNRECOVERABLE,
                                        xor_setid);
                } else if (missing_count > 0) {
                    scr_hash *buildcmd_hash = scr_hash_set_kv_int(
                        dset_hash, SCR_SCAN_KEY_BUILD, build_command_count);
                    build_command_count++;

                    int argc = 0;

                    /* write the command name */
                    scr_hash_setf(buildcmd_hash, NULL, "%d %s", argc,
                                  BUILD_CMD);
                    argc++;
                    ...
                    /* write the index of the missing xor file (convert from
                     * 1-based index to 0-based index) */
                    scr_hash_setf(buildcmd_hash, NULL, "%d %d", argc,
                                  missing_member - 1);
                    argc++;

                    /* build the name of the missing xor file */
                    scr_path *missing_filepath = scr_path_from_str(".scr");
                    scr_path_append_strf(missing_filepath, "%d_of_%d_in_%d.xor",
                                         missing_member, members, xor_setid);
                    char *missing_filename_str =
                        scr_path_strdup(missing_filepath);
                    scr_hash_setf(buildcmd_hash, NULL, "%d %s", argc,
                                  missing_filename_str);
                    scr_free(&missing_filename_str);
                    scr_path_delete(&missing_filepath);
                    argc++;

                    /* write each of the existing xor file names, skipping the
                     * missing member */
                    for (member = 1; member <= members; member++) {
                        ...
                        scr_hash *member_hash = scr_hash_get_kv_int(
                            xor_hash, SCR_SCAN_KEY_MEMBER, member);
                        char *filename = scr_hash_elem_get_first_val(
                            member_hash, SCR_SUMMARY_6_KEY_FILE);
                        scr_hash_setf(buildcmd_hash, NULL, "%d %s", argc,
                                      filename);
                        argc++;
                    }
            ...
            /* rebuild if we can */
            scr_hash *unrecoverable =
                scr_hash_get(dset_hash, SCR_SCAN_KEY_UNRECOVERABLE);
            char *dir_str = scr_path_strdup(dir);
            if (unrecoverable != NULL) {
                /* at least some files cannot be recovered */
                scr_err("Insufficient files to attempt rebuild of dataset %d "
                        "in %s @ %s:%d",
                        dset_id, dir_str, __FILE__, __LINE__);
                rc = SCR_FAILURE;
            } else {
                /* we have a shot to rebuild everything, let's give it a go */
                scr_hash *builds_hash =
                    scr_hash_get(dset_hash, SCR_SCAN_KEY_BUILD);
                if (scr_fork_rebuilds(dir, builds_hash) != SCR_SUCCESS) {
                    scr_err("At least one rebuild failed for dataset %d in %s "
                            "@ %s:%d",
                            dset_id, dir_str, __FILE__, __LINE__);
                    rc = SCR_FAILURE;
                }
...
{
    /* assume nothing is missing, we'll set this to 1 if we find anything that
     * is */
    int any_missing = 0;
    ...
    for (dset_elem = scr_hash_elem_first(dsets); dset_elem != NULL;
         dset_elem = scr_hash_elem_next(dset_elem)) {
        /* get the dataset id */
        int dset_id = scr_hash_elem_key_int(dset_elem);
        ...
        scr_hash *rank2file_hash =
            scr_hash_get(dset_hash, SCR_SUMMARY_6_KEY_RANK2FILE);
        scr_hash *ranks_count_hash =
            scr_hash_get(rank2file_hash, SCR_SUMMARY_6_KEY_RANKS);
        ...
        if (ranks_size != 1) {
            ...
            scr_err("Dataset %d has more than one value for the number of "
                    "ranks @ %s:%d",
        ...
        scr_hash *ranks_hash =
            scr_hash_get(rank2file_hash, SCR_SUMMARY_6_KEY_RANK);
        scr_hash_sort_int(ranks_hash, SCR_HASH_SORT_ASCENDING);

        /* for each rank, check that we have each of its files */
        int expected_rank = 0;
        scr_hash_elem *rank_elem = NULL;
        for (rank_elem = scr_hash_elem_first(ranks_hash); rank_elem != NULL;
            ...
            if (rank_id < expected_rank) {
                ...
                dataset_valid = 0;
                scr_err("Internal error: Rank out of order %d expected %d in "
                        "dataset %d @ %s:%d",
            ...
            if (rank_id >= ranks) {
                ...
                dataset_valid = 0;
                scr_err("Rank %d out of range, expected at most %d ranks in "
                        "dataset %d @ %s:%d",
                        rank_id, ranks, dset_id, __FILE__, __LINE__);
            }

            /* if rank_id is higher than expected rank, mark the expected rank
             * as missing */
            while (expected_rank < rank_id) {
                scr_hash_set_kv_int(dset_hash, SCR_SCAN_KEY_MISSING,
                                    expected_rank);
                expected_rank++;
            }

            /* get the hash for the FILES key */
            scr_hash *files_count_hash =
                scr_hash_get(rank_hash, SCR_SUMMARY_6_KEY_FILES);

            /* check that this dataset has only one value for the FILES key */
            int files_size = scr_hash_size(files_count_hash);
            if (files_size != 1) {
                /* found more than one FILES value for this rank, mark it as
                 * incomplete */
                dataset_valid = 0;
                scr_err("Rank %d of dataset %d has more than one value for the "
                        "number of files @ %s:%d",
            ...
            scr_hash *files_hash =
                scr_hash_get(rank_hash, SCR_SUMMARY_6_KEY_FILE);

            /* check that each file is marked as complete */
            int file_count = 0;
            scr_hash_elem *file_elem = NULL;
            for (file_elem = scr_hash_elem_first(files_hash); file_elem != NULL;
                ...
                int complete;
                if (scr_hash_util_get_int(file_hash, SCR_SUMMARY_6_KEY_COMPLETE,
                                          &complete) == SCR_SUCCESS) {
                    if (complete == 0) {
                        /* file is explicitly marked as incomplete, add the rank
                         * to the missing list */
                        scr_hash_set_kv_int(dset_hash, SCR_SCAN_KEY_MISSING,
                                            rank_id);
                    }
            ...
            /* if we found more files than expected, mark the dataset as
             * incomplete */
            if (file_count > files) {
                dataset_valid = 0;
                scr_err("Rank %d in dataset %d has more files than expected @ "
                        "%s:%d",
                        rank_id, dset_id, __FILE__, __LINE__);
            }
        ...
        if (expected_rank > ranks) {
            ...
            dataset_valid = 0;
            scr_err("Dataset %d has more ranks than expected @ %s:%d", dset_id,
                    __FILE__, __LINE__);
        }
        ...
        /* if dataset is not marked invalid, and if there are no missing files,
...
/* Reads *.scrfilemap files from given dataset directory and adds them to scan
 * hash. Returns SCR_SUCCESS if the files could be scanned */
int scr_scan_file(const scr_path *dir, const scr_path *path_name, int *ranks,
                  regex_t *re_xor_file, scr_hash *scan)
{
    ...
    if (scr_filemap_read(path_name, rank_map) != SCR_SUCCESS) {
        ...
        scr_err("Error reading filemap: %s @ %s:%d", path_err, __FILE__,
                __LINE__);
        scr_free(&path_err);
    ...
    scr_hash_elem *dset_elem = NULL;
    for (dset_elem = scr_filemap_first_dataset(rank_map); dset_elem != NULL;
         dset_elem = scr_hash_elem_next(dset_elem)) {
        /* get the dataset id */
        int dset_id = scr_hash_elem_key_int(dset_elem);
        ...
        scr_hash *list_hash =
            scr_hash_set_kv_int(scan, SCR_SCAN_KEY_DLIST, dset_id);

        /* lookup rank2file hash for this dataset, allocate a new one if it's
         * not found */
        scr_hash *rank2file_hash =
            scr_hash_get(list_hash, SCR_SUMMARY_6_KEY_RANK2FILE);
        if (rank2file_hash == NULL) {
            /* there is no existing rank2file hash, create a new one and add it
             */
            rank2file_hash = scr_hash_new();
            scr_hash_set(list_hash, SCR_SUMMARY_6_KEY_RANK2FILE,
                         rank2file_hash);
        }
        ...
        for (rank_elem = scr_filemap_first_rank_by_dataset(rank_map, dset_id);
             rank_elem != NULL; rank_elem = scr_hash_elem_next(rank_elem)) {
            /* get the rank number */
            int rank_id = scr_hash_elem_key_int(rank_elem);
            ...
            scr_dataset *current_dset =
                scr_hash_get(list_hash, SCR_SUMMARY_6_KEY_DATASET);
            if (current_dset == NULL) {
                /* there is no dataset hash currently assigned, so use the one
                 * for the current rank */
                scr_hash_set(list_hash, SCR_SUMMARY_6_KEY_DATASET, rank_dset);
            } else {
                /* TODODSET */
                /* check that the dataset for this rank matches the one we
                 * already have */
            ...
            scr_hash *rank_hash = scr_hash_set_kv_int(
                rank2file_hash, SCR_SUMMARY_6_KEY_RANK, rank_id);

            /* set number of expected files for this rank */
            int num_expect =
                scr_filemap_get_expected_files(rank_map, dset_id, rank_id);
            ...
            for (file_elem = scr_filemap_first_file(rank_map, dset_id, rank_id);
                 file_elem != NULL; file_elem = scr_hash_elem_next(file_elem)) {
                ...
                if (scr_filemap_get_meta(rank_map, dset_id, rank_id, file_name,
                                         meta) != SCR_SUCCESS) {
                    scr_err("Failed to read meta data for %s from dataset %d @ "
                            "%s:%d",
                ...
                /* only check files ending with .scr and skip the summary.scr
                 * file check that file is complete check that file exists check
                 * that file size matches check that ranks agree check that
                 * checkpoint id agrees */
        ...
        if (scr_meta_get_rank(meta, &meta_rank) != SCR_SUCCESS) {
          ...
          continue;
                ...
                if (scr_meta_get_filename(meta, &meta_filename) !=
                    SCR_SUCCESS) {
                ...
                if (scr_meta_get_filesize(meta, &meta_filesize) !=
                    SCR_SUCCESS) {
                ...
                if (scr_meta_is_complete(meta) != SCR_SUCCESS) {
                    scr_err("File is not complete: %s @ %s:%d", full_filename,
                            __FILE__, __LINE__);
                    scr_meta_delete(&meta);
                    scr_free(&full_filename);
                    continue;
                ...
                if (scr_file_exists(full_filename) != SCR_SUCCESS) {
                    scr_err("File does not exist: %s @ %s:%d", full_filename,
                            __FILE__, __LINE__);
                    scr_meta_delete(&meta);
                    scr_free(&full_filename);
                    continue;
                ...
                if (meta_filesize != size) {
                    scr_err("File is %lu bytes but expected to be %lu bytes: "
                            "%s @ %s:%d",
                            size, meta_filesize, full_filename, __FILE__,
                            __LINE__);
                    scr_meta_delete(&meta);
                    scr_free(&full_filename);
                    continue;
                ...
                if (meta_ranks != *ranks) {
                    scr_err("File was created with %d ranks, but expected %d "
                            "ranks: %s @ %s:%d",
                            meta_ranks, *ranks, full_filename, __FILE__,
                            __LINE__);
                    scr_meta_delete(&meta);
                    scr_free(&full_filename);
                    continue;
                ...
                scr_hash *rank2file_hash =
                    scr_hash_get(list_hash, SCR_SUMMARY_6_KEY_RANK2FILE);
                scr_hash_set_kv_int(rank2file_hash, SCR_SUMMARY_6_KEY_RANKS,
                                    meta_ranks);
                scr_hash *rank_hash = scr_hash_set_kv_int(
                    rank2file_hash, SCR_SUMMARY_6_KEY_RANK, rank_id);
                scr_hash *file_hash = scr_hash_set_kv(
                    rank_hash, SCR_SUMMARY_6_KEY_FILE, file_name);
                scr_hash_util_set_bytecount(file_hash, SCR_SUMMARY_6_KEY_SIZE,
                                            meta_filesize);

                uLong meta_crc;
                if (scr_meta_get_crc32(meta, &meta_crc) == SCR_SUCCESS) {
                    scr_hash_util_set_crc32(file_hash, SCR_SUMMARY_6_KEY_CRC,
                                            meta_crc);
                }

                /* if the file is an XOR file, read in the XOR set parameters */
                if (scr_meta_check_filetype(meta, SCR_META_FILE_XOR) ==
                    SCR_SUCCESS) {
                    /* mark this file as being an XOR file */
                    scr_hash_set(file_hash, SCR_SUMMARY_6_KEY_NOFETCH, NULL);

                    /* extract the xor set id, the size of the xor set, and our
                     * position within the set */
                    size_t nmatch = 4;
                    regmatch_t pmatch[nmatch];
                    char *value = NULL;
                    int xor_rank, xor_ranks, xor_setid;
                    if (regexec(re_xor_file, full_filename, nmatch, pmatch,
                                0) == 0) {
                        xor_rank = -1;
                        xor_ranks = -1;
                        xor_setid = -1;
                        ...
                        if (xor_rank != -1 && xor_ranks != -1 &&
                             ...
                             *             RANK
                             *               <rank_id> */
                            scr_hash *xor_hash = scr_hash_set_kv_int(
                                list_hash, SCR_SCAN_KEY_XOR, xor_setid);
                            scr_hash_set_kv_int(xor_hash, SCR_SCAN_KEY_MEMBERS,
                                                xor_ranks);
                            scr_hash *xor_rank_hash = scr_hash_set_kv_int(
                                xor_hash, SCR_SCAN_KEY_MEMBER, xor_rank);
                            scr_hash_set_kv(xor_rank_hash,
                                            SCR_SUMMARY_6_KEY_FILE, file_name);
                            scr_hash_set_kv_int(
                                xor_rank_hash, SCR_SUMMARY_6_KEY_RANK, rank_id);
                        } else {
                            scr_err("Failed to extract XOR rank, set size, or "
                                    "set id from %s @ %s:%d",
                                    full_filename, __FILE__, __LINE__);
                        }
                    } else {
                        scr_err("XOR file does not match expected file name "
                                "format %s @ %s:%d",
                                full_filename, __FILE__, __LINE__);
                    }
                }
...
int scr_scan_files(const scr_path *dir, scr_hash *scan)
{
    ...
    /* set up a regular expression so we can extract the xor set information
     * from a file */
        ...
        if (dp != NULL) {
            ...
            if (name != NULL) {
                ...
                if (strcmp(ext, ".scrfilemap") == 0) {
                    ...
                    int tmp_rc = scr_scan_file(dir, path_name, &ranks,
                                               &re_xor_file, scan);
...
int scr_summary_build(const scr_path *dir)
{
    ...
    if (scr_summary_read(dir, summary) != SCR_SUCCESS) {
         ...
         *   delete XOR files from the file list, and adjust the expected number
         * of files (maybe we should just leave these in here, at least the
         * missing list?) */
        scr_hash_elem *list_elem = NULL;
        scr_hash *list_hash = scr_hash_get(scan, SCR_SCAN_KEY_DLIST);
        int list_size = scr_hash_size(list_hash);
        if (list_size == 1) {
            for (list_elem = scr_hash_elem_first(list_hash); list_elem != NULL;
                 list_elem = scr_hash_elem_next(list_elem)) {
                /* get the hash for this checkpoint */
                scr_hash *dset_hash = scr_hash_elem_hash(list_elem);

                /* unset the BUILD, MISSING, UNRECOVERABLE, INVALID, and XOR
                 * keys for this checkpoint */
                ...
                scr_hash_set_kv_int(dset_hash, SCR_SUMMARY_KEY_VERSION,
                                    SCR_SUMMARY_FILE_VERSION_6);
...
int is_complete(const scr_path *prefix, const scr_path *subdir)
{
    ...
    if (scr_index_get_id_by_dir(index, subdir_str, &id) == SCR_SUCCESS) {
        if (id != -1) {
            ...
            int complete = 0;
            if (scr_index_get_complete(index, id, subdir_str, &complete) ==
                SCR_SUCCESS) {
                if (complete == 1) {
                    /* only return success if we find a value for complete, and
                     * if that value is 1 */
                    rc = SCR_SUCCESS;
                }
            }
...
int index_list(const scr_path *prefix)
{
    ...
    if (scr_index_read(prefix, index) != SCR_SUCCESS) {
        scr_err("Failed to read index file in %s @ %s:%d", prefix_str, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    scr_hash_elem *elem;
    for (elem = scr_hash_elem_first(dset_hash); elem != NULL;
         elem = scr_hash_elem_next(elem)) {
        /* get the dataset id */
        ...
        for (dir_elem = scr_hash_elem_first(dir_hash); dir_elem != NULL;
            ...
            int complete = 0;
            scr_hash_util_get_int(info_hash, SCR_INDEX_1_KEY_COMPLETE,
                                  &complete);

            /* determine time at which this checkpoint was marked as failed */
            char *failed_str = NULL;
            scr_hash_util_get_str(info_hash, SCR_INDEX_1_KEY_FAILED,
                                  &failed_str);

            /* determine time at which this checkpoint was flushed */
            char *flushed_str = NULL;
            scr_hash_util_get_str(info_hash, SCR_INDEX_1_KEY_FLUSHED,
                                  &flushed_str);

            /* compute number of times (and last time) checkpoint has been
             * fetched */
            /*
                  scr_hash* fetched_hash = scr_hash_get(info_hash,
               SCR_INDEX_1_KEY_FETCHED); int num_fetch =
               scr_hash_size(fetched_hash); scr_hash_sort(fetched_hash,
               SCR_HASH_SORT_DESCENDING); scr_hash_elem* fetched_elem =
               scr_hash_elem_first(fetched_hash); char* fetched_str =
               scr_hash_elem_key(fetched_elem);
            */
    ...
    if (scr_index_get_id_by_dir(index, subdir_str, &id) != SCR_SUCCESS) {
        ...
        if (dataset != NULL) {
            ...
            if (scr_dataset_get_id(dataset, &dataset_id) == SCR_SUCCESS) {
                /* found the id, now check whether it's complete (assume that
                 * it's not) */
                int complete;
                if (scr_hash_util_get_int(summary, SCR_SUMMARY_6_KEY_COMPLETE,
                                          &complete) == SCR_SUCCESS) {
                    /* write values to the index file */
                    scr_index_set_dataset(index, dataset_id, subdir_str,
                                          dataset, complete);
...
int print_usage()
{
    ...
    printf("  Options:\n");
    printf(
        "    -l, --list           List indexed datasets (default behavior)\n");
    printf("    -a, --add=<dir>      Add dataset directory <dir> to index\n");
    printf("    -r, --remove=<dir>   Remove dataset directory <dir> from index "
           "(does not delete files)\n");
    printf("    -c, --current=<dir>  Set <dir> as restart directory\n");
    printf("    -p, --prefix=<dir>   Specify prefix directory (defaults to "
...
int get_args(int argc, char **argv, struct arglist *args)
{
    ...
    /* if the user didn't specify a prefix directory, use the current working
     * directory */
    if (args->prefix == NULL) {
        char prefix[SCR_MAX_FILENAME];
        if (getcwd(prefix, sizeof(prefix)) == NULL) {
            scr_err("Problem reading current working directory (getcwd() "
                    "errno=%d %s) @ %s:%d",
                    errno, strerror(errno), __FILE__, __LINE__);
            return SCR_FAILURE;
        }
...
int main(int argc, char *argv[])
{
    ...
    if (args.add == 1) {
        /* add the dataset directory dir to the index.scr file in the prefix
         * directory, rebuild missing files if necessary */
        rc = SCR_FAILURE;
        if (index_add_dir(prefix, subdir) == SCR_SUCCESS) {
            rc = is_complete(prefix, subdir);
        }
    } else if (args.remove == 1) {
        /* remove the named directory from the index file (does not delete
         * files) */
