...
int scr_cache_dir_get(const scr_reddesc *red, int id, char *dir)
{
    ...
    if (red == NULL || red->directory == NULL) {
        scr_abort(
            -1, "NULL redundancy descriptor or NULL dataset directory @ %s:%d",
            __FILE__, __LINE__);
    }
...
int scr_cache_dir_create(const scr_reddesc *red, int id)
{
    ...
    if (store != NULL) {
        ...
        if (scr_storedesc_dir_create(store, dir) != SCR_SUCCESS) {
             ...
             * fatal error if not */
            scr_abort(-1,
                      "Failed to create dataset directory, aborting @ %s:%d",
                      __FILE__, __LINE__);
        }
    } else {
        scr_abort(-1, "Invalid store descriptor @ %s:%d", __FILE__, __LINE__);
    }
...
int scr_cache_delete(scr_filemap *map, int id)
{
    ...
    for (rank_elem = scr_filemap_first_rank_by_dataset(map, id);
        ...
        for (file_elem = scr_filemap_first_file(map, id, rank);
            ...
            /* check file's crc value (monitor that cache hardware isn't
             * corrupting files on us) */
            if (scr_crc_on_delete) {
                /* TODO: if corruption, need to log */
                if (scr_compute_crc(map, id, rank, file) != SCR_SUCCESS) {
                    scr_err("Failed to verify CRC32 before deleting file %s, "
                            "bad drive? @ %s:%d",
    ...
    /* TODO: due to bug in scr_cache_rebuild, we need to pull the dataset
     * directory
     * from somewhere other than the redundancy descriptor, which may not be
     * defined */
...
int scr_unlink_rank(scr_filemap *map, int id, int rank)
{
    ...
    scr_hash_elem *file_elem;
    for (file_elem = scr_filemap_first_file(map, id, rank); file_elem != NULL;
         file_elem = scr_hash_elem_next(file_elem)) {
        char *file = scr_hash_elem_key(file_elem);
        if (file != NULL) {
            scr_dbg(2, "Delete file Dataset %d, Rank %d, File %s", id, rank,
                    file);
...
int scr_cache_clean(scr_filemap *map)
{
    ...
    for (dset_elem = scr_filemap_first_dataset(map); dset_elem != NULL;
        ...
        for (rank_elem = scr_filemap_first_rank_by_dataset(map, dset);
            ...
            for (file_elem = scr_filemap_first_file(map, dset, rank);
                ...
                if (!scr_bool_have_file(map, dset, rank, file,
                    ...
                    missing_file = 1;
                    scr_dbg(2,
                            "File is unreadable or incomplete: Dataset %d, "
                            "Rank %d, File: %s",
                            dset, rank, file);
                }
            }
            ...
            if (scr_filemap_get_dataset(map, dset, rank, dataset) ==
                SCR_SUCCESS) {
            ...
            int expected_files =
                scr_filemap_get_expected_files(map, dset, rank);
             ...
             * keep_map */
            if (!missing_file) {
                scr_filemap_set_expected_files(keep_map, dset, rank,
                                               expected_files);
            }
            ...
            for (file_elem = scr_filemap_first_file(map, dset, rank);
                ...
                if (missing_file) {
                    ...
                    if (scr_filemap_get_meta(map, dset, rank, file, meta) ==
                        SCR_SUCCESS) {
...
int scr_cache_check_files(const scr_filemap *map, int id)
{
    ...
    for (rank_elem = scr_filemap_first_rank_by_dataset(map, id);
        ...
        for (file_elem = scr_filemap_first_file(map, id, rank);
            ...
            scr_meta *meta = scr_meta_new();
            if (scr_filemap_get_meta(map, id, rank, file, meta) !=
                SCR_SUCCESS) {
                failed_read = 1;
            } else {
...
int scr_bool_have_file(const scr_filemap *map, int dset, int rank,
                       const char *file, int ranks)
{
    /* if no filename is given return false */
    if (file == NULL || strcmp(file, "") == 0) {
        scr_dbg(2, "File name is null or the empty string @ %s:%d", __FILE__,
                __LINE__);
        return 0;
    }
    ...
    /* check that we can read the file */
    if (scr_file_is_readable(file) != SCR_SUCCESS) {
        scr_dbg(2, "Do not have read access to file: %s @ %s:%d", file,
                __FILE__, __LINE__);
        return 0;
    }
    ...
    if (scr_filemap_get_meta(map, dset, rank, file, meta) != SCR_SUCCESS) {
        scr_dbg(2, "Failed to read meta data for file: %s @ %s:%d", file,
                __FILE__, __LINE__);
        scr_meta_delete(&meta);
        return 0;
    }
    ...
    /* check that the file is complete */
    if (scr_meta_is_complete(meta) != SCR_SUCCESS) {
        scr_dbg(2, "File is marked as incomplete: %s @ %s:%d", file, __FILE__,
                __LINE__);
        scr_meta_delete(&meta);
        return 0;
    }
  ...
  if (ranks != meta_ranks) {
    ...
    if (size != meta_size) {
        scr_dbg(
            2,
            "Filesize is incorrect, currently %lu, expected %lu for %s @ %s:%d",
