...
int need_transfer(scr_hash *files, char *src, char **dst, off_t *position,
                  off_t *filesize)
{
    ...
    char *dest;
    if (scr_hash_util_get_bytecount(file_hash, SCR_TRANSFER_KEY_SIZE, &size) ==
            SCR_SUCCESS &&
        scr_hash_util_get_bytecount(file_hash, SCR_TRANSFER_KEY_WRITTEN,
                                    &written) == SCR_SUCCESS &&
        scr_hash_util_get_str(file_hash, SCR_TRANSFER_KEY_DESTINATION, &dest) ==
            SCR_SUCCESS) {
...
int find_file(scr_hash *hash, char **src, char **dst, off_t *position,
              off_t *filesize)
{
    int found_a_file = 0;
    ...
    if (files != NULL) {
        ...
        if (!found_a_file && src != NULL && *src != NULL) {
             ...
             * need_transfer call */
            char *tmp_dst = NULL;
            if (need_transfer(files, *src, &tmp_dst, position, filesize) ==
                SCR_SUCCESS) {
        ...
        if (!found_a_file) {
            ...
            for (elem = scr_hash_elem_first(files); elem != NULL;
                ...
                if (name != NULL && need_transfer(files, name, dst, position,
                                                  filesize) == SCR_SUCCESS) {
...
int set_transfer_file_state(char *s, int done)
{
    ...
    if (scr_hash_lock_open_read(scr_transfer_file, &fd, hash) == SCR_SUCCESS) {
        ...
        if (done) {
            scr_hash_set_kv(hash, SCR_TRANSFER_KEY_FLAG,
...
scr_hash *read_transfer_file()
{
    ...
    if (value != NULL) {
        ...
        if (scr_atod(value, &percent) == SCR_SUCCESS) {
            ...
            /* could not interpret bandwidth value */
            scr_err(
                "scr_transfer: Ignoring invalid PERCENT value in %s @ %s:%d",
                scr_transfer_file, __FILE__, __LINE__);
        }
    } else {
    ...
    /* check for DONE flag */
    int done = 0;
    scr_hash *done_hash = scr_hash_get_kv(hash, SCR_TRANSFER_KEY_FLAG,
                                          SCR_TRANSFER_KEY_FLAG_DONE);
    if (done_hash != NULL) {
        done = 1;
    }
    ...
    if (value != NULL) {
        if (strcmp(value, SCR_TRANSFER_KEY_COMMAND_EXIT) == 0) {
            ...
            scr_err("scr_transfer: Unknown command %s in %s @ %s:%d", value,
                    scr_transfer_file, __FILE__, __LINE__);
        }
    }
    ...
    if (value == NULL) {
        if (state == STOPPED) {
            ...
            scr_err("scr_transfer: Unknown state %d @ %s:%d", state, __FILE__,
                    __LINE__);
        }
    }
...
int update_transfer_file(char *src, char *dst, off_t position)
{
    ...
    if (scr_hash_lock_open_read(scr_transfer_file, &fd, hash) == SCR_SUCCESS) {
        ...
        if (src != NULL) {
            scr_hash *file_hash =
                scr_hash_get_kv(hash, SCR_TRANSFER_KEY_FILES, src);
            if (file_hash != NULL) {
                /* update the bytes written field */
                scr_hash_util_set_bytecount(file_hash, "WRITTEN",
                                            (uint64_t)position);
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    if (scr_transfer_file == NULL) {
        scr_err("scr_transfer: Copying transfer file name @ %s:%d", __FILE__,
                __LINE__);
        return 1;
    }
    ...
    if (buf == NULL) {
        scr_err("scr_transfer: Failed to allocate %llu bytes for file copy "
                "buffer @ %s:%d",
    ...
    while (keep_running) {
        ...
        while (keep_running && (state == STOPPED || secs_remain > 0.0)) {
            ...
            if (state == RUNNING) {
                ...
                /* based on the amount we last wrote and our allocated
                 * bandwidth, compute time we need to sleep before attempting
                 * our next write */
                 ...
                 * compute time we need to sleep before attempting our next
                 * write */
                ...
                if (percent_runtime > 0.0) {
                    ...
                    secs_remain_runtime =
                        secs_run / percent_runtime - secs_total;
                }
            ...
            if (state != old_state) {
                if (state == RUNNING) {
                    ...
                    clear_parameters(&new_file_src, &new_file_dst,
                                     &new_position);
                    clear_parameters(&old_file_src, &old_file_dst,
                                     &old_position);

                    /* after closing our files, update our state in the transfer
                     * file */
                    set_transfer_file_state(SCR_TRANSFER_KEY_STATE_STOP, 0);
                }
            }
        ...
        if (state == RUNNING) {
            ...
            off_t filesize = 0;
            find_file(hash, &new_file_src, &new_file_dst, &new_position,
                      &filesize);
            ...
            if (bool_diff_files(new_file_dst, old_file_dst)) {
                ...
                if (new_file_dst != NULL) {
                    fd_dst =
                        scr_open(new_file_dst, O_RDWR | O_CREAT, mode_file);
                    /* TODO: check for errors here */
            ...
            if (fd_src >= 0 && fd_dst >= 0) {
                ...
                if (nread > 0) {
                    ...
                    update_transfer_file(new_file_src, new_file_dst,
                                         new_position);
                }
