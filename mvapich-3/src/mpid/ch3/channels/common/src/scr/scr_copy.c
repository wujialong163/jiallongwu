...
int process_args(int argc, char **argv, struct arglist *args)
{
    ...
    do {
        ...
        int option_index = 0;
        c = getopt_long(argc, argv, "c:i:d:b:rpnh", long_options,
                        &option_index);
        switch (c) {
            case 'c':
                /* control directory */
            ...
            case 'i':
                ...
                id = atoi(optarg);
                if (id <= 0) {
                    scr_err("%s: Dataset id must be positive '--id %s'", PROG,
                            optarg);
                    return 0;
                }
                args->id = id;
            ...
            default:
                if (c != -1) {
                    /* missed an option */
                    scr_err("%s: Option '%s' specified but not processed", PROG,
                            argv[option_index]);
                }
        }
    } while (c != -1);
    ...
    if (args->dstdir == NULL) {
        scr_err(
            "%s: Must specify destination directory via '--dstdir <dst_dir>'",
            PROG);
        return 0;
    }
...
static int scr_bool_have_file(const scr_filemap *map, int id, int rank,
                              const char *file)
{
    ...
    if (valid &&
        scr_filemap_get_meta(map, id, rank, file, meta) != SCR_SUCCESS) {
  ...
  if (valid && scr_meta_check_rank(meta, rank) != SCR_SUCCESS) {
    ...
    if (valid && scr_meta_check_filesize(meta, size) != SCR_SUCCESS) {
        scr_dbg(2, "%s: Filesize is incorrect, currently %lu for %s", PROG,
                size, file);
        valid = 0;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (gethostname(hostname, sizeof(hostname)) != 0) {
        scr_err("scr_copy: Call to gethostname failed @ %s:%d", __FILE__,
                __LINE__);
    ...
    for (rank_elem = scr_filemap_first_rank_by_dataset(map, args.id);
        ...
        int preserve_dirs = 0;
        scr_hash_util_get_int(flushdesc, SCR_SCAVENGE_KEY_PRESERVE,
                              &preserve_dirs);

        /* determine whether we're using containers */
        int container = 0;
        scr_hash_util_get_int(flushdesc, SCR_SCAVENGE_KEY_CONTAINER,
                              &container);
        ...
        scr_hash_util_set_int(rank_flushdesc, SCR_SCAVENGE_KEY_PRESERVE,
                              preserve_dirs);
        scr_hash_util_set_int(rank_flushdesc, SCR_SCAVENGE_KEY_CONTAINER,
                              container);
        ...
        for (file_elem = scr_filemap_first_file(map, args.id, rank);
            ...
            if (scr_bool_have_file(map, args.id, rank, file)) {
                ...
                int user_file = 0;
                if (scr_meta_check_filetype(meta, SCR_META_FILE_USER) ==
                    SCR_SUCCESS) {
                    user_file = 1;
                }
                ...
                if (user_file) {
                    ...
                    if (preserve_dirs) {
                         ...
                         */
                        if (scr_meta_get_origpath(meta, &dst_dir) !=
                            SCR_SUCCESS) {
                            printf("scr_copy: %s: Could not find original path "
                                   "for file %s in dataset id %d\n",
                        ...
                        /* TODO: keep a cache of directory names that we've
                         * already created */

                        /* make directory to file */
                        if (scr_mkdir(dst_dir, S_IRWXU) != SCR_SUCCESS) {
                            printf("scr_copy: %s: Failed to create original "
                                   "path for file %s in dataset id %d\n",
                                   hostname, file, args.id);
                            printf("scr_copy: %s: Return code: 1\n", hostname);
                            scr_meta_delete(&meta);
                            scr_filemap_delete(&map);
                ...
                scr_path *path_relative =
                    scr_path_relative(path_dataset, dst_path);
                ...
                if (crc_valid) {
                    ...
                    if (scr_meta_get_crc32(meta, &meta_crc) == SCR_SUCCESS) {
                        if (crc != meta_crc) {
                            ...
                            rc = 1;
                            scr_err("scr_copy: CRC32 mismatch detected when "
                                    "flushing file %s to %s @ %s:%d",
                                    file, dst_file, __FILE__, __LINE__);

                            /* TODO: would be good to log this, but right now
                             * only rank 0 can write log entries */
                            /*
                            if (scr_log_enable) {
                              time_t now = scr_log_seconds();
                              scr_log_event("CRC32 MISMATCH", my_flushed_file,
                            NULL, &now, NULL);
                            }
                            */
                        }
                ...
                scr_filemap_set_meta(rank_map, args.id, rank, file_relative,
                                     meta);
                ...
                rc = 1;
                scr_err("scr_copy: File is unreadable or incomplete: "
                        "CheckpointID %d, Rank %d, File: %s",
                        args.id, rank, file);
            }
        }
