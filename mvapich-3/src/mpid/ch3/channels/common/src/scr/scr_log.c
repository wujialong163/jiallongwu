...
static unsigned long scr_db_jobid =
    0; /* caches the jobid for the current job */
#ifdef HAVE_LIBMYSQLCLIENT
static scr_hash *scr_db_types =
    NULL; /* caches type string to type id lookups */
#endif
...
int scr_mysql_connect()
{
    ...
    if (scr_db_types == NULL) {
        scr_err("Failed to create a hash to cache type string to id lookups @ "
                "%s:%d",
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (!mysql_real_connect(&scr_mysql, scr_db_host, scr_db_user, scr_db_pass,
                            scr_db_name, 0, NULL, 0)) {
...
char *scr_mysql_quote_string(const char *value)
{
    ...
    if (value != NULL) {
        ...
        if (q != NULL) {
            ...
            scr_err(
                "Failed to allocate buffer space to encode string %s @ %s:%d",
...
/* given a number of seconds since the epoch, fill in a string for mysql
 * datetime */
...
char *scr_mysql_quote_int(const int *value)
{
    ...
    if (value != NULL) {
        ...
        if (n >= sizeof(buf)) {
            scr_err("Insufficient buffer space (%lu bytes) to encode value "
                    "(%lu needed) @ %s:%d",
...
int scr_mysql_read_id(const char *table, const char *name, unsigned long *id)
{
    ...
    int n = snprintf(query, sizeof(query),
                     "SELECT * FROM `%s` WHERE `name` = %s ;", table, qname);
    ...
    if (n >= sizeof(query)) {
        scr_err("Insufficient buffer space (%lu bytes) to build query (%lu "
                "bytes) @ %s:%d",
    ...
    if (mysql_real_query(&scr_mysql, query, (unsigned int)strlen(query))) {
        scr_err("Select failed, query = (%s), error = (%s) @ %s:%d", query,
                mysql_error(&scr_mysql), __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (res == NULL) {
        scr_err("Result failed, query = (%s), error = (%s) @ %s:%d", query,
                mysql_error(&scr_mysql), __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (row == NULL) {
        scr_err("Row fetch failed, query = (%s), error = (%s) @ %s:%d", query,
                mysql_error(&scr_mysql), __FILE__, __LINE__);
        mysql_free_result(res);
        return SCR_FAILURE;
    }
...
int scr_mysql_read_write_id(const char *table, const char *name,
                            unsigned long *id)
{
    int rc = SCR_SUCCESS;
    ...
    if (mysql_real_query(&scr_mysql, query, (unsigned int)strlen(query))) {
        scr_err("Insert failed, query = (%s), error = (%s) @ %s:%d", query,
                mysql_error(&scr_mysql), __FILE__, __LINE__);
        /* don't return failure, since another process may have just beat us to
         * the punch */
        /*return SCR_FAILURE;*/
    }
...
int scr_mysql_type_id(const char *type, int *id)
{
    ...
    if (type == NULL) {
        scr_err("Type string is NULL @ %s:%d", __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (id_str != NULL) {
        /* found our id from the hash, convert to an unsigned long, then to an
         * int */
    ...
    if (scr_mysql_read_write_id("types", type, &tmp_id) != SCR_SUCCESS) {
        scr_err("Failed to find type_id for %s @ %s:%d", type, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
...
int scr_mysql_log_event(const char *type, const char *note, const int *ckpt,
                        const time_t *start, const double *secs)
{
#ifdef HAVE_LIBMYSQLCLIENT
    /* lookup the id for the type string */
    int type_id = -1;
    if (scr_mysql_type_id(type, &type_id) == SCR_FAILURE) {
        scr_err("Failed to lookup id for type string %s @ %s:%d", type,
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    /* check that we got valid strings for each of our parameters */
    if (qnote == NULL || qckpt == NULL || qstart == NULL || qsecs == NULL) {
        scr_err("Failed to escape and quote one or more arguments @ %s:%d",
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
...
/* records an SCR file transfer (checkpoint/fetch/flush/drain) in the SCR log
 * database */
int scr_mysql_log_transfer(const char *type, const char *from, const char *to,
                           const int *ckpt, const time_t *start,
                           const double *secs, const double *bytes)
{
#ifdef HAVE_LIBMYSQLCLIENT
    /* lookup the id for the type string */
    int type_id = -1;
    ...
    /* check that we got valid strings for each of our parameters */
    if (qfrom == NULL || qto == NULL || qckpt == NULL || qstart == NULL ||
        qend == NULL || qsecs == NULL || qbytes == NULL || qbw == NULL) {
        scr_err("Failed to escape and quote one or more arguments @ %s:%d",
                __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    ...
    int n = snprintf(query, sizeof(query),
                     "INSERT"
                     " INTO `transfers`"
                     " (`id`,`job_id`,`type_id`,`checkpoint_id`,`start`,`end`,`"
                     "time`,`bytes`,`bw`,`from`,`to`)"
                     " VALUES"
                     " (NULL, %lu, %d, %s, %s, %s, %s, %s, %s, %s, %s)"
                     " ;",
                     scr_db_jobid, type_id, qckpt, qstart, qend, qsecs, qbytes,
                     qbw, qfrom, qto);
...
int scr_mysql_read_job(unsigned long username_id, unsigned long jobname_id,
                       unsigned long *id)
{
#ifdef HAVE_LIBMYSQLCLIENT
    ...
    char query[1024];
    int n = snprintf(query, sizeof(query),
                     "SELECT * FROM `jobs` WHERE `username_id` = '%lu' AND "
                     "`jobname_id` = '%lu' ;",
...
int scr_mysql_register_job(const char *username, const char *jobname,
                           unsigned long start, unsigned long *jobid)
{
    int rc = SCR_SUCCESS;
    ...
    if (rc != SCR_SUCCESS) {
        scr_err("Failed to find username_id for %s @ %s:%d", username, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
    ...
    if (rc != SCR_SUCCESS) {
        scr_err("Failed to find jobname_id for %s @ %s:%d", jobname, __FILE__,
                __LINE__);
        return SCR_FAILURE;
    }
...
int scr_log_init()
{
    ...
    if (scr_db_enable) {
        if (scr_mysql_connect() != SCR_SUCCESS) {
            scr_err("Failed to connect to SCR logging database, disabling "
                    "database logging @ %s:%d",
                    __FILE__, __LINE__);
            scr_db_enable = 0;
            rc = SCR_FAILURE;
        }
...
/* given a username, a jobname, and a start time, lookup (or create) the id for
 * this job */
int scr_log_job(const char *username, const char *jobname, time_t start)
{
    ...
    if (scr_db_enable) {
        if (username != NULL && jobname != NULL) {
            int rc =
                scr_mysql_register_job(username, jobname, start, &scr_db_jobid);
            if (rc != SCR_SUCCESS) {
                scr_err("Failed to register job for username %s and jobname "
                        "%s, disabling database logging @ %s:%d",
            ...
            scr_err("Failed to read username or jobname from environment, "
                    "disabling database logging @ %s:%d",
                    __FILE__, __LINE__);
            scr_db_enable = 0;
            rc = SCR_FAILURE;
        }
...
/* log an event */
int scr_log_event(const char *type, const char *note, const int *ckpt,
                  const time_t *start, const double *secs)
{
    int rc = SCR_SUCCESS;
    if (scr_db_enable) {
...
int scr_log_transfer(const char *type, const char *from, const char *to,
                     const int *ckpt_id, const time_t *start,
                     const double *secs, const double *bytes)
{
    int rc = SCR_SUCCESS;
    if (scr_db_enable) {
        rc =
            scr_mysql_log_transfer(type, from, to, ckpt_id, start, secs, bytes);
    }
    return rc;
}
