...
char *scr_param_get(char *name)
{
    ...
    if (no_user == NULL && getenv(name) != NULL) {
        ...
        if (scr_hash_util_get_str(scr_env_hash, name, &value) != SCR_SUCCESS) {
            ...
            if (scr_hash_util_get_str(scr_env_hash, name, &value) !=
                SCR_SUCCESS) {
...
int scr_param_init()
{
    ...
    if (scr_param_ref_count == 0) {
        ...
        scr_hash_elem *elem;
        for (elem = scr_hash_elem_first(scr_no_user_hash); elem != NULL;
            ...
            if (env_val != NULL || env_hash != NULL) {
                scr_err("%s cannot be set in the environment or user "
                        "configuration file, ignoring setting",
                        key);
            }
        }
    }
