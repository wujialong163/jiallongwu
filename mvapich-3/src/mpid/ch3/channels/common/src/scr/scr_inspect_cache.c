...
static int scr_bool_have_file(const scr_filemap *map, int dset, int rank,
                              const char *file)
{
...
int main(int argc, char *argv[])
{
    ...
    for (dset_elem = scr_filemap_first_dataset(map); dset_elem != NULL;
        ...
        for (rank_elem = scr_filemap_first_rank_by_dataset(map, dset);
            ...
            if (expected == num) {
                /* first time through the file list, check that we have each
                 * file */
            ...
            if (!missing_file) {
                ...
                char *type =
                    scr_hash_elem_get_first_val(desc, SCR_CONFIG_KEY_TYPE);
                char *groups_str =
                    scr_hash_elem_get_first_val(desc, SCR_CONFIG_KEY_GROUPS);
                char *group_id_str =
                    scr_hash_elem_get_first_val(desc, SCR_CONFIG_KEY_GROUP_ID);
                char *group_size_str = scr_hash_elem_get_first_val(
                    desc, SCR_CONFIG_KEY_GROUP_SIZE);
                char *group_rank_str = scr_hash_elem_get_first_val(
                    desc, SCR_CONFIG_KEY_GROUP_RANK);
                if (type != NULL && groups_str != NULL &&
                    group_id_str != NULL && group_size_str != NULL &&
                    group_rank_str != NULL) {
                    /* we already have a group id and rank, use that to rebuild
                     * the communicator */
                    int groups = atoi(groups_str);
                    int group_id = atoi(group_id_str);
                    int group_size = atoi(group_size_str);
                    int group_rank = atoi(group_rank_str);
                    printf("DSET=%d RANK=%d TYPE=%s GROUPS=%d GROUP_ID=%d "
                           "GROUP_SIZE=%d GROUP_RANK=%d FILES=1\n",
                           dset, rank, type, groups, group_id, group_size,
                           group_rank);
                }
            }
        }
