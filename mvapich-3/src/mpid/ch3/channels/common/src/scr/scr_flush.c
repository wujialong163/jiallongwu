...
int scr_bool_flush_file(const scr_filemap *map, int dset, int rank,
                        const char *file)
{
...
int scr_flush_pick_writer(int level, unsigned long count, int *outwriter,
                          int *outranks)
{
...
static int scr_flush_identify_files(const scr_filemap *map, int id,
                                    scr_hash *file_list)
{
    int rc = SCR_SUCCESS;
    ...
    for (elem = scr_filemap_first_file(map, id, scr_my_rank_world);
        ...
        scr_meta *meta = scr_meta_new();
        if (scr_filemap_get_meta(map, id, scr_my_rank_world, file, meta) ==
            SCR_SUCCESS) {
            /* don't flush XOR files */
            int flush = 1;
            ...
            /* if we need to flush this file, add it to the list and attach its
             * meta data */
            if (flush) {
                scr_hash *file_hash =
                    scr_hash_set_kv(file_list, SCR_KEY_FILE, file);
    ...
    size_t subdir_size) /* IN  - size of subdir buffer */
{
    ...
    if (child_components <= parent_components) {
        scr_abort(
            -1, "Directory %s must have more components than parent %s @ %s:%d",
            dir, parent, __FILE__, __LINE__);
    }

    /* ensure that parent is prefix of child path */
    if (!scr_path_is_child(parent, child)) {
        scr_abort(-1, "Directory %s not contained in parent %s @ %s:%d", dir,
    ...
    if (scr_path_is_null(subdir_path)) {
        scr_abort(-1, "Failed to get subdirectory from %s @ %s:%d", dir,
                  __FILE__, __LINE__);
    }
...
static int scr_flush_identify_dirs(scr_hash *file_list)
{
    ...
    if (scr_preserve_directories) {
        ...
        int i = 0;
        scr_hash_elem *elem = NULL;
        for (elem = scr_hash_elem_first(files); elem != NULL;
            ...
            if (scr_meta_get_origpath(meta, &dir) == SCR_SUCCESS) {
                ...
                if (scr_find_subdir(scr_prefix, dir, subdir, sizeof(subdir)) ==
                    SCR_SUCCESS) {
                ...
                scr_abort(
                    -1, "Failed to read original path name for a file @ %s:%d",
                    __FILE__, __LINE__);
            }
        ...
        if (!scr_alltrue(valid_subdir)) {
            if (scr_my_rank_world == 0) {
                scr_abort(-1,
                          "One or more processes found an invalid subdirectory "
                          "@ %s:%d",
                          __FILE__, __LINE__);
            }
        }
        ...
        uint64_t groups;
        int dtcmp_rc =
            DTCMP_Rankv_strings(count, subdirs, &groups, group_id, group_ranks,
                                group_rank, DTCMP_FLAG_NONE, scr_comm_world);
        ...
        if (groups != 1) {
            if (scr_my_rank_world == 0) {
                scr_abort(-1,
                          "Identified more than one subdirectory during flush "
                          "@ %s:%d",
                          __FILE__, __LINE__);
            }
        }
        ...
        dtcmp_rc =
            DTCMP_Rankv_strings(count, dirs, &groups, group_id, group_ranks,
                                group_rank, DTCMP_FLAG_NONE, scr_comm_world);
        if (dtcmp_rc != DTCMP_SUCCESS) {
            scr_abort(-1, "Failed to rank strings during flush @ %s:%d",
                      __FILE__, __LINE__);
        ...
        /* TODO: PRESERVE need to track directory names in summary file so we
        ...
        if (scr_dataset_get_name(dataset, &name) != SCR_SUCCESS) {
            scr_abort(-1, "Failed to get dataset name @ %s:%d", __FILE__,
                      __LINE__);
        }
...
static int scr_container_construct_name(const scr_dataset *dataset, int id,
                                        char *file, int len)
{
...
static int scr_flush_identify_containers(scr_hash *file_list)
{
    ...
    MPI_Allreduce(&my_bytes, &total_bytes, 1, MPI_UNSIGNED_LONG, MPI_SUM,
                  scr_comm_world);

    /* compute total number of bytes we need to write on the node */
    unsigned long local_bytes;
    MPI_Reduce(&my_bytes, &local_bytes, 1, MPI_UNSIGNED_LONG, MPI_SUM, 0,
               scr_comm_node);

    /* compute offset for each node */
    unsigned long local_offset = 0;
    if (rank_node == 0) {
        MPI_Scan(&local_bytes, &local_offset, 1, MPI_UNSIGNED_LONG, MPI_SUM,
                 scr_comm_node_across);
        local_offset -= local_bytes;
    }

    /* compute offset for each process,
     * note that local_offset == 0 for all procs on the node except for rank ==
     * 0, which contains the offset for the node */
    unsigned long my_offset = 0;
    local_offset += my_bytes;
    MPI_Scan(&local_offset, &my_offset, 1, MPI_UNSIGNED_LONG, MPI_SUM,
             scr_comm_node);
    my_offset -= my_bytes;
        ...
        if (scr_meta_get_filesize(meta, &filesize) == SCR_SUCCESS) {
            ...
            while (remaining > 0) {
                ...
                unsigned long container_offset =
                    file_offset - (container_id * container_size);
                ...
                /* store segment length, container id, and container offset
                 * under new file segment */
                scr_hash *segment_hash = scr_hash_set_kv_int(
                    hash, SCR_SUMMARY_6_KEY_SEGMENT, file_segment);
                scr_hash_util_set_bytecount(
                    segment_hash, SCR_SUMMARY_6_KEY_LENGTH, container_length);
                scr_hash *container_hash = scr_hash_new();
                scr_hash_util_set_int(container_hash, SCR_SUMMARY_6_KEY_ID,
                                      container_id);
                scr_hash_util_set_bytecount(
                    container_hash, SCR_SUMMARY_6_KEY_OFFSET, container_offset);
                scr_hash_set(segment_hash, SCR_SUMMARY_6_KEY_CONTAINER,
                             container_hash);

                /* add entry for container name in the file list */
                scr_hash *details = scr_hash_set_kv_int(
                    file_list, SCR_SUMMARY_6_KEY_CONTAINER, container_id);

                /* compute name of container */
                char container_name[SCR_MAX_FILENAME];
                scr_container_construct_name(dataset, container_id,
                                             container_name,
                ...
                scr_hash_util_set_bytecount(details, SCR_SUMMARY_6_KEY_SIZE,
                                            size);
...
static int scr_flush_identify(const scr_filemap *map, int id,
                              scr_hash *file_list)
{
    /* check that we have all of our files */
    int have_files = 1;
    if (scr_cache_check_files(map, id) != SCR_SUCCESS) {
        scr_err("Missing one or more files for dataset %d @ %s:%d", id,
                __FILE__, __LINE__);
        have_files = 0;
    }
    if (!scr_alltrue(have_files)) {
        if (scr_my_rank_world == 0) {
            scr_err("One or more processes are missing files for dataset %d @ "
                    "%s:%d",
    ...
    if (scr_flush_identify_files(map, id, file_list) != SCR_SUCCESS) {
        scr_abort(-1, "Failed to get list of files for dataset %d @ %s:%d", id,
                  __FILE__, __LINE__);
    }
    ...
    if (scr_flush_identify_dirs(file_list) != SCR_SUCCESS) {
        scr_abort(-1,
                  "Failed to get list of directories for dataset %d @ %s:%d",
                  id, __FILE__, __LINE__);
    }
    ...
    if (scr_use_containers) {
        if (scr_flush_identify_containers(file_list) != SCR_SUCCESS) {
            scr_abort(-1,
                      "Failed to identify containers for dataset %d @ %s:%d",
                      id, __FILE__, __LINE__);
        }
    }
...
static int scr_flush_create_dirs(scr_hash *file_list)
{
    ...
    if (scr_my_rank_world == 0) {
        /* get top level path */
        char *flushdir;
        if (scr_hash_util_get_str(file_list, SCR_KEY_PATH, &flushdir) !=
            SCR_SUCCESS) {
            scr_abort(-1, "Failed to get flush directory @ %s:%d", __FILE__,
                      __LINE__);
        }
        ...
        if (scr_dataset_get_id(dataset, &id) != SCR_SUCCESS) {
            scr_abort(-1, "Failed to get dataset id @ %s:%d", __FILE__,
                      __LINE__);
        }
        ...
        if (scr_mkdir(dir_scr, mode_dir) != SCR_SUCCESS) {
            ...
            scr_abort(
                -1,
                "Failed to make .scr subdirectory directory mkdir(%s) @ %s:%d",
    ...
    for (elem = scr_hash_elem_first(dirs); elem != NULL;
         elem = scr_hash_elem_next(elem)) {
        /* create directory */
    ...
    /* TODO: PRESERVE need to track directory names in summary file so we can
     * delete them later */
...
static int scr_flush_create_containers(const scr_hash *file_list)
{
     ...
     * and the process which writes data to offset 0 is responsible for creating
     * the container */
    ...
    scr_hash *files = scr_hash_get(file_list, SCR_KEY_FILE);
    for (file_elem = scr_hash_elem_first(files); file_elem != NULL;
        ...
        for (segment_elem = scr_hash_elem_first(segments); segment_elem != NULL;
            ...
            if (scr_container_get_name_size_offset_length(
                    segment, containers, &name, &size, &offset, &length) ==
                SCR_SUCCESS) {
                /* if we write something to offset 0 of this container,
                 * we are responsible for creating the file */
                if (offset == 0 && length > 0) {
                    /* open the file with create and truncate options, then just
                     * immediately close it */
                    int fd =
                        scr_open(name, O_WRONLY | O_CREAT | O_TRUNC, mode_file);
                    if (fd < 0) {
                        /* the create failed */
                        scr_err("Opening file for writing: scr_open(%s) "
                                "errno=%d %s @ %s:%d",
                                name, errno, strerror(errno), __FILE__,
                                __LINE__);
                        success = SCR_FAILURE;
                    } else {
                ...
                /* failed to read container details from segment hash, consider
                 * this an error */
                success = SCR_FAILURE;
            }
        }
...
int scr_flush_verify(const scr_filemap *map, int id, char *dir, size_t dir_size)
{
    ...
    char *flush_dir;
    if (scr_hash_util_get_str(file_list, SCR_KEY_PATH, &flush_dir) !=
        SCR_SUCCESS) {
        scr_abort(-1, "Failed to get flush directory of dataset %d @ %s:%d", id,
                  __FILE__, __LINE__);
    }
...
/* given a filemap and a dataset id, prepare and return a list of files to be
 * flushed, also create corresponding directories and container files */
...
static unsigned long scr_flush_summary_map(const scr_path *dataset_path,
                                           const scr_path *file,
                                           unsigned long offset, int level,
                                           int valid)
{
    int rc = SCR_SUCCESS;
    ...
    scr_hash_exchange_direction(send, recv, scr_comm_world,
    ...
    if (ranks < scr_ranks_world) {
        ...
        scr_path_append_strf(rank2file_path, "rank2file.%d.%d.scr", level,
                             writer);
    } else {
    ...
    if (ranks < scr_ranks_world) {
        ...
        if (scr_flush_summary_map(dataset_path, rank2file_path, newoffset,
                                  newlevel, newvalid) != SCR_SUCCESS) {
            rc = SCR_FAILURE;
        }
    }
    ...
    if (scr_my_rank_world == writer) {
        ...
        if (fd >= 0) {
            ...
            scr_err("Opening file for write: %s @ %s:%d", rank2file_file,
                    __FILE__, __LINE__);
            rc = SCR_FAILURE;
        }
    }
...
static int scr_flush_summary(const char *dataset_dir,
                             const scr_dataset *dataset,
                             const scr_hash *file_list, scr_hash *data)
{
    int rc = SCR_SUCCESS;
    ...
    if (scr_my_rank_world == 0) {
        ...
        if (fd < 0) {
            scr_err("Opening hash file for write: %s @ %s:%d", summary_file,
                    __FILE__, __LINE__);
            rc = SCR_FAILURE;
        }
        ...
        if (fd >= 0) {
            ...
            scr_hash_util_set_int(summary_hash, SCR_SUMMARY_KEY_VERSION,
                                  SCR_SUMMARY_FILE_VERSION_6);

            /* mark whether the flush is complete in the summary file */
            scr_hash_util_set_int(summary_hash, SCR_SUMMARY_6_KEY_COMPLETE,
                                  all_complete);
            ...
            ssize_t write_rc =
                scr_hash_write_fd(summary_file, fd, summary_hash);
            if (write_rc < 0) {
                rc = SCR_FAILURE;
            }
    ...
    if (scr_flush_summary_map(dataset_path, rank2file_path, offset, level,
                              valid) != SCR_SUCCESS) {
        rc = SCR_FAILURE;
    }
...
/* given a dataset id that has been flushed, the list provided by
 * scr_flush_prepare, and data to include in the summary file, complete the
 * flush by writing the summary file */
int scr_flush_complete(int id, scr_hash *file_list, scr_hash *data)
{
    ...
    char *summary_dir;
    if (scr_hash_util_get_str(file_list, SCR_KEY_PATH, &summary_dir) !=
        SCR_SUCCESS) {
        return SCR_FAILURE;
    }

    /* write summary file */
    if (scr_flush_summary(summary_dir, dataset, file_list, data) !=
        SCR_SUCCESS) {
        flushed = SCR_FAILURE;
    }
    ...
    if (scr_my_rank_world == 0) {
        ...
        if (flushed == SCR_SUCCESS) {
            ...
            char subdir[SCR_MAX_FILENAME];
            if (scr_find_subdir(scr_prefix, summary_dir, subdir,
                                sizeof(subdir)) == SCR_SUCCESS) {
            ...
            int id;
            if (scr_dataset_get_id(dataset, &id) == SCR_SUCCESS) {
                scr_index_set_dataset(index_hash, id, subdir, dataset,
                                      complete);
            } else {
                scr_abort(-1, "Failed to read dataset id @ %s:%d", __FILE__,
                          __LINE__);
            }
