...
scr_meta *scr_meta_new()
{
    ...
    if (meta == NULL) {
        scr_err("Failed to allocate meta data object @ %s:%d", __FILE__,
                __LINE__);
    }
    return meta;
}
...
/* sets complete value in meta data, overwrites any existing value with new
 * value */
...
/* gets original filename recorded in meta data, returns SCR_SUCCESS if
 * successful */
...
/* gets full path to the original filename recorded in meta data, returns
 * SCR_SUCCESS if successful */
...
/* gets the name of the original filename recorded in meta data, returns
 * SCR_SUCCESS if successful */
...
{
    int complete = 0;
    if (scr_hash_util_get_int(meta, SCR_META_KEY_COMPLETE, &complete) ==
        SCR_SUCCESS) {
        if (complete == 1) {
            return SCR_SUCCESS;
        }
...
/* return SCR_SUCCESS if rank is set in meta data, and if it matches the
 * specified value */
int scr_meta_check_rank(const scr_meta *meta, int rank)
{
    int rank_meta;
    if (scr_hash_util_get_int(meta, SCR_META_KEY_RANK, &rank_meta) ==
        SCR_SUCCESS) {
        if (rank == rank_meta) {
            return SCR_SUCCESS;
        }
...
/* return SCR_SUCCESS if ranks is set in meta data, and if it matches the
 * specified value */
int scr_meta_check_ranks(const scr_meta *meta, int ranks)
{
    int ranks_meta;
    if (scr_hash_util_get_int(meta, SCR_META_KEY_RANKS, &ranks_meta) ==
        SCR_SUCCESS) {
        if (ranks == ranks_meta) {
            return SCR_SUCCESS;
        }
...
/* return SCR_SUCCESS if checkpoint_id is set in meta data, and if it matches
 * the specified value */
int scr_meta_check_checkpoint(const scr_meta *meta, int ckpt)
{
    int ckpt_meta;
    if (scr_hash_util_get_int(meta, SCR_META_KEY_CKPT, &ckpt_meta) ==
        SCR_SUCCESS) {
        if (ckpt == ckpt_meta) {
            return SCR_SUCCESS;
        }
...
/* return SCR_SUCCESS if filename is set in meta data, and if it matches the
 * specified value */
...
/* return SCR_SUCCESS if filetype is set in meta data, and if it matches the
 * specified value */
...
/* returns SCR_SUCCESS if filesize is set in meta data, and if it matches
 * specified value */
int scr_meta_check_filesize(const scr_meta *meta, unsigned long filesize)
{
    unsigned long filesize_meta = 0;
    if (scr_hash_util_get_bytecount(meta, SCR_META_KEY_SIZE, &filesize_meta) ==
        SCR_SUCCESS) {
