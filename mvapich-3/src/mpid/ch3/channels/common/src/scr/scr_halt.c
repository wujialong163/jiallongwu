...
int scr_halt_read(const scr_path *path_file, scr_hash *hash)
{
    ...
    if (fd < 0) {
        scr_err("Opening file for read: scr_open(%s) errno=%d %s @ %s:%d", file,
...
/* read in halt file (which user may have changed via scr_halt), update internal
 * data structure, optionally decrement the checkpoints_left field, and write
 * out halt file all while locked */
int scr_halt_sync_and_decrement(const scr_path *file_path, scr_hash *hash,
                                int dec_count)
{
    /* assume we'll fail */
    int rc = SCR_FAILURE;
    ...
    /* if the file already existed before we opened it, override our current
     * settings with its values */
    if (exists) {
        /* for the exit reason, only override our current value if the file has
         * a setting but we don't,
         * otherwise the running program could never set this value */
        /* if we have an exit reason set, but the file doesn't, make a copy
         * before we unset out hash */
        char *save_reason = NULL;
        char *reason =
            scr_hash_elem_get_first_val(hash, SCR_HALT_KEY_EXIT_REASON);
        char *file_reason =
            scr_hash_elem_get_first_val(file_hash, SCR_HALT_KEY_EXIT_REASON);
    ...
    char *ckpts_str =
        scr_hash_elem_get_first_val(hash, SCR_HALT_KEY_CHECKPOINTS);
    if (ckpts_str != NULL) {
