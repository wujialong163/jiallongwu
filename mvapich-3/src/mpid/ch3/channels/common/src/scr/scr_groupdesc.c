...
int scr_groupdesc_init(scr_groupdesc *d)
{
    ...
    if (d == NULL) {
        scr_err("No group descriptor @ %s:%d", __FILE__, __LINE__);
        return SCR_FAILURE;
    }
...
static int scr_split_by_string(const char *str, MPI_Comm *comm)
{
    ...
    int dtcmp_rc =
        DTCMP_Rank_strings(1, &str, &groups, &group_id, &group_ranks,
                           &group_rank, DTCMP_FLAG_NONE, scr_comm_world);
    if (dtcmp_rc != DTCMP_SUCCESS) {
        scr_abort(-1, "Failed to rank strings @ %s:%d", __FILE__, __LINE__);
    }
...
int scr_groupdesc_create_by_str(scr_groupdesc *d, int index, const char *key,
                                const char *value)
{
    /* initialize the descriptor */
...
int scr_groupdescs_create()
{
    ...
    MPI_Allreduce(&count, &scr_ngroupdescs, 1, MPI_INT, MPI_MAX,
                  scr_comm_world);

    /* allocate our group descriptors */
    if (scr_ngroupdescs > 0) {
        scr_groupdescs =
            (scr_groupdesc *)malloc(scr_ngroupdescs * sizeof(scr_groupdesc));
    ...
    int index = 0;
    scr_groupdesc_create_by_str(&scr_groupdescs[index], index, SCR_GROUP_NODE,
                                scr_my_hostname);
    index++;

    /* create group descriptor for all procs in job */
    scr_groupdesc_create_by_str(&scr_groupdescs[index], index, SCR_GROUP_WORLD,
                                "ALL");
    index++;
    ...
    if (scr_my_rank_world == 0) {
        scr_hash_elem *elem;
        for (elem = scr_hash_elem_first(groups); elem != NULL;
            ...
            if (scr_alltrue(have_match)) {
                /* create group */
                scr_groupdesc_create_by_str(&scr_groupdescs[index], index, key,
                                            value);
                index++;
            } else {
                /* TODO: print error */
