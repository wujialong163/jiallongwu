...
#ifndef SCR_MACHINE_TYPE
#define SCR_MACHINE_TYPE 0
#endif
...
{
    /* returning a negative number tells the caller this functionality is
     * disabled */
    long int secs = -1;
...
char *scr_env_nodename()
{
    ...
    if (gethostname(hostname, sizeof(hostname)) == 0) {
        ...
        scr_err("Call to gethostname failed @ %s:%d", __FILE__, __LINE__);
    }
    ...
    bg_decodeComputeCardCoreOnNodeBoardUCI(personality.Kernel_Config.UCI, &x,
                                           &y, &m, &n, &j, &c);
    ...
    int num = snprintf(
        ...
        "R%X%X-M%d-N%02d-J%02d-A%dof%d-B%dof%d-C%dof%d-D%dof%d-E%dof%d", x, y,
        m, n, j, personality.Network_Config.Acoord,
        personality.Network_Config.Anodes, personality.Network_Config.Bcoord,
        personality.Network_Config.Bnodes, personality.Network_Config.Ccoord,
        personality.Network_Config.Cnodes, personality.Network_Config.Dcoord,
        personality.Network_Config.Dnodes, personality.Network_Config.Ecoord,
    ...
    if (num < 0) {
        ...
        /* name was truncated */
        scr_err("Temporary buffer of %d bytes too small to construct hostname, "
                "needed %d bytes @ %s:%d",
