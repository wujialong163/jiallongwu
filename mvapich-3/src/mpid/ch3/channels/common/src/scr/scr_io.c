...
int scr_open(const char *file, int flags, ...)
{
    ...
    if (fd < 0) {
        scr_dbg(1, "Opening file: open(%s) errno=%d %s @ %s:%d", file, errno,
        ...
        if (fd < 0) {
            scr_err("Opening file: open(%s) errno=%d %s @ %s:%d", file, errno,
                    strerror(errno), __FILE__, __LINE__);
        }
    }
    return fd;
...
int scr_close(const char *file, int fd)
{
    ...
    if (fsync(fd) < 0) {
        ...
        scr_dbg(2,
                "Failed to fsync file descriptor: %s errno=%d %s @ file %s:%d",
...
int scr_file_lock_read(const char *file, int fd)
{
    ...
    if (flock(fd, LOCK_SH) != 0) {
        scr_err("Failed to acquire file lock on %s: flock(%d, %d) errno=%d %s "
                "@ %s:%d",
    ...
    if (fcntl(fd, F_SETLK, &lck) < 0) {
        scr_err("Failed to acquire file read lock on %s: fnctl(%d, %d) "
                "errno=%d %s @ %s:%d",
...
int scr_lseek(const char *file, int fd, off_t pos, int whence)
{
    ...
    if (rc == (off_t)-1) {
        scr_err("Error seeking %s: errno=%d %s @ %s:%d", file, errno,
                strerror(errno), __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    return SCR_SUCCESS;
    ...
    while (n < size) {
        ...
        if (rc > 0) {
            ...
            if (retries) {
                ...
                scr_err(
                    "Error reading %s: read(%d, %x, %ld) errno=%d %s @ %s:%d",
                    file, fd, (char *)buf + n, size - n, errno, strerror(errno),
                    __FILE__, __LINE__);
            } else {
                /* too many failed retries, give up */
                scr_err("Giving up read of %s: read(%d, %x, %ld) errno=%d %s @ "
                        "%s:%d",
                        file, fd, (char *)buf + n, size - n, errno,
                        strerror(errno), __FILE__, __LINE__);
                exit(1);
            }
        }
...
/* reliable write to opened file descriptor (retries, if necessary, until hard
 * error) */
    ...
    while (n < size) {
        ...
        if (rc > 0) {
            ...
            if (retries) {
                /* print an error and try again */
                scr_err(
                    "Error writing %s: write(%d, %x, %ld) errno=%d %s @ %s:%d",
                    file, fd, (char *)buf + n, size - n, errno, strerror(errno),
                    __FILE__, __LINE__);
            } else {
                /* too many failed retries, give up */
                scr_err("Giving up write to %s: write(%d, %x, %ld) errno=%d %s "
                        "@ %s:%d",
                        file, fd, (char *)buf + n, size - n, errno,
                        strerror(errno), __FILE__, __LINE__);
                exit(1);
...
/* make a good attempt to read from file (retries, if necessary, return error if
 * fail) */
ssize_t scr_read_attempt(const char *file, int fd, void *buf, size_t size)
{
    ...
    while (n < size) {
        ...
        if (rc > 0) {
            ...
            if (retries) {
                /* print an error and try again */
                scr_err("Error reading file %s errno=%d %s @ %s:%d", file,
                        errno, strerror(errno), __FILE__, __LINE__);
            } else {
                /* too many failed retries, give up */
                scr_err("Giving up read on file %s errno=%d %s @ %s:%d", file,
                        errno, strerror(errno), __FILE__, __LINE__);
                return -1;
            }
        }
...
/* make a good attempt to write to file (retries, if necessary, return error if
 * fail) */
ssize_t scr_write_attempt(const char *file, int fd, const void *buf,
                          size_t size)
{
    ssize_t n = 0;
    int retries = 10;
    while (n < size) {
        ...
        if (rc > 0) {
            ...
            /* something bad happened, print an error and abort */
            scr_err("Error writing file %s write returned 0 @ %s:%d", file,
                    __FILE__, __LINE__);
            return -1;
        } else { /* (rc < 0) */
            /* got an error, check whether it was serious */
            ...
            if (retries) {
                /* print an error and try again */
                scr_err("Error writing file %s errno=%d %s @ %s:%d", file,
                        errno, strerror(errno), __FILE__, __LINE__);
            } else {
                /* too many failed retries, give up */
                scr_err("Giving up write of file %s errno=%d %s @ %s:%d", file,
                        errno, strerror(errno), __FILE__, __LINE__);
                return -1;
            }
        }
...
ssize_t scr_read_line(const char *file, int fd, char *buf, size_t size)
{
    ...
    while (n < size - 1 && !found_end) {
        ...
        if (nread > 0) {
            ...
            /* we hit an error */
            scr_err("Error reading from file %s @ %s:%d", file, __FILE__,
                    __LINE__);
            return -1;
        }
    }
    ...
    /* if we exit the while loop but didn't find the end of the line, the buffer
     * was too small */
    if (!found_end) {
        scr_err("Buffer too small to read line from file %s @ %s:%d", file,
                __FILE__, __LINE__);
        return -1;
    }

    /* NOTE: we don't want to count the NULL which we added, but there is no
     * need to adjust n here */
    return n;
}
...
ssize_t scr_writef(const char *file, int fd, const char *format, ...)
{
    ...
    if (sizeof(buf) <= n) {
        /* TODO: instead of throwing a fatal error, we could allocate a bigger
...
/* logically concatenate n opened files and read count bytes from this logical
 * file into buf starting from offset, pad with zero on end if missing data */
int scr_read_pad_n(int n, char **files, int *fds, char *buf,
                   ...
                   unsigned long *filesizes)
{
    ...
    if (i < n) {
        ...
        if (lseek(fds[i], pos, SEEK_SET) == (off_t)-1) {
            ...
            scr_err("Failed to seek to byte %lu in %s @ %s:%d", pos, files[i],
                    __FILE__, __LINE__);
            return SCR_FAILURE;
        }
    }
    ...
    while (nread < count && i < n) {
        ...
        /* if we don't need to read the whole remainder of the file, adjust to
         * the smaller amount */
        ...
        if (scr_read_attempt(files[i], fds[i], buf + nread, num_to_read) !=
            num_to_read) {
...
/* write to an array of open files with known filesizes treating them as one
 * single large file */
int scr_write_pad_n(int n, char **files, int *fds, char *buf,
                    unsigned long count, unsigned long offset,
                    unsigned long *filesizes)
{
    int i = 0;
    ...
    while (nwrite < count && i < n) {
        ...
        /* if we don't need to write the whole remainder of the file, adjust to
         * the smaller amount */
        ...
        if (scr_write_attempt(files[i], fds[i], buf + nwrite, num_to_write) !=
            num_to_write) {
...
{
    if (unlink(file) != 0) {
        scr_dbg(2, "Failed to delete file: %s errno=%d %s @ file %s:%d", file,
                errno, strerror(errno), __FILE__, __LINE__);
        return SCR_FAILURE;
    }
    return SCR_SUCCESS;
...
int scr_crc32(const char *filename, uLong *crc)
{
    ...
    *crc = crc32(0L, Z_NULL, 0);
    ...
    if (fd < 0) {
        scr_dbg(1,
                "Failed to open file to compute crc: %s errno=%d @ file %s:%d",
...
int scr_mkdir(const char *dir, mode_t mode)
{
    ...
    /* With dirname, either the original string may be modified or the function
     * may return a pointer to static storage which will be overwritten by the
     * next call to dirname, so we need to strdup both the argument and the
     * return string. */
    ...
    /* if we can read path or path=="." or path=="/", then there's nothing to
     * do, otherwise, try to create it */
    if (access(path, R_OK) < 0 && strcmp(path, ".") != 0 &&
        strcmp(path, "/") != 0) {
    ...
    if (access(path, W_OK) == 0 && rc == SCR_SUCCESS) {
        ...
        if (tmp_rc < 0) {
            if (errno == EEXIST) {
                /* don't complain about mkdir for a directory that already
                 * exists */
                scr_free(&dircopy);
                scr_free(&path);
                return SCR_SUCCESS;
            } else {
                scr_err("Creating directory: mkdir(%s, %x) path=%s errno=%d %s "
                        "@ %s:%d",
                        dir, mode, path, errno, strerror(errno), __FILE__,
                        __LINE__);
                rc = SCR_FAILURE;
            }
        }
    } else {
        scr_err("Cannot write to directory: %s @ %s:%d", path, __FILE__,
                __LINE__);
        rc = SCR_FAILURE;
    }
...
int scr_getcwd(char *buf, size_t size)
{
    ...
    if (getcwd(buf, size) == NULL) {
        scr_abort(-1,
                  "Problem reading current working directory (getcwd() "
                  "errno=%d %s) @ %s:%d",
                  errno, strerror(errno), __FILE__, __LINE__);
        rc = SCR_FAILURE;
    }
 ...
 */
int scr_file_copy(const char *src_file, const char *dst_file,
                  unsigned long buf_size, uLong *crc)
{
    ...
    if (dst_file == NULL || strcmp(dst_file, "") == 0) {
        scr_err("Invalid destination file @ %s:%d", __FILE__, __LINE__);
        return SCR_FAILURE;
    }
