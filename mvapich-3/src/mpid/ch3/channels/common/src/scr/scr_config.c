 ...
 */
static int scr_config_read_whitespace(FILE *fs, const char *file, int linenum,
...
static int scr_config_read_token(FILE *fs, const char *file, int linenum,
                                 int *n_external, char *c_external, char *token,
                                 int size)
{
    /* remove whitespace (spaces and tabs) until we hit a character */
    int n = *n_external;
    ...
    while (n != EOF && c != ' ' && c != '\t' && c != '\n' && c != '=') {
        ...
        if (i >= size) {
            scr_err("Internal buffer too short (%d bytes) while reading token "
                    "in configuration file @ %s:%d",
                    size, file, linenum);
            return SCR_FAILURE;
        }
    ...
    if (i == 0) {
        scr_err("Missing token in configuration file @ %s:%d", file, linenum);
        return SCR_FAILURE;
    }

    /* terminate our token with a null character */
    if (i >= size) {
        scr_err("Internal buffer too short (%d bytes) while reading token in "
                              ...
                              scr_hash **hash2)
{
    ...
    if (c != '=') {
        scr_err(
            "Ill-formed key value pair detected in configuration file @ %s:%d",
            file, linenum);
        return SCR_FAILURE;
    }
    n = fgetc(fs);
 ...
 */
static int scr_config_read_comment(FILE *fs, const char *file, int linenum,
                                   int *n_external, char *c_external)
{
...
static int scr_config_read_line(FILE *fs, const char *file, int linenum,
                                int *n_external, char *c_external,
                                scr_hash *hash)
{
    int n = *n_external;
    ...
    while (n != EOF && c != '\n') {
        ...
        /* if the line didn't end, we must have found a comment or another
         * key/value pair */
        if (n != EOF && c != '\n') {
            if (c == '#') {
                ...
                scr_hash *tmp_hash;
                scr_config_read_kv(fs, file, linenum, &n, &c, target_hash,
                                   &tmp_hash);
...
int scr_config_read_common(const char *file, scr_hash *hash)
{
    ...
    if (fs == NULL) {
        scr_err("Opening configuration file for read: fopen(%s, \"r\") "
                "errno=%d %s @ %s:%d",
