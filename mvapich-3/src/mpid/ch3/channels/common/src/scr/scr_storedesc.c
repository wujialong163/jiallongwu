...
int scr_storedesc_init(scr_storedesc *s)
{
    ...
    if (s == NULL) {
        scr_err("No store descriptor @ %s:%d", __FILE__, __LINE__);
        return SCR_FAILURE;
    }
...
static int scr_storedesc_copy(scr_storedesc *out, const scr_storedesc *in)
{
    ...
    if (out == NULL || in == NULL) {
        scr_err("Invalid store descriptors @ %s:%d", __FILE__, __LINE__);
        return SCR_FAILURE;
    }
...
int scr_storedesc_create_from_hash(scr_storedesc *s, int index,
                                   const scr_hash *hash)
{
    int rc = SCR_SUCCESS;
    ...
    if (s == NULL) {
        scr_err("No store descriptor to fill from hash @ %s:%d", __FILE__,
                __LINE__);
        rc = SCR_FAILURE;
    }
    ...
    if (scr_hash_util_get_str(hash, SCR_CONFIG_KEY_GROUP, &group) ==
        SCR_SUCCESS) {
...
int scr_storedesc_dir_delete(const scr_storedesc *store, const char *dir)
{
    ...
    if (store->rank == 0 && store->can_mkdir) {
        ...
        if (scr_rmdir(dir) != SCR_SUCCESS) {
            ...
            rc = SCR_FAILURE;
            scr_err("Error deleting directory: %s @ %s:%d", dir, __FILE__,
                    __LINE__);
        }
    }
...
int scr_storedescs_create()
{
    ...
    if (scr_nstoredescs > 0) {
        scr_storedescs =
            (scr_storedesc *)malloc(scr_nstoredescs * sizeof(scr_storedesc));
    ...
    for (i = 0; i < scr_nstoredescs; i++) {
        ...
        scr_hash *hash = scr_hash_get_kv_int(scr_storedesc_hash,
                                             SCR_CONFIG_KEY_STOREDESC, i);
        if (scr_storedesc_create_from_hash(&scr_storedescs[i], i, hash) !=
            SCR_SUCCESS) {
            all_valid = 0;
        }
    }
    ...
    if (scr_storedesc_cntl != NULL && index >= 0) {
        ...
        scr_abort(
            -1,
            "Failed to create store descriptor for control directory @ %s:%d",
            __FILE__, __LINE__);
    }
