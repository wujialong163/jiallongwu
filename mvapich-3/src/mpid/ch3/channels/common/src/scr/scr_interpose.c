...
/* This code builds a library which can be interpositioned into an existing
 * binary such that it may utilize SCR without requiring any changes to the
 * application or even a rebuild.  This library intercepts the following calls
 * made by the application: 1) MPI_Init() to call SCR_Init() after returning
 * from MPI_Init() 2) MPI_Finalize() to call SCR_Finalize() before calling
 * MPI_Finalize() 3) open()/fopen() to call SCR_Start_checkpoint() and/or
 * SCR_Route_file() before opening the file 4) close()/fclose() to call
 * SCR_Complete_checkpoint() after closing file
 *
...
struct scri_checkpointfile {
    ...
    int need_closed; /* whether checkpoint file is open and needs to be closed
...
static int scri_file_matches(const char *filename, regex_t *re)
{
    ...
    if (regexec(re, filename, nmatch, pmatch, 0) == 0 &&
        regexec(&scri_re_scr_file, filename, nmatch, pmatch, 0) ==
            REG_NOMATCH) {
        return 1;
    }
    return 0;
...
static int scri_complete_checkpoint(int index)
{
    if (scri_in_checkpoint) {
        ...
        for (i = 0; i < MAX_CHECKPOINT_FILES; i++) {
            if (scri_checkpoint_files[i].valid &&
                scri_checkpoint_files[i].need_closed) {
                still_open = 1;
            }
        }
        ...
        if (!still_open) {
            /* disable the interposer since SCR_Complete_checkpoint calls
             * open/close */
            scri_interpose_enabled = 0;
...
{
    if (scri_interpose_enabled && scri_checkpoint_dir_valid &&
...
static int scri_is_checkpoint_filename(const char *file)
{
    ...
    if (scri_interpose_enabled && i < MAX_CHECKPOINT_FILES &&
...
static int scri_add_checkpoint_fd(const char *file, const char *temp,
                                  const int fd, const int flags)
{
    int i = scri_index_by_filename(file);
    ...
    fprintf(stderr,
            "SCRI: ERROR: Too many checkpoint files open when registering %s, "
            "maximum supported is %d @ %s:%d\n",
...
/* record the fstream value and the mode used in the fopen call for this
 * filename */
static int scri_add_checkpoint_fstream(const char *file, const char *temp,
                                       const FILE *fstream, const char *mode)
{
    int i = scri_index_by_filename(file);
    if (i < MAX_CHECKPOINT_FILES) {
        scri_checkpoint_files[i].tempname = strdup(temp);
...
static int scri_define_checkpoint_dirname_regex(const char *dirname)
{
    ...
    if (rc != 0) {
        fprintf(stderr,
                "SCRI: ERROR: Checkpoint directory name regex compilation for "
                "%s failed (rc=%d) @ %s:%d\n",
...
/* given a regular expression for a checkpoint file, add it to our list and
...
static int scri_define_checkpoint_filename_regex(const char *filename)
{
    ...
    for (i = 0; i < MAX_CHECKPOINT_FILES; i++) {
        if (!scri_checkpoint_files[i].valid) {
            ...
            if (scri_checkpoint_files[i].filename == NULL) {
                fprintf(stderr,
                        "SCRI: ERROR: Failed to allocate space to record "
                        "filename regex for %s @ %s:%d\n",
            ...
            /* compile the filename regex pattern */
            int rc =
                regcomp(&scri_checkpoint_files[i].re, filename, REG_EXTENDED);
            if (rc != 0) {
                fprintf(stderr,
                        "SCRI: ERROR: Failed to compile filename regex %s "
                        "(rc=%d) @ %s:%d\n",
    ...
    /* couldn't find an empty slot for this file */
    fprintf(stderr,
            "SCRI: ERROR: Too many filename regex specified, maximum is %d @ "
            "%s:%d\n",
...
static int scri_define_checkpoint_filename_regex_by_rank(const char *filename)
{
    ...
    if (regexec(&scri_re_low_N, filename, nmatch, pmatch, 0) == 0) {
        ...
        fprintf(stderr,
                "SCRI: ERROR: Unknown MPI rank range for file: %s, perhaps "
                "specify '0-N:%s' @ %s:%d\n",
...
static int scri_define_checkpoint_files()
{
    ...
    /* read in the list of checkpoint file regular expressions separated by
     * token */
    ...
    /* override default token of ':' for separating checkpoint file regex in
     * pattern */
    ...
    if (pattern != NULL) {
        ...
        while (stop != NULL) {
            if (i >= MAX_CHECKPOINT_FILES) {
                fprintf(stderr,
                        "SCRI: ERROR: Rank %d: Too many files in "
                        "SCR_CHECKPOINT_PATTERN '%s' maximum allowed is %d @ "
                        "%s:%d\n",
                        scri_rank, pattern, MAX_CHECKPOINT_FILES, __FILE__,
                        __LINE__);
                exit(1);
            }
            *stop = '\0';
    ...
    /* check that every rank has at least one file (so we know when to complete
     * each checkpoint) */
    int i;
    int found_one = 0;
    for (i = 0; i < MAX_CHECKPOINT_FILES; i++) {
    ...
    if (!found_one) {
        fprintf(stderr,
                "SCRI: ERROR: Rank %d: No checkpoint file specified @ %s:%d\n",
...
static void scr_interpose_init()
{
    ...
    */
    ...
    if (scri_real_fopen == NULL) {
        scri_real_fopen =
            (FILE * (*)(const char *, const char *)) mydlsym("fopen");
    }
    ...
    /* interpose read/write functions */
    /*
      real_read  = (ssize_t (*)(int fd, void *buf, size_t count))
      mydlsym("read"); real_write = (ssize_t (*)(int fd, const void *buf, size_t
      count)) mydlsym("write");
    */
    ...
    if (!scri_re_low_high_compiled) {
        ...
        if (rc != 0) {
            fprintf(stderr,
                    "SCRI: ERROR: Failed to compile low-to-high range regex: "
                    "%s (rc=%d) @ %s:%d\n",
    ...
    if (!scri_re_low_N_compiled) {
        ...
        if (rc != 0) {
            fprintf(stderr,
                    "SCRI: ERROR: Failed to compile low-to-N range regex: %s "
                    "(rc=%d) @ %s:%d\n",
    ...
    if (!scri_re_scr_file_compiled) {
        ...
        if (rc != 0) {
            fprintf(stderr,
                    "SCRI: ERROR: Failed to compile scr file extension regex: "
                    "%s (rc=%d) @ %s:%d\n",
...
/* this function has the logic, the C and Fortran versions of MPI_Init route
 * here */
static int _MPI_Init(int *argc, char ***argv)
{
    ...
    /* parse our checkpoint files (we do this after MPI_Init so we know which
     * rank we are) */
...
int open(const char *pathname, int flags, ...)
{
    ...
    if (checkpoint) {
         ...
         */
        /* O_RDONLY == 0 so we can't do a straight bit test, instead check
         * whether either RDWR or WRONLY is set */
    ...
    if (checkpoint) {
        if (rc < 0) {
            /* Don't want to kick out here because user may have expected this
             * open to fail, e.g., read-only */
            fprintf(stderr,
                    "SCRI: ERROR: Failed to open %s for rerouting %s (errno=%d "
                    "%s) @ %s:%d\n",
...
int close(int fd)
{
    ...
    /* if fd matches a checkpoint file, call SCR_COMPLETE and then remove fd
     * from list */
...
FILE *fopen(const char *pathname, const char *mode)
{
    ...
    if (checkpoint) {
        if (rc == NULL) {
             ...
             * open to fail, e.g., read-only */
            fprintf(stderr,
                    "SCRI: ERROR: Failed to fopen %s for rerouting %s with "
                    "mode %s (errno=%d %s) @ %s:%d\n",
                    name, pathname, mode, errno, strerror(errno), __FILE__,
                    __LINE__);
        } else {
