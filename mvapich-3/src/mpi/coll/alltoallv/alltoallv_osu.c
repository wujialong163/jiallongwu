                             ...
                             MPIR_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (sendbuf == MPI_IN_PLACE) {
        ...
        for (i = 0; i < comm_size; ++i) {
            ...
            for (j = i; j < comm_size; ++j) {
                if (rank == i) {
                    ...
                    MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Alltoallv,mvp_coll_alltoallv,j,j
            ,1,start,end,sendrecv,recvcnts[i]*size1,recvcnts[i]*size1);
                    if (mpi_errno) {
                    ...
                    MPIR_Datatype_get_size_macro(recvtype, size1);
                    MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Alltoallv,mvp_coll_alltoallv,i,i
            ,1,start,end,sendrecv,recvcnts[i]*size1,recvcnts[i]*size1);
                    if (mpi_errno) {
                        /* for communication errors, just record the error but
                         * continue */
        ...
        for (i = 1; i < comm_size; i++) {
            ...
            MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Alltoallv,mvp_coll_alltoallv,dst,src
            ,1,start,end,sendrecv,sendcnts[dst]*size1,recvcnts[src]*size2);
            if (mpi_errno) {
                /* for communication errors, just record the error but
                 * continue */
                *errflag = MPIR_ERR_GET_CLASS(mpi_errno);
