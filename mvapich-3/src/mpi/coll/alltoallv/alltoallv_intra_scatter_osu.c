                                     ...
                                     MPIR_Errflag_t *errflag)
{
    ...
    for (i = 0, ii = 0; ii < comm_size; ii += bblock) {
        ...
        for (i = 0; i < ss; i++) {
            ...
            MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Alltoallv,mvp_coll_alltoallv,src,rank
            ,1,start,end,sendrecv,recvcnts[src]*size1,0);
        ...
        for (i = 0; i < ss; i++) {
            ...
            MPIR_Datatype_get_size_macro(sendtype_tmp, size1);
            MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Alltoallv,mvp_coll_alltoallv,rank,dst
            ,1,start,end,sendrecv,sendcnt_tmp[dst]*size1,0);
            MPIR_ERR_CHECK(mpi_errno);
            sreq_cnt++;
