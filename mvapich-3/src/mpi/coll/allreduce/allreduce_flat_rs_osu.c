                                ...
                                MPIR_Comm *comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    if (newrank != -1) {
        if ((HANDLE_GET_KIND(op) != HANDLE_KIND_BUILTIN) || (count < pof2)) {
            ...
            while (mask < pof2) {
                ...
                MPIR_Datatype_get_size_macro(datatype, size1);
                MPIR_Datatype_get_size_macro(datatype, size2);
                // printf("timer=%lf\n",time);
                // printf("send=%d recv=%d \n", MPIR_Rank_list_mapper(comm_ptr, dst),MPIR_Rank_list_mapper(comm_ptr, dst));
                MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Allreduce,mvp_coll_allreduce,MPIR_Rank_list_mapper(comm_ptr, dst),MPIR_Rank_list_mapper(comm_ptr, dst)
            ,1,start,end,sendrecv,count*size1,count*size2);
            ...
            while (mask < pof2) {
                ...
                // printf("send=%d recv=%d \n", MPIR_Rank_list_mapper(comm_ptr, dst),MPIR_Rank_list_mapper(comm_ptr, dst));
                MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Allreduce,mvp_coll_allreduce, MPIR_Rank_list_mapper(comm_ptr, dst),MPIR_Rank_list_mapper(comm_ptr, dst)
            ,1,start,end,sendrecv,send_cnt*size1,recv_cnt*size2);
            //     MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Allreduce,mvp_coll_allreduce,MPIR_Rank_list_mapper(comm_ptr, dst),rank
            // ,1,start,end,sendrecv,count,datatype);
                if (mpi_errno) {
                    /* for communication errors,
    ...
    if (rank < 2 * rem) {
        if (rank % 2) { /* odd */
            ...
            MPIR_Datatype_get_size_macro(datatype, size1);
            MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Allreduce,mvp_coll_allreduce,rank,MPIR_Rank_list_mapper(comm_ptr, rank - 1)
            ,1,start,end,sendrecv,count*siez1,0);
        } else { /* even */
            start=MPI_Wtime();
            MPIR_PVAR_INC(allreduce, pt2pt_rs, recv, count, datatype);
...

    {
        ...
        for (i = 1; i < comm_size; i++) {
            ...
            */
            ...
            ,1,start,end,sendrecv,chunk_count*size1,0);
        ...
        */
        ...
        for (i = 1; i < comm_size; i++) {
            ...
            MPIR_Datatype_get_size_macro(datatype, size1);
            MPI_PVAR_DETAIL_INFO_INC(MVP,MPI_T_Allreduce,mvp_coll_allreduce,rank,left
            ,1,start,end,sendrecv,chunk_count*size1,0);
